...
;(function () {
  ...
  const register$1 = (editor) => {
    ...
    registerOption('link_list', {
      processor: (value) =>
        isString(value) || isFunction(value) || isArrayOf(value, isObject)
    })
  ...
  const isAnchor = (elm) =>
    isNonNullable(elm) && elm.nodeName.toLowerCase() === 'a'
  ...
  const getHref = (elm) => {
    var _a, _b
    return (_b =
      (_a = elm.getAttribute('data-mce-href')) !== null && _a !== void 0
        ? _a
        : elm.getAttribute('href')) !== null && _b !== void 0
      ? _b
      : ''
  }
  const applyRelTargetRules = (rel, isUnsafe) => {
    ...
    const removeTargetRules = (rels) =>
      rels.filter((val) => global$4.inArray(rules, val) === -1)
  ...
  const isInAnchor = (editor, selectedElm) =>
  ...
  const isImageFigure = (elm) =>
    isNonNullable(elm) &&
    elm.nodeName === 'FIGURE' &&
    /\bimage\b/i.test(elm.className)
  ...
  const unlinkSelection = (editor) => {
    ...
    const startAnchorElm = dom.getParent(
      rng.startContainer,
      'a[href]',
      editor.getBody()
    )
    const endAnchorElm = dom.getParent(
      rng.endContainer,
      'a[href]',
      editor.getBody()
    )
  ...
  const link = (editor, attachState, data) => {
    ...
    editor.hasPlugin('rtc', true)
      ...
      : linkDomMutation(editor, attachState, sanitizedData)
  }
  const unlink = (editor) => {
    editor.hasPlugin('rtc', true)
      ? editor.execCommand('unlink')
  ...
  const unlinkImageFigure = (editor, fig) => {
    ...
    if (img) {
      ...
      if (a) {
        ;(_a = a.parentNode) === null || _a === void 0
          ? void 0
          : _a.insertBefore(img, a)
        editor.dom.remove(a)
      }
    }
  }
  const linkImageFigure = (dom, fig, attrs) => {
    ...
    if (img) {
      ...
      ;(_a = img.parentNode) === null || _a === void 0
        ? void 0
        : _a.insertBefore(a, img)
      a.appendChild(img)
    }
  }

  const isListGroup = (item) => hasNonNullableKey(item, 'items')
  const findTextByValue = (value, catalog) =>
    findMap(catalog, (item) => {
      if (isListGroup(item)) {
  ...
  const getDelta = (persistentText, fieldName, catalog, data) => {
    ...
    return value !== undefined
      ? findTextByValue(value, catalog).map((i) => ({
          url: {
            value: i.value,
            meta: {
  ...
  const init = (initialData, linkCatalog) => {
    ...
    const getTitleFromUrlChange = (url) => {
      ...
      return someIf(
        ...
        Optional.from(
          (_a = url.meta) === null || _a === void 0 ? void 0 : _a.title
        ).getOr('')
      )
    }
    const getTextFromUrlChange = (url) => {
      ...
      return someIf(
        ...
        Optional.from(
          (_a = url.meta) === null || _a === void 0 ? void 0 : _a.text
        ).getOr(url.value)
      )
    }
  ...
  const tryProtocolTransform =
    (assumeExternalTargets, defaultLinkProtocol) => (data) => {
      const url = data.href
  ...
  const getLinks = (editor) => {
    const extractor = (item) =>
      editor.convertURL(item.value || item.url || '', 'href')
    const linkList = getLinkList(editor)
    return new Promise((resolve) => {
      if (isString(linkList)) {
        fetch(linkList)
          .then((res) =>
            res.ok ? res.text().then(parseJson) : Promise.reject()
          )
  ...
  const getRels = (editor, initialTarget) => {
    ...
    if (list.length > 0) {
      ...
      const safeRelExtractor = (item) =>
        applyRelTargetRules(ListOptions.getValue(item), isTargetBlank)
  ...
  const extractFromAnchor = (editor, anchor) => {
    ...
    const text = onlyText
      ...
      : Optional.none()
    const url = anchor.bind((anchorElm) =>
      Optional.from(dom.getAttrib(anchorElm, 'href'))
    )
    const target = anchor.bind((anchorElm) =>
      Optional.from(dom.getAttrib(anchorElm, 'target'))
    )
    const rel = anchor.bind((anchorElm) => nonEmptyAttr(dom, anchorElm, 'rel'))
    const linkClass = anchor.bind((anchorElm) =>
      nonEmptyAttr(dom, anchorElm, 'class')
    )
    const title = anchor.bind((anchorElm) =>
      nonEmptyAttr(dom, anchorElm, 'title')
    )
    return {
      url,
      text,
  ...
  const handleSubmit = (editor, info) => (api) => {
    ...
    const getChangedValue = (key) =>
      Optional.from(data[key]).filter((value) => !is(info.anchor[key], value))
    const changedData = {
      href: data.url.value,
  ...
  const register = (editor) => {
    editor.addCommand('mceLink', (_ui, value) => {
      if (
        (value === null || value === void 0 ? void 0 : value.dialog) === true ||
        !useQuickLink(editor)
      ) {
        open$1(editor)
      } else {
    ...
    const updateState = () =>
      api.setActive(
        !editor.mode.isReadOnly() &&
          isInAnchor(editor, editor.selection.getNode())
      )
    updateState()
  ...
  const toggleEnabledState = (editor) => (api) => {
    const updateState = () =>
      api.setEnabled(isInAnchor(editor, editor.selection.getNode()))
    updateState()
    return toggleState(editor, updateState)
  }
  const toggleUnlinkState = (editor) => (api) => {
    const hasLinks$1 = (parents) =>
  ...
  const setupContextMenu = (editor) => {
    ...
    editor.ui.registry.addContextMenu('link', {
      update: (element) =>
        hasLinks(editor.dom.getParents(element, 'a')) ? inLink : noLink
    })
  }
  const setupContextToolbars = (editor) => {
    ...
    editor.ui.registry.addContextForm('quicklink', {
      ...
      label: 'Link',
      predicate: (node) =>
        hasContextToolbar(editor) && isInAnchor(editor, node),
      initValue: () => {
