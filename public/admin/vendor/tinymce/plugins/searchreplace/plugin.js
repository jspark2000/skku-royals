...
;(function () {
  ...
  const isSimpleBoundary = (dom, node) =>
    dom.isBlock(node) || has(dom.schema.getVoidElements(), node.nodeName)
  const isContentEditableFalse = (dom, node) =>
    dom.getContentEditable(node) === 'false'
  const isContentEditableTrueInCef = (dom, node) =>
    dom.getContentEditable(node) === 'true' &&
    node.parentNode &&
    dom.getContentEditableParent(node.parentNode) === 'false'
  const isHidden = (dom, node) =>
    !dom.isBlock(node) && has(dom.schema.getWhitespaceElements(), node.nodeName)
  ...
  const collectTextToBoundary = (dom, section, node, rootNode, forwards) => {
    ...
    const rootBlock =
      (_a = dom.getParent(rootNode, dom.isBlock)) !== null && _a !== void 0
        ? _a
        : dom.getRoot()
    const walker = new global(node, rootBlock)
    const walkerFn = forwards
      ? walker.next.bind(walker)
      : walker.prev.bind(walker)
    ...
    return collect(
      ...
      endNode,
      {
        ...
        cef: (node) => {
          ...
          return sort(sections, (a, b) =>
            documentPositionPreceding(a.elements[0].dom, b.elements[0].dom)
              ? 1
              : -1
          )
        }
  ...
  const findSpansByIndex = (editor, index) => {
    const spans = []
    const nodes = global$1.toArray(
      editor.getBody().getElementsByTagName('span')
    )
    if (nodes.length) {
  ...
  const escapeSearchText = (text, wholeWord) => {
    ...
    return wholeWord
      ? `(?:^|\\s|${punctuation()})` + wordRegex + `(?=$|\\s|${punctuation()})`
      : wordRegex
  }
  const find = (
    editor,
  ...
  const hasNext = (editor, currentSearchState) =>
    currentSearchState.get().count > 1
  const hasPrev = (editor, currentSearchState) =>
    currentSearchState.get().count > 1
  ...
  const open = (editor, currentSearchState) => {
    ...
    const selectedText = global$1.trim(
    ...
    const focusButtonIfRequired = (api, name) => {
      if (
        global$2.browser.isSafari() &&
        global$2.deviceType.isTouch() &&
