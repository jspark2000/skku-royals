...
;(function () {
  ...
  const matchNodeName = (name) => (node) =>
    isNonNullable(node) && node.nodeName.toLowerCase() === name
  const matchNodeNames = (regex) => (node) =>
    isNonNullable(node) && regex.test(node.nodeName)
  ...
  const isTextBlock = (editor, node) =>
    isNonNullable(node) && node.nodeName in editor.schema.getTextBlockElements()
  const isBlock = (node, blockElements) =>
    isNonNullable(node) && node.nodeName in blockElements
  ...
  const isParentListSelected = (parentList, selectedBlocks) =>
    isNonNullable(parentList) &&
    selectedBlocks.length === 1 &&
    selectedBlocks[0] === parentList
  const findSubLists = (parentList) =>
    filter$1(parentList.querySelectorAll(listSelector), isListNode)
  ...
  const getSelectedDlItems = (editor) =>
    filter$1(getSelectedListItems(editor), isDlItemNode)
  const getClosestEditingHost = (editor, elm) => {
    const parentTableCell = editor.dom.getParents(elm, 'TD,TH')
    return parentTableCell.length > 0 ? parentTableCell[0] : editor.getBody()
  }
  const isListHost = (schema, node) =>
    !isListNode(node) &&
    !isListItemNode(node) &&
    exists(listNames, (listName) =>
      schema.isValidChild(node.nodeName, listName)
    )
  ...
  const getSelectedLists = (editor) => {
    ...
    const subsequentLists = filter$1(
  ...
  const getUniqueListRoots = (editor, lists) => {
    const listRoots = map(lists, (list) =>
  ...
  const fireListEvent = (editor, action, element) =>
    editor.dispatch('ListMutation', {
      action,
      element
    })
  ...
  const closestSiblingEntry = (entries, start) => {
    ...
    return findUntil(reverse(entries.slice(0, start)), matches, until).orThunk(
      () => findUntil(entries.slice(start + 1), matches, until)
    )
  }
      ...
      .fold(
        () => {
          ...
          const childListEntries = lastChild(item)
            .filter(isList)
            .map((list) =>
              parseList(depth, itemSelection, selectionState, list)
            )
            .getOr([])
  ...
  const composeEntries = (editor, entries) =>
    bind(groupBy(entries, isIndented), (entries) => {
  ...
  const indentSelectedEntries = (entries, indentation) => {
    each$1(filter$1(entries, isSelected), (entry) =>
  ...
  const handleIndentation = (editor, indentation) =>
    !selectionIsWithinNonEditableList(editor) &&
    selectionIndentation(editor, indentation)
  ...
  const resolveBookmark = (bookmark) => {
    const restoreEndPoint = (start) => {
      const nodeIndex = (container) => {
        var _a
        let node =
          (_a = container.parentNode) === null || _a === void 0
            ? void 0
            : _a.firstChild
        let idx = 0
        while (node) {
  ...
  const isValidLists = (list1, list2) => {
    return (
      isListNode(list1) &&
      list1.nodeName ===
        (list2 === null || list2 === void 0 ? void 0 : list2.nodeName)
    )
  }
  ...
  const findNextCaretContainer = (editor, rng, isForward, root) => {
    ...
    const walkFn = isForward
      ? walker.next.bind(walker)
      : walker.prev2.bind(walker)
    while ((node = walkFn())) {
  ...
  const moveChildren = (dom, fromElm, toElm) => {
    let node
    const targetElm = hasOnlyOneBlockChild(dom, toElm)
      ? toElm.firstChild
      : toElm
  ...
  const backspaceDeleteIntoListCaret = (editor, isForward) => {
    ...
    if (block && dom.isEmpty(block)) {
      ...
      if (otherLi) {
        const findValidElement = (element) =>
          contains$1(['td', 'th', 'caption'], name(element))
        const findRoot = (node) => node.dom === root
        const otherLiCell = closest(
          SugarElement.fromDom(otherLi),
          findValidElement,
          findRoot
        )
        const caretCell = closest(
  ...
  const backspaceDelete = (editor, isForward) => {
    ...
    return (
      !isWithinNonEditableList(editor, selection.getNode()) &&
  ...
  const open = (editor) => {
    ...
    if (
      !isOlNode(currentList) ||
      isWithinNonEditableList(editor, currentList)
    ) {
      return
    }
    editor.windowManager.open({
      ...
      initialData: {
        start: parseDetail({
          ...
          listStyleType: Optional.from(
            editor.dom.getStyle(currentList, 'list-style-type')
          )
        })
      },
      buttons: [
  ...
  const setupMenuButtonHandler = (editor, listName) => (api) => {
    const menuButtonHandler = (e) =>
      api.setEnabled(
        inList(e.parents, listName) &&
          !isWithinNonEditableList(editor, e.element)
      )
