...
<p>When a tabbed dialog is opened, the first button in the tab menu is focused. Pressing tab will navigate to the first interactive component in that tab, and will cycle through the tab\u2019s components, the footer buttons, then back to the tab button. To switch to another tab, focus the tab button for the current tab, then use the arrow keys to cycle through the tab buttons.</p>`
  ...
  const tab$1 = (editor) => {
    const availablePlugins = () => {
      ...
      const sortedPremiumPlugins = sort(
        map(premiumPlugins, (p) => p.name),
        (s1, s2) => s1.localeCompare(s2)
      )
      const premiumPluginList = map(
        sortedPremiumPlugins,
        (pluginName) => `<li>${pluginName}</li>`
      ).join('')
      return (
    ...
    const getPluginData = (editor, key) =>
      find(urls, (x) => {
        return x.key === key
      }).fold(
        () => {
    ...
    const getPluginKeys = (editor) => {
      ...
      return isUndefined(forcedPlugins)
        ? keys$1
        : filter(keys$1, (k) => !contains(forcedPlugins, k))
    }
    const pluginLister = (editor) => {
      const pluginKeys = getPluginKeys(editor)
      const sortedPluginData = sort(
        map(pluginKeys, (k) => getPluginData(editor, k)),
  ...
  const tab = () => {
    const getVersion = (major, minor) =>
      major.indexOf('@') === 0 ? 'X.X.X' : major + '.' + minor
