...
;(function () {
  ...
  const columnGroups = (ancestor) =>
    table(ancestor).fold(constant([]), (table) => children$1(table, 'colgroup'))
  ...
  const generate$1 = (list) => {
    ...
    const { columns, colgroups } = last(colgroupRows)
      .map((rowData) => {
  ...
  const getAlignment = (formats, formatName, editor, elm) =>
    find(
      formats,
      (name) => !isUndefined(editor.formatter.matchNode(elm, formatName + name))
    ).getOr('')
  ...
  const extractDataFromTableElement = (editor, elm, hasAdvTableTab) => {
    ...
    return {
      ...
      cellspacing:
        cellspacing !== null && cellspacing !== void 0 ? cellspacing : '',
      cellpadding:
        cellpadding !== null && cellpadding !== void 0 ? cellpadding : '',
