...
;(function () {
  ...
  const filter = always
  const bind$1 = (element, event, handler) =>
  ...
  const isFullscreenElement = (elem) =>
    elem.dom === getFullscreenElement(owner(elem).dom)

  const ancestors$1 = (scope, predicate, isRoot) =>
    filter$1(parents(scope, isRoot), predicate)
  const siblings$1 = (scope, predicate) =>
    filter$1(siblings$2(scope), predicate)
  ...
  const toggleFullscreen = (editor, fullscreenState) => {
    ...
    const iframeStyle =
      iframe === null || iframe === void 0 ? void 0 : iframe.style
    ...
    const cleanup = () => {
      ...
      Optional.from(fullscreenState.get()).each((info) =>
        info.fullscreenChangeHandler.unbind()
      )
    }
    if (!fullscreenInfo) {
