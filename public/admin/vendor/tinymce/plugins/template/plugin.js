...
;(function () {
  ...
  const getPreviewContent = (editor, html) => {
    ...
    if (html.indexOf('<html>') === -1) {
      ...
      const cors = shouldUseContentCssCors(editor)
  ...
  const open = (editor, templateList) => {
    const createTemplates = () => {
      ...
      return Optional.from(
        global$1.map(templateList, (template, index) => {
    ...
    const openDialog = (templates) => {
      ...
      const updateDialog = (dialogApi, template, previewHtml) => {
        ...
        const bodyItems = [
          ...
          {
            type: 'htmlpanel',
            html: `<p aria-live="polite">${htmlEscape(
  ...
  const register$1 = (editor) => {
    ...
    editor.addCommand(
      'mceTemplate',
      createTemplateList(editor, showDialog(editor))
    )
  }
