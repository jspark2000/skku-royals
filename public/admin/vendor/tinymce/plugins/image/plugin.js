...
;(function () {
  ...
  const is = (value, constructor) =>
    isObject(value) &&
    hasProto(value, constructor, (o, proto) => getPrototypeOf(o) === proto)
  ...
  const hasUploadUrl = (editor) =>
    isNotEmpty(editor.options.get('images_upload_url'))
  const hasUploadHandler = (editor) =>
    isNonNullable(editor.options.get('images_upload_handler'))
  ...
  const readImageDataFromSelection = (editor) => {
    ...
    return image
      ? read((css) => normalizeCss$1(editor, css), image)
      : defaultData()
  }
  ...
  const findEntryDelegate = (list, value) =>
    findMap(list, (item) => {
      if (isGroup(item)) {
  ...
  const makeItems = (info) => {
    ...
    const getDialogContainerType = (useColumns) =>
      useColumns
        ? {
            type: 'grid',
            columns: 2
  ...
  const toImageData = (data, removeEmptyAlt) => ({
    src: data.src.value,
    alt:
      (data.alt === null || data.alt.length === 0) && removeEmptyAlt
        ? null
        : data.alt,
    title: data.title,
  ...
  const serializeStyle = (editor) => (stylesArg, name) =>
    editor.dom.serializeStyle(stylesArg, name)
  const uploadImage = (editor) => (blobInfo) =>
    global$1(editor)
  ...
  const register = (editor) => {
    ...
    editor.ui.registry.addContextMenu('image', {
      update: (element) =>
        isFigure(element) || (isImage(element) && !isPlaceholderImage(element))
          ? ['image']
          : []
    })
  }
