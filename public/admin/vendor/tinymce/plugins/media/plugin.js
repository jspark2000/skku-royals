...
;(function () {
  ...
  const guess = (url) => {
    ...
    const fileEnd =
      (_a = url.toLowerCase().split('.').pop()) !== null && _a !== void 0
        ? _a
        : ''
  ...
  const dataToHtml = (editor, dataIn) => {
    ...
    if (!data.source) {
      global$5.extend(
        data,
        htmlToData(
          (_a = data.embed) !== null && _a !== void 0 ? _a : '',
          editor.schema
        )
      )
      if (!data.source) {
  ...
  const isMediaElement = (element) =>
    element.hasAttribute('data-mce-object') ||
    element.hasAttribute('data-ephox-embed-iri')
  ...
  const defaultPromise = (data, dataToHtml) =>
    Promise.resolve({
  ...
  const extractMeta = (sourceInput, data) =>
    get$1(data, sourceInput).bind((mainData) => get$1(mainData, 'meta'))
  const getValue = (data, metaData, sourceInput) => (prop) => {
    const getFromData = () => get$1(data, prop)
    const getFromMetaData = () => get$1(metaData, prop)
    const getNonEmptyValue = (c) =>
      get$1(c, 'value').bind((v) =>
        v.length > 0 ? Optional.some(v) : Optional.none()
      )
    ...
    const getFromMetaFirst = () =>
      getFromMetaData().orThunk(() =>
        getFromData().bind((child) =>
          isObject(child) ? getNonEmptyValue(child) : Optional.from(child)
        )
      )
    return {
  ...
  const getDimensions = (data, metaData) => {
    ...
    get$1(data, 'dimensions').each((dims) => {
      each$1(['width', 'height'], (prop) => {
        get$1(metaData, prop)
          .orThunk(() => get$1(dims, prop))
          .each((value) => (dimensions[prop] = value))
      })
    })
    return dimensions
  }
  const unwrap = (data, sourceInput) => {
    const metaData =
      sourceInput && sourceInput !== 'dimensions'
        ? extractMeta(sourceInput, data).getOr({})
        : {}
  ...
  const submitForm = (prevData, newData, editor) => {
    var _a
    newData.embed = updateHtml(
      (_a = newData.embed) !== null && _a !== void 0 ? _a : '',
      newData,
      false,
      editor.schema
    )
  ...
  const createPreviewNode = (editor, node) => {
    ...
    const styles = editor.dom.parseStyle(
      (_a = node.attr('style')) !== null && _a !== void 0 ? _a : ''
    )
  ...
  const retainAttributesAndInnerHtml = (editor, sourceNode, targetNode) => {
    var _a
    const attribs =
      (_a = sourceNode.attributes) !== null && _a !== void 0 ? _a : []
  ...
  const isWithinEmbedWrapper = (node) => {
    ...
    while ((tempNode = tempNode.parent)) {
      if (
        tempNode.attr('data-ephox-embed-iri') ||
        isPageEmbedWrapper(tempNode)
      ) {
        return true
      }
    }
    ...
    editor.on('PreInit', () => {
      ...
      serializer.addAttributeFilter('data-mce-object', (nodes, name) => {
        ...
        while (i--) {
          ...
          const attribs =
            (_a = node.attributes) !== null && _a !== void 0 ? _a : []
          let ai = attribs.length
          while (ai--) {
            const attrName = attribs[ai].name
          ...
          if (innerHtml) {
            const fragment = parseAndSanitize(
              editor,
              realElmName,
              unescape(innerHtml)
            )
  ...
  const register = (editor) => {
    ...
    editor.ui.registry.addToggleButton('media', {
      ...
      onSetup: (buttonApi) => {
        ...
        return selection.selectorChangedWithUnbind(
