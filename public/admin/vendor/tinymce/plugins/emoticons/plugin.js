...
;(function () {
  ...
  const translateCategory = (categories, name) =>
    has(categories, name) ? categories[name] : name
  ...
  const initDatabase = (editor, databaseUrl, databaseId) => {
    ...
    const getEmoji = (lib) => {
      if (startsWith(lib.char, '<img')) {
        return lib.char.replace(
          /src="([^"]+)"/,
          (match, url) => `src="${emojiImagesUrl}${url}"`
        )
      } else {
        return lib.char
      }
    ...
    editor.on('init', () => {
      global.load(databaseId, databaseUrl).then(
        (emojis) => {
  ...
  const emojiMatches = (emoji, lowerCasePattern) =>
