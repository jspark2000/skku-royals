...
;(function () {
  ...
  const getText = (node, schema) => {
    ...
    const isNewline = (node) =>
      blockElements[node.nodeName] || voidElements[node.nodeName]
    const textBlocks = []
    let txt = ''
  ...
  const createBodyCounter = (editor, count) => () =>
    count(editor.getBody(), editor.schema)
  const createSelectionCounter = (editor, count) => () =>
    count(editor.selection.getRng().cloneContents(), editor.schema)
  const createBodyWordCounter = (editor) =>
