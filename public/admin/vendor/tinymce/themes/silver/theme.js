...
;(function () {
  ...
  const eq$1 = (t) => (a) => t === a
  const is$2 = (value, constructor) =>
    isObject(value) &&
    hasProto(value, constructor, (o, proto) => getPrototypeOf$1(o) === proto)
  ...
  const is$1 = (lhs, rhs, comparator = tripleEquals) =>
  ...
  const lift3 = (oa, ob, oc, f) =>
    oa.isSome() && ob.isSome() && oc.isSome()
      ...
      : Optional.none()
  const mapFrom = (a, f) =>
    a !== undefined && a !== null ? Optional.some(f(a)) : Optional.none()
  ...
  const isHTMLElement = (element) =>
    isElement$1(element) && isPrototypeOf(element.dom)
  ...
  const all$3 = (selector, scope) => {
    ...
    return bypassSelector(base)
      ? []
      : map$2(base.querySelectorAll(selector), SugarElement.fromDom)
  }
  ...
  const offsetParent = (element) =>
    Optional.from(element.dom.offsetParent).map(SugarElement.fromDom)
  const nextSibling = (element) =>
    Optional.from(element.dom.nextSibling).map(SugarElement.fromDom)
  const children = (element) =>
    map$2(element.dom.childNodes, SugarElement.fromDom)
  ...
  const getUnsafeProperty = (dom, property) =>
    isSupported$1(dom) ? dom.style.getPropertyValue(property) : ''
  ...
  const absolute$3 = (element) => {
    ...
    return viewport$1(element).translate(
  ...
  const capture$1 = (element, event, filter, handler) =>
    binder(element, event, filter, handler, true)
  ...
  const fromOption = (optional, err) =>
    optional.fold(() => error$1(err), value$4)
  const Result = {
    value: value$4,
    error: error$1,
    fromOption
  ...
  const mergeErrors$1 = (errors) =>
    compose(SimpleResult.serror, flatten)(errors)
  ...
  const missingKey = (path, key) =>
    nu$a(path, () => 'Choice schema did not contain choice key: "' + key + '"')
  ...
  const unsupportedFields = (path, unsupported) =>
    nu$a(
      path,
      () =>
  ...
  const requiredAccess = (path, obj, key, bundle) =>
    get$g(obj, key).fold(() => missingRequired(path, key, obj), bundle)
  ...
  const optionDefaultedAccess = (obj, key, fallback, bundle) => {
    const opt = get$g(obj, key).map((val) =>
  ...
  const objOf = (values) => {
    ...
    const toString = () => {
      const fieldStrings = map$2(values, (value) =>
        fold(
          value,
  ...
  const oneOf = (props, rawF) => {
    ...
    const toString = () =>
      'oneOf(' + map$2(props, (prop) => prop.toString()).join(', ') + ')'
    return {
      extract,
      toString
    }
  ...
  const setOf$1 = (validator, prop) => {
    const validateKeys = (path, keys) =>
      arrOf(value$3(validator)).extract(path, keys)
    const extract = (path, o) => {
      const keys$1 = keys(o)
  ...
  const valueOf = (validator) =>
    value$3((v) => validator(v).fold(SimpleResult.serror, SimpleResult.svalue))
  const setOf = (validator, prop) =>
    setOf$1((v) => SimpleResult.fromResult(validator(v)), prop)
  ...
  const requiredStringEnum = (key, values) =>
    field$1(key, key, required$2(), validateEnum(values))
  const requiredBoolean = (key) => requiredOf(key, boolean)
  const requiredFunction = (key) => requiredOf(key, functionProcessor)
  const forbid = (key, message) =>
    field$1(
      key,
      key,
      asOption(),
      value$3((_v) =>
        SimpleResult.serror('The field: ' + key + ' is forbidden. ' + message)
      )
    )
  const requiredObjOf = (key, objSchema) =>
    field$1(key, key, required$2(), objOf(objSchema))
  const requiredArrayOfObj = (key, objFields) =>
    field$1(key, key, required$2(), arrOfObj(objFields))
  const requiredArrayOf = (key, schema) =>
    field$1(key, key, required$2(), arrOf(schema))
  ...
  const optionObjOfOnly = (key, objSchema) =>
    optionOf(key, objOfOnly(objSchema))
  const defaulted = (key, fallback) =>
    field$1(key, key, defaulted$1(fallback), anyValue())
  const defaultedOf = (key, fallback, schema) =>
    field$1(key, key, defaulted$1(fallback), schema)
  ...
  const defaultedPostMsg = (key, fallback) =>
    defaultedOf(key, fallback, postMessageable)
  const defaultedArrayOf = (key, fallback, schema) =>
    defaultedOf(key, fallback, arrOf(schema))
  const defaultedObjOf = (key, fallback, objSchema) =>
  ...
  const closest$4 = (scope, transform, isRoot) => {
    ...
    return current.orThunk(() =>
      stop(scope) ? Optional.none() : ancestor$2(scope, transform, stop)
    )
  }

  const isSource = (component, simulatedEvent) =>
    eq(component.element, simulatedEvent.event.target)
  ...
  const all$2 =
    ...
    (...args) =>
      foldl(
        handlers,
        (acc, handler) => acc && f(handler).apply(undefined, args),
        true
      )
  const any =
    (handlers, f) =>
  ...
  const runWithTarget = (name, f) => {
    return run$1(name, (component, simulatedEvent) => {
      ...
      const target = component
        ...
        .getByDom(ev.target)
        .getOrThunk(() => {
          const closest = closest$4(
            ev.target,
            (el) => component.getSystem().getByDom(el).toOptional(),
            never
          )
  ...
  const isRecursive = (component, originator, target) =>
    eq(originator, component.element) && !eq(originator, target)
  const events$i = derive$2([
    can(focus$4(), (component, simulatedEvent) => {
  ...
  const NoContextApi = (getComp) => {
    const getMessage = (event) =>
      `The component must be in a context to execute: ${event}` +
  ...
  const cleanParameters = (parameters) =>
    map$2(parameters, (p) =>
      endsWith(p, '/*') ? p.substring(0, p.length - '/*'.length) : p
    )
  ...
  const makeApi = (f) =>
    markAsSketchApi(
      (component, ...rest) => f(component.getApis(), component, ...rest),
      f
    )

  const NoState = {
    init: () => nu$8({ readState: constant$1('No State required') })
  }
  const nu$8 = (spec) => spec

  const generateFrom$1 = (spec, all) => {
    const schema = map$2(all, (a) =>
      optionObjOf(a.name(), [required$1('config'), defaulted('state', NoState)])
    )
    ...
    const r = mapToArray(byEventName, (tuples, eventName) => {
      const combined =
        tuples.length === 1
          ...
          : fuse(tuples, eventOrder, eventName)
      return combined.map((handler) => {
        const assembled = assemble(handler)
        const purpose =
          tuples.length > 1
            ? filter$2(eventOrder[eventName], (o) =>
                exists(tuples, (t) => t.name === o)
              ).join(' > ')
            : tuples[0].name
  ...
  const toModification = (detail) =>
    detail.domModification.fold(() => nu$7({}), nu$7)
  ...
  const hasAll = (element, classes) =>
    forall(classes, (clazz) => has(element, clazz))
  ...
  const get$7 = (element) =>
    supports(element) ? getNative(element) : get$8(element)
  ...
  const reconcileToDom = (definition, obsoleted) => {
    ...
    const { toSet: attrsToSet, toRemove: attrsToRemove } = diffKeyValueSet(
    ...
    const { toSet: stylesToSet, toRemove: stylesToRemove } = diffKeyValueSet(
      definition.styles,
      existingStyles
    )
    const updateStyles = () => {
  ...
  const hasMixedChildren = (definition) =>
    definition.innerHtml.isSome() && definition.domChildren.length > 0
  const renderToDom = (definition, optObsoleted) => {
    const canBePatched = (candidate) =>
      name$3(candidate) === definition.tag &&
      !hasMixedChildren(definition) &&
      !isPremade(candidate)
  ...
  const getDomDefinition = (info, bList, bData) => {
    ...
    const modification =
      bList.length > 0
        ? combine$2(bData, baseModification, bList, definition)
        : infoModification
  ...
  const build$2 = (spec, obsoleted) => {
    ...
    const syncComponents = () => {
      ...
      const subs = bind$3(children$1, (child) =>
        systemApi
          .get()
          .getByDom(child)
          .fold(() => [], pure$2)
  ...
  const buildSubcomponents = (spec, obsoleted) => {
    ...
    return obsoleted.fold(
      () => map$2(components, build$1),
      (obs) =>
        map$2(components, (c, i) => {
  ...
  const external$1 = (spec) => {
    const extSpec = asRawOrDie$1(
      'external.component',
      objOfOnly([required$1('element'), option$3('uid')]),
      spec
    )
  ...
  const buildOrPatch = (spec, obsoleted) =>
    getPremade(spec).getOrThunk(() => {
      const userSpecWithUid = isSketchSpec$1(spec)
        ? spec
        : {
            uid: uids(''),
            ...spec
  ...
  const closest$2 = (scope, predicate, isRoot) =>
    closest$3(scope, predicate, isRoot).isSome()

  const ancestor = (scope, selector, isRoot) =>
    ancestor$1(scope, (e) => is(e, selector), isRoot)
  ...
  const isAriaPartOf = (component, queryElem) =>
    find$1(queryElem).exists((owner) => isPartOf$1(component, owner))
  const isPartOf$1 = (component, queryElem) =>
    closest$2(queryElem, (el) => eq(el, component.element), never) ||
  ...
  const processEvent = (eventName, initialTarget, f) => {
    const status = get$g(eventConfig.get(), eventName)
      .orThunk(() => {
        const patterns = keys(eventConfig.get())
        return findMap(patterns, (p) =>
          eventName.indexOf(p) > -1
    ...
    if (err.stack !== undefined) {
      ...
      return find$5(
        ...
        (line) =>
          ...
          !exists(path, (p) => line.indexOf(p) > -1)
      ).getOr(unknown)
    } else {
      return unknown
    }
  ...
  const tieredMenuMarkers = () =>
    requiredObjOf(
      'markers',
      [required$1('backgroundMenu')].concat(menuFields()).concat(itemFields())
    )
  const markers$1 = (required) =>
    requiredObjOf('markers', map$2(required, required$1))
  ...
  const onHandler = (fieldName) =>
    onPresenceHandler('onHandler', fieldName, defaulted$1(noop))
  const onKeyboardHandler = (fieldName) =>
    onPresenceHandler(
      'onKeyboardHandler',
      fieldName,
      defaulted$1(Optional.none)
    )
  const onStrictHandler = (fieldName) =>
    onPresenceHandler('onHandler', fieldName, required$2())
  const onStrictKeyboardHandler = (fieldName) =>
    onPresenceHandler('onKeyboardHandler', fieldName, required$2())
  ...
  const cataVertical = (subject, south, middle, north) =>
    subject.fold(south, south, north, north, south, north, middle, middle)
  const cataHorizontal = (subject, east, middle, west) =>
    subject.fold(east, west, east, west, middle, middle, east, west)
  ...
  const boundsRestriction = (anchor, restrictions) =>
    mapToObject(['left', 'right', 'top', 'bottom'], (dir) =>
      get$g(restrictions, dir).map((restriction) =>
        getRestriction(anchor, restriction)
      )
    )
  const adjustBounds = (bounds$1, restriction, bubbleOffset) => {
    const applyRestriction = (dir, current) =>
      restriction[dir]
        .map((pos) => {
          const isVerticalAxis = dir === 'top' || dir === 'bottom'
          const offset = isVerticalAxis ? bubbleOffset.top : bubbleOffset.left
          const comparator =
            dir === 'left' || dir === 'top' ? Math.max : Math.min
          const newPos = comparator(pos, current) + offset
          return isVerticalAxis
            ? clamp(newPos, bounds$1.y, bounds$1.bottom)
            : clamp(newPos, bounds$1.x, bounds$1.right)
        })
        .getOr(current)
  ...
  const middleX$1 = (anchor, element) =>
  ...
  const centreY$1 = (anchor, element) =>
    anchor.y + anchor.height / 2 - element.height / 2
  const eastEdgeX$1 = (anchor) => anchor.x + anchor.width
  const westEdgeX$1 = (anchor, element) => anchor.x - element.width
  const southeast$2 = (anchor, element, bubbles) =>
    nu$6(
      eastX$1(anchor),
      southY$2(anchor),
      bubbles.southeast(),
  ...
  const southwest$2 = (anchor, element, bubbles) =>
    nu$6(
  ...
  const northeast$2 = (anchor, element, bubbles) =>
    nu$6(
      eastX$1(anchor),
      northY$2(anchor, element),
      bubbles.northeast(),
      northeast$3(),
      'northeast',
      boundsRestriction(anchor, {
  ...
  const northwest$2 = (anchor, element, bubbles) =>
    nu$6(
      westX$1(anchor, element),
      northY$2(anchor, element),
      bubbles.northwest(),
  ...
  const north$2 = (anchor, element, bubbles) =>
    nu$6(
      ...
      north$3(),
      'north',
      boundsRestriction(anchor, { bottom: 2 }),
      labelPrefix$2
    )
  const south$2 = (anchor, element, bubbles) =>
    nu$6(
      middleX$1(anchor, element),
      southY$2(anchor),
      bubbles.south(),
      south$3(),
      'south',
      boundsRestriction(anchor, { top: 3 }),
      labelPrefix$2
    )
  const east$2 = (anchor, element, bubbles) =>
    nu$6(
      eastEdgeX$1(anchor),
      centreY$1(anchor, element),
      bubbles.east(),
      east$3(),
      'east',
      boundsRestriction(anchor, { left: 0 }),
      labelPrefix$2
    )
  const west$2 = (anchor, element, bubbles) =>
    nu$6(
      westEdgeX$1(anchor, element),
      centreY$1(anchor, element),
      bubbles.west(),
      west$3(),
      'west',
      boundsRestriction(anchor, { right: 1 }),
      labelPrefix$2
    )
  const all$1 = () => [
    southeast$2,
  ...
  const chooseChannels = (channels, message) =>
    message.universal
      ? channels
      : filter$2(channels, (ch) => contains$2(message.channels, ch))
  const events$h = (receiveConfig) =>
    derive$2([
  ...
  const executeEvent = (bConfig, bState, executor) =>
    runOnExecute$1((component) => {
      executor(component, bConfig, bState)
    })
  const loadEvent = (bConfig, bState, f) =>
    runOnInit((component, _simulatedEvent) => {
  ...
  const doCreate = (
    ...
    const getConfig = (info) =>
      hasNonNullableKey(info, name) ? info[name]() : Optional.none()
    const wrappedApis = map$1(apis, (apiF, apiName) =>
      wrapApi(name, apiF, apiName)
    )
    const wrappedExtra = map$1(extra, (extraF, extraName) =>
      markAsExtraApi(extraF, extraName)
    )
    const me = {
      ...wrappedExtra,
      ...wrappedApis,
      ...
      config: (spec) => {
        ...
        return {
          ...
          value: {
            ...
            me,
            configAsRaw: cached(() =>
              asRawOrDie$1(name + '-config', configSchema, spec)
            ),
            initialConfig: spec,
            state
          }
      ...
      exhibit: (info, base) => {
        return lift2(
          getConfig(info),
          get$g(active, 'exhibit'),
          (behaviourInfo, exhibitor) => {
  ...
  const exhibit$6 = (base, posConfig) =>
    nu$7({
      classes: [],
  ...
  const preserve$1 = (f, container) => {
    ...
    const refocus = active$1(dos).bind((focused) => {
      ...
      return hasFocus(container)
        ...
        : descendant$1(container, hasFocus)
    })
    const result = f(container)
    refocus.each((oldFocus) => {
      active$1(dos)
        .filter((newFocus) => eq(newFocus, oldFocus))
        .fold(() => {
          focus$3(oldFocus)
        }, noop)
    })
    ...
    origin.fold(
      () => {
        ...
        return NuPositionCss(
          'absolute',
          Optional.some(decisionRect.x),
          Optional.some(decisionRect.y),
          Optional.none(),
          Optional.none()
        )
      },
  ...
  const viewport = (origin, getBounds) =>
    getBounds.fold(
  ...
  const cata$1 = (subject, onNone, onRelative, onFixed) =>
    subject.fold(onNone, onRelative, onFixed)
  adt$9.none
  ...
  const determinePosition = (box, bounds) => {
    ...
    const visibleW = Math.min(
      width,
      x >= boundsX ? boundsRight - x : right - boundsX
    )
    const visibleH = Math.min(
      height,
      y >= boundsY ? boundsBottom - y : bottom - boundsY
    )
    return {
      originInBounds,
      sizeInBounds,
  ...
  const calcReposition = (box, bounds$1) => {
    ...
    const restrictedWidth =
      Math.min(restrictedX + width, boundsRight) - restrictedX
    const restrictedHeight =
      Math.min(restrictedY + height, boundsBottom) - restrictedY
  ...
  const attempt = (candidate, width, height, bounds$1) => {
    ...
    const { originInBounds, sizeInBounds, visibleW, visibleH } =
      determinePosition(box, adjustedBounds)
    ...
    const { maxWidth, maxHeight } = calcMaxSizes(
      candidate.direction,
      fittedBox,
      bounds$1
    )
    ...
    return fits || candidate.alwaysFit
      ? adt$8.fit(reposition)
      : adt$8.nofit(reposition, visibleW, visibleH, isPartlyVisible)
  }
  const attempts = (
    element,
    candidates,
    ...
    const attemptBestFit = (
      ...
      return attemptLayout.fold(
        constant$1(attemptLayout),
        (newReposition, newVisibleW, newVisibleH, newIsVisible) => {
          const improved =
            isVisible === newIsVisible
              ? newVisibleH > visibleH || newVisibleW > visibleW
              : !isVisible && newIsVisible
          return improved
            ? attemptLayout
            : adt$8.nofit(reposition, visibleW, visibleH, isVisible)
        }
      )
    }
    const abc = foldl(
  ...
  const capture = (element, event, handler) =>
    capture$1(element, event, filter, handler)
  const fromRawEvent = fromRawEvent$1

  const properties = ['top', 'bottom', 'right', 'left']
  const timerAttr = 'data-alloy-transition-timer'
  const isTransitioning$1 = (element, transition) =>
    hasAll(element, transition.classes)
  ...
  const setupTransitionListeners = (element, transition) => {
    ...
    const isSourceTransition = (e) => {
      var _a
      const pseudoElement =
        (_a = e.raw.pseudoElement) !== null && _a !== void 0 ? _a : ''
      return (
        eq(e.target, element) &&
        isEmpty(pseudoElement) &&
        contains$2(properties, e.raw.propertyName)
      )
    }
  ...
  const applyTransitionCss = (
    ...
    const shouldTransition = shouldApplyTransitionCss(
      transition,
      decision,
      lastPlacement
    )
    if (shouldTransition || isTransitioning$1(element, transition)) {
      ...
      const intermediateCssOptions = mapToObject(
        properties,
        (prop) => intermediatePosition[prop]
      )
  ...
  const defaultOr = (options, key, dephault) =>
    options[key] === undefined ? dephault : options[key]
  const simple = (
    anchor,
    element,
    bubble,
  ...
  const nu$5 = (xOffset, yOffset, classes, insetModifier = 1) => {
    ...
    return {
      ...
      insetNortheast: () =>
        make(insetXOffset, insetYOffset, ['top', 'alignLeft', 'inset']),
      insetNorthwest: () =>
        make(-insetXOffset, insetYOffset, ['top', 'alignRight', 'inset']),
      insetNorth: () =>
        make(-insetXOffset / 2, insetYOffset, ['top', 'alignCentre', 'inset']),
      insetSoutheast: () =>
        make(insetXOffset, -insetYOffset, ['bottom', 'alignLeft', 'inset']),
      insetSouthwest: () =>
        make(-insetXOffset, -insetYOffset, ['bottom', 'alignRight', 'inset']),
      insetSouth: () =>
        make(-insetXOffset / 2, -insetYOffset, [
          'bottom',
          'alignCentre',
          'inset'
        ]),
      insetEast: () =>
        make(-insetXOffset, -insetYOffset / 2, [
          'valignCentre',
          'right',
          'inset'
        ]),
      insetWest: () =>
        make(insetXOffset, -insetYOffset / 2, ['valignCentre', 'left', 'inset'])
    }
  }
  ...
  const onDirection = (isLtr, isRtl) => (element) =>
    getDirection(element) === 'rtl' ? isRtl : isLtr
  const getDirection = (element) =>
    get$e(element, 'direction') === 'rtl' ? 'rtl' : 'ltr'
  ...
  const isBottomToTopDir = (el) =>
    closest$2(
      el,
      (current) =>
        isElement$1(current) &&
        get$f(current, 'data-alloy-vertical-dir') === AttributeValue.BottomToTop
    )
  ...
  const toFixed = (pos) =>
    pos.fold(identity, (point, scrollLeft, scrollTop) =>
      point.translate(-scrollLeft, -scrollTop)
    )
  const toAbsolute = (pos) => pos.fold(identity, identity)
  const sum = (points) =>
    foldl(points, (b, a) => b.translate(a.left, a.top), SugarPosition(0, 0))
  ...
  const getOffset = (component, origin, anchorInfo) => {
    ...
    return Optional.from(win.frameElement)
  ...
  const doGetExact = (selection) => {
    if (selection.anchorNode === null || selection.focusNode === null) {
      ...
      return after(anchor, selection.anchorOffset, focus, selection.focusOffset)
        ? Optional.some(
            SimRange.create(
              anchor,
  ...
  const descendOnce = (element, offset) =>
    isText(element) ? point(element, offset) : descendOnce$1(element, offset)
  ...
  const placement$1 = (component, anchorInfo, origin) => {
    ...
    const selectionBox = getAnchorSelection(win, anchorInfo).bind((sel) => {
      const optRect = getBounds$1(
        win,
        SimSelection.exactFromRange(sel)
      ).orThunk(() => {
      ...
      return optRect.bind((rawRect) =>
        getBox(rawRect.left, rawRect.top, rawRect.width, rawRect.height)
      )
    })
    const targetElement = getAnchorSelection(win, anchorInfo).bind((sel) =>
      isElement$1(sel.start)
        ? Optional.some(sel.start)
        : parentElement(sel.start)
    )
  ...
  const northY$1 = (anchor, element) =>
    anchor.y - element.height + anchor.height
  const southY$1 = (anchor) => anchor.y
  const southeast$1 = (anchor, element, bubbles) =>
    nu$6(
      eastX(anchor),
      southY$1(anchor),
      bubbles.southeast(),
  ...
  const southwest$1 = (anchor, element, bubbles) =>
    nu$6(
  ...
  const northeast$1 = (anchor, element, bubbles) =>
    nu$6(
      eastX(anchor),
  ...
  const northwest$1 = (anchor, element, bubbles) =>
    nu$6(
      westX(anchor, element),
      northY$1(anchor, element),
      bubbles.northwest(),
  ...
  const positionWithinBounds = (
    ...
    const placeeDetail = asRawOrDie$1(
      'placement.info',
      objOf(PlacementSchema),
      placementSpec
    )
  ...
  const getMode = (component, pConfig, _pState) =>
    pConfig.useFixed() ? 'fixed' : 'absolute'
  const reset$1 = (component, pConfig, posState, placee) => {
    const element = placee.element
    each$1(['position', 'left', 'right', 'top', 'bottom'], (prop) =>
      remove$6(element, prop)
    )
    reset$2(element)
  ...
  const detach = (component) => {
    const parent$1 = parent(component.element).bind((p) =>
      component.getSystem().getByDom(p).toOptional()
    )
    doDetach(component)
    parent$1.each((p) => {
      p.syncComponents()
  ...
  const setContent = (sandbox, sConfig, sState, data) =>
  ...
  const isPartOf = (sandbox, sConfig, sState, queryElem) =>
    isOpen$1(sandbox, sConfig, sState) &&
    sState.get().exists((data) => sConfig.isPartOf(sandbox, data, queryElem))
  ...
  const restore = (sandbox, cssKey, attr) => {
    getOpt(sandbox.element, attr).fold(
  ...
  const hasPosition = (element) =>
    exists(['top', 'left', 'right', 'bottom'], (pos) =>
      getRaw(element, pos).isSome()
    )
  ...
  const events$g = (sandboxConfig, sandboxState) =>
    derive$2([
  ...
  const getValue$3 = (component, repConfig, repState) =>
  ...
  const dataset = () => {
    ...
    const lookup = (itemString) =>
      get$g(dataByValue.get(), itemString).orThunk(() =>
        get$g(dataByText.get(), itemString)
      )
    const update = (items) => {
  ...
  const getValue$1 = (component, repConfig, _repState) =>
  ...
  const field = (name, forbidden) =>
    defaultedObjOf(
      name,
      {},
      map$2(forbidden, (f) =>
        forbid(f.name(), 'Cannot configure ' + f.name() + ' for ' + name)
  ...
  const substitute = (owner, detail, compSpec, placeholders) => {
    ...
    return base.fold(
      (req, valueThunk) => {
        ...
        const substituted = bind$3(childSpecs, (c) =>
  ...
  const substituteAll = (owner, detail, components, placeholders) =>
    bind$3(components, (c) => substitute(owner, detail, c, placeholders))
  ...
  const fPname = field$1(
    'pname',
    'pname',
    defaultedThunk((typeSpec) => '<alloy.' + generate$6(typeSpec.name) + '>'),
    anyValue()
  )
  ...
  const combine = (detail, data, partSpec, partValidated) =>
    deepMerge(
  ...
  const subs = (owner, detail, parts) => {
    ...
    each$1(parts, (part) => {
      part.fold(
        (data) => {
          internals[data.pname] = single$2(
            true,
            (detail, partSpec, partValidated) =>
              data.factory.sketch(
        ...
        (data) => {
          internals[data.pname] = multiple(
            true,
            (detail, _partSpec, _partValidated) => {
  ...
  const schemas = (parts) =>
    bind$3(parts, (part) =>
      part
        ...
        .map((data) =>
          requiredObjOf(data.name, data.schema.concat([snapshot(original())]))
        )
        .toArray()
    )
  const names = (parts) => map$2(parts, name$2)
  const substitutes = (owner, detail, parts) => subs(owner, detail, parts)
  const components$1 = (owner, detail, internals) =>
    substitutePlaces(Optional.some(owner), detail, detail.components, internals)
  const getPart = (component, detail, partKey) => {
    const uid = detail.partUids[partKey]
    return component.getSystem().getByUid(uid).toOptional()
  }
  const getPartOrDie = (component, detail, partKey) =>
    getPart(component, detail, partKey).getOrDie(
  ...
  const getAllParts = (component, detail) => {
    const system = component.getSystem()
    return map$1(detail.partUids, (pUid, _k) =>
      constant$1(system.getByUid(pUid))
    )
  }
  ...
  const defaultUidsSchema = (partTypes) =>
    field$1(
      'partUids',
      'partUids',
      mergeWithThunk((spec) => defaultUids(spec.uid, partTypes)),
      anyValue()
    )
  ...
  const base = (partSchemas, partUidsSchemas) => {
    const ps =
      partSchemas.length > 0 ? [requiredObjOf('parts', partSchemas)] : []
    return ps
      .concat([
        required$1('uid'),
        defaulted('dom', {}),
  ...
  const single = (rawConfig) => {
    const config = asRawOrDie$1(
      'Sketcher for ' + rawConfig.name,
      singleSchema,
      rawConfig
    )
    const sketch = (spec) =>
      single$1(config.name, config.configFields, config.factory, spec)
  ...
  const inside = (target) =>
    (isTag('input')(target) && get$f(target, 'type') !== 'radio') ||
    isTag('textarea')(target)

  const getCurrent = (component, composeConfig, _composeState) =>
    composeConfig.find(component)
  ...
  const hasNative = (component, config) =>
    config.useNative === true &&
    contains$2(nativeDisabled, name$3(component.element))
  ...
  const ariaIsDisabled = (component) =>
    get$f(component.element, 'aria-disabled') === 'true'
  ...
  const isDisabled = (component, disableConfig) =>
  ...
  const exhibit$5 = (base, disableConfig) =>
    nu$7({
      classes: disableConfig.disabled()
        ? disableConfig.disableClass.toArray()
        : []
    })
  const events$e = (disableConfig, disableState) =>
    derive$2([
      abort(execute$5(), (component, _simulatedEvent) =>
        isDisabled(component, disableConfig)
      ),
  ...
  const dehighlightAll = (component, hConfig, hState) =>
  ...
  const isHighlighted = (component, hConfig, hState, queryTarget) =>
    has(queryTarget.element, hConfig.highlightClass)
  const getHighlighted = (component, hConfig, _hState) =>
    descendant(component.element, '.' + hConfig.highlightClass).bind((e) =>
      component.getSystem().getByDom(e).toOptional()
    )
  const getByIndex = (component, hConfig, hState, index) => {
    const items = descendants(component.element, '.' + hConfig.itemClass)
    return Optional.from(items[index]).fold(
      () => Result.error(new Error('No element found with index ' + index)),
      component.getSystem().getByDom
    )
  }
  const getFirst = (component, hConfig, _hState) =>
    descendant(component.element, '.' + hConfig.itemClass).bind((e) =>
      component.getSystem().getByDom(e).toOptional()
    )
  ...
  const getDelta$2 = (component, hConfig, hState, delta) => {
    const items = descendants(component.element, '.' + hConfig.itemClass)
    const current = findIndex$1(items, (item) =>
      has(item, hConfig.highlightClass)
    )
  ...
  const getPrevious = (component, hConfig, hState) =>
    getDelta$2(component, hConfig, hState, -1)
  const getNext = (component, hConfig, hState) =>
    getDelta$2(component, hConfig, hState, +1)
  const getCandidates = (component, hConfig, _hState) => {
    const items = descendants(component.element, '.' + hConfig.itemClass)
    return cat(
      map$2(items, (i) => component.getSystem().getByDom(i).toOptional())
    )
  }
  ...
  const typical = (
    ...
    const schema = () =>
      infoSchema.concat([
        ...
        defaultedOf(
          ...
          'onFocus',
          valueOf((val) =>
            contains$2(['onFocus', 'onEnterOrSpace', 'onApi'], val)
              ? Result.value(val)
    ...
    const processKey = (
      ...
      return choose(rules, simulatedEvent.event).bind((rule) =>
  ...
  const create$1 = (cyclicField) => {
    ...
    const isVisible = (tabbingConfig, element) => {
      const target = tabbingConfig.visibilitySelector
        .bind((sel) => closest$1(element, sel))
        .getOr(element)
    ...
    const findInitial = (component, tabbingConfig) => {
      ...
      const visibles = filter$2(tabstops, (elem) =>
        isVisible(tabbingConfig, elem)
      )
      return Optional.from(visibles[tabbingConfig.firstTabstop])
    }
    const findCurrent = (component, tabbingConfig) =>
      tabbingConfig.focusManager
        .get(component)
        .bind((elem) => closest$1(elem, tabbingConfig.selector))
    ...
    const goFromTabstop = (
      ...
      cycle(tabstops, stopIndex, (elem) => isTabstop(tabbingConfig, elem)).fold(
    ...
    const go = (component, _simulatedEvent, tabbingConfig, cycle) => {
      ...
      return findCurrent(component, tabbingConfig).bind((tabstop) => {
        ...
        return optStopIndex.bind((stopIndex) =>
          goFromTabstop(component, tabstops, stopIndex, tabbingConfig, cycle)
        )
      })
    }
    ...
    const execute = (component, simulatedEvent, tabbingConfig) =>
    ...
    return typical(schema, NoState.init, getKeydownRules, getKeyupRules, () =>
  ...
  const stopEventForFirefox = (_component, _simulatedEvent) =>
    Optional.some(true)
  ...
  const execute$4 = (component, simulatedEvent, executeConfig) =>
    executeConfig.execute(component, simulatedEvent, component.element)
  const getKeydownRules$5 = (
    component,
    _simulatedEvent,
    executeConfig,
    _executeState
  ...
  const useV = (move) => (component, simulatedEvent, config, state) =>
    use(move, component, simulatedEvent, config, state)
  const use = (move, component, simulatedEvent, config, state) => {
    const outcome = config.focusManager
      .get(component)
      .bind((focused) => move(component.element, focused, config, state))
  ...
  const locate = (candidates, predicate) =>
    findIndex$1(candidates, predicate).map((index) => ({
      index,
      candidates
    }))
  ...
  const findIndex = (elements, target) =>
    findIndex$1(elements, (elem) => eq(target, elem))
  ...
  const cycleHorizontal$1 = (values, index, numRows, numCols, delta) =>
    withGrid(values, index, numCols, (oldRow, oldColumn) => {
  ...
  const cycleVertical$1 = (values, index, numRows, numCols, delta) =>
    withGrid(values, index, numCols, (oldRow, oldColumn) => {
  ...
  const cycleRight$1 = (values, index, numRows, numCols) =>
    cycleHorizontal$1(values, index, numRows, numCols, +1)
  const cycleLeft$1 = (values, index, numRows, numCols) =>
    cycleHorizontal$1(values, index, numRows, numCols, -1)
  const cycleUp$1 = (values, index, numRows, numCols) =>
    cycleVertical$1(values, index, numRows, numCols, -1)
  const cycleDown$1 = (values, index, numRows, numCols) =>
    cycleVertical$1(values, index, numRows, numCols, +1)
  ...
  const findCurrent$1 = (component, gridConfig) =>
    gridConfig.focusManager
      .get(component)
      .bind((elem) => closest$1(elem, gridConfig.selector))
  const execute$3 = (component, simulatedEvent, gridConfig, _gridState) =>
    findCurrent$1(component, gridConfig).bind((focused) =>
      gridConfig.execute(component, simulatedEvent, focused)
    )
  const doMove$2 = (cycle) => (element, focused, gridConfig, gridState) =>
    locateVisible(element, focused, gridConfig.selector).bind((identified) =>
      cycle(
  ...
  const handleTab = (_component, _simulatedEvent, gridConfig) =>
    gridConfig.captureTab ? Optional.some(true) : Optional.none()
  const doEscape$1 = (component, simulatedEvent, gridConfig) =>
  ...
  const findCurrent = (component, flowConfig) =>
    flowConfig.focusManager
      .get(component)
      .bind((elem) => closest$1(elem, flowConfig.selector))
  const execute$2 = (component, simulatedEvent, flowConfig) =>
    findCurrent(component, flowConfig).bind((focused) =>
      flowConfig.execute(component, simulatedEvent, focused)
    )
  const focusIn$2 = (component, flowConfig, _state) => {
    flowConfig
      .getInitial(component)
      .orThunk(() => descendant(component.element, flowConfig.selector))
      .each((first) => {
        flowConfig.focusManager.set(component, first)
      })
  }
  const moveLeft$2 = (element, focused, info) =>
    horizontal(element, info.selector, focused, -1)
  const moveRight$2 = (element, focused, info) =>
    horizontal(element, info.selector, focused, +1)
  const doMove$1 =
    (movement) => (component, simulatedEvent, flowConfig, flowState) =>
  ...
  const doEscape = (component, simulatedEvent, flowConfig) =>
  ...
  var FlowType = typical(
    ...
    getKeydownRules$3,
    getKeyupRules$3,
    () => Optional.some(focusIn$2)
  )

  const toCell = (matrix, rowIndex, columnIndex) =>
    Optional.from(matrix[rowIndex]).bind((row) =>
      Optional.from(row[columnIndex]).map((cell) => ({
        rowIndex,
        columnIndex,
        cell
      }))
  ...
  const cycleDown = (matrix, startRow, startCol) =>
    cycleVertical(matrix, startCol, startRow, +1)
  const moveLeft$1 = (matrix, startRow, startCol) =>
    moveHorizontal(matrix, startRow, startCol, -1)
  const moveRight$1 = (matrix, startRow, startCol) =>
    moveHorizontal(matrix, startRow, startCol, +1)
  const moveUp$1 = (matrix, startRow, startCol) =>
    moveVertical(matrix, startCol, startRow, -1)
  const moveDown$1 = (matrix, startRow, startCol) =>
    moveVertical(matrix, startCol, startRow, +1)
  ...
  const execute$1 = (component, simulatedEvent, matrixConfig) =>
    search(component.element).bind((focused) =>
      matrixConfig.execute(component, simulatedEvent, focused)
    )
  const toMatrix = (rows, matrixConfig) =>
    map$2(rows, (row) => descendants(row, matrixConfig.selectors.cell))
  ...
  const moveDown = (element, focused, info) =>
    horizontal(element, info.selector, focused, +1)
  const fireShiftTab = (component, simulatedEvent, menuConfig, menuState) =>
    menuConfig.moveOnTab
      ? move$1(moveUp)(component, simulatedEvent, menuConfig, menuState)
      : Optional.none()
  const fireTab = (component, simulatedEvent, menuConfig, menuState) =>
    menuConfig.moveOnTab
  ...
  var SpecialType = typical(
    ...
    getKeydownRules,
    getKeyupRules,
    (specialInfo) => specialInfo.focusIn
  )
  ...
  const isFlatgridState = (keyState) =>
    hasNonNullableKey(keyState, 'setGridSize')
  ...
  const virtualReplace = (component, replacee, replaceeIndex, childSpec) => {
    ...
    const child = patchSpecChild(
      component.element,
      replaceeIndex,
      childSpec,
      component.getSystem().buildOrPatch
    )
  ...
  const remove = (component, replaceConfig, replaceState, removee) => {
    ...
    const foundChild = find$5(children, (child) =>
      eq(removee.element, child.element)
    )
  ...
  const replaceBy = (
    ...
    return findIndex$1(children, replaceePred).bind((replaceeIndex) =>
      replaceAt(component, replaceConfig, replaceState, replaceeIndex, replacer)
    )
  }
  ...
  const getItemRole = (detail) =>
    detail.toggling
      .map((toggling) =>
        toggling.exclusive ? 'menuitemradio' : 'menuitemcheckbox'
      )
      .getOr('menuitem')
  ...
  const builder$2 = (detail) => ({
    ...
    behaviours: SketchBehaviours.augment(detail.itemBehaviours, [
      detail.toggling.fold(Toggling.revoke, (tConfig) =>
        Toggling.config(getTogglingSpec(tConfig))
      ),
      Focusing.config({
  ...
  const builder = (detail) => {
    ...
    const focusWidget = (component) =>
      getPart(component, detail, 'widget').map((widget) => {
        Keying.focusIn(widget)
        return widget
      })
    const onHorizontalArrow = (component, simulatedEvent) =>
      inside(simulatedEvent.event.target)
        ? Optional.none()
        : (() => {
            if (detail.autofocus) {
  ...
  const deselectOtherRadioItems = (menu, item) => {
    const checkedRadioItems = descendants(
      menu.element,
  ...
  const trace = (items, byItem, byMenu, finish) =>
    get$g(byMenu, finish)
      .bind((triggerItem) =>
        get$g(items, triggerItem).bind((triggerMenu) => {
  ...
  const init$c = () => {
    ...
    const getTriggeringItem = (menuValue) =>
      find$4(expansions.get(), (v, _k) => v === menuValue)
    const getTriggerData = (menuValue, getItemByValue, path) =>
    ...
    const getTriggeringPath = (itemValue, getItemByValue) => {
      const extraPath = filter$2(lookupItem(itemValue).toArray(), (menuValue) =>
        getPreparedMenu(menuValue).isSome()
      )
      return get$g(paths.get(), itemValue).bind((path) => {
        ...
        const triggers = bind$3(revPath, (menuValue, menuIndex) =>
          getTriggerData(
            ...
            () => (is$1(primary.get(), menuValue) ? [] : [Optional.none()]),
    ...
    const expand = (itemValue) =>
      get$g(expansions.get(), itemValue).map((menu) => {
        const current = get$g(paths.get(), itemValue).getOr([])
        return [menu].concat(current)
      })
    const collapse = (itemValue) =>
      get$g(paths.get(), itemValue).bind((path) =>
        path.length > 1 ? Optional.some(path.slice(1)) : Optional.none()
      )
    const refresh = (itemValue) => get$g(paths.get(), itemValue)
    const getPreparedMenu = (menuValue) =>
  ...
  const extractPreparedMenu = (prep) =>
    prep.type === 'prepared' ? Optional.some(prep.menu) : Optional.none()
  const LayeredState = {
    init: init$c,
    extractPreparedMenu
  }
  ...
  const onMenuItemDehighlightedEvent = generate$6(
    'tiered-menu-item-dehighlight'
  )

  var HighlightOnOpen
  ;(function (HighlightOnOpen) {
    HighlightOnOpen[(HighlightOnOpen['HighlightMenuAndItem'] = 0)] =
      'HighlightMenuAndItem'
    HighlightOnOpen[(HighlightOnOpen['HighlightJustMenu'] = 1)] =
      'HighlightJustMenu'
  ...
  const make$6 = (detail, _rawUiSpec) => {
    ...
    const buildMenus = (container, primaryName, menus) =>
      map$1(menus, (spec, name) => {
        const makeSketch = () =>
          Menu.sketch({
            ...spec,
            value: name,
    ...
    const toDirectory = (_container) =>
      map$1(detail.data.menus, (data, _menuName) =>
        bind$3(data.items, (item) =>
          item.type === 'separator' ? [] : [item.data.value]
        )
      )
    ...
    const setActiveMenuAndItem = (container, menu) => {
      ...
      Highlighting.getHighlighted(menu)
        .orThunk(() => Highlighting.getFirst(menu))
        .each((item) => {
          if (detail.fakeFocus) {
            Highlighting.highlight(menu, item)
          } else {
    ...
    const getMenus = (state, menuValues) =>
      cat(
        map$2(menuValues, (mv) =>
          state
            .lookupMenu(mv)
    ...
    const getSubmenuParents = (container) =>
      submenuParentItems.get().getOrThunk(() => {
        const r = {}
        const items = descendants(container.element, `.${detail.markers.item}`)
        const parentItems = filter$2(
          items,
          (i) => get$f(i, 'aria-haspopup') === 'true'
        )
    ...
    const updateMenuPath = (container, state, path) =>
      Optional.from(path[0]).bind((latestMenuName) =>
        state.lookupMenu(latestMenuName).bind((menuPrep) => {
    ...
    const expandRight = (
      ...
      if (item.hasConfigured(Disabling) && Disabling.isDisabled(item)) {
        ...
        return layeredState.expand(value).bind((path) => {
          ...
          return Optional.from(path[0]).bind((menuName) =>
            layeredState.lookupMenu(menuName).bind((activeMenuPrep) => {
    ...
    const keyOnItem = (f) => (container, simulatedEvent) => {
      return closest$1(
        ...
        container
          ...
          .getByDom(target)
          .toOptional()
          .bind((item) => f(container, item).map(always))
      )
    }
    const events = derive$2([
      run$1(focus(), (tmenu, simulatedEvent) => {
        ...
        layeredState.lookupItem(getItemValue(item)).each(() => {
          ...
          layeredState
            .refresh(value)
            .each((path) => closeOthers(tmenu, layeredState, path))
        })
      }),
      runOnExecute$1((component, simulatedEvent) => {
      ...
      runOnAttached((container, _simulatedEvent) => {
        setup(container).each((primary) => {
          ...
          if (detail.highlightOnOpen === HighlightOnOpen.HighlightMenuAndItem) {
            ...
            detail.highlightOnOpen === HighlightOnOpen.HighlightJustMenu
          ) {
    ...
    const getActiveItem = (container) =>
      Highlighting.getHighlighted(container).bind(Highlighting.getHighlighted)
    ...
    const extractMenuFromContainer = (container) =>
      Optional.from(container.components()[0]).filter(
        (comp) => get$f(comp.element, 'role') === 'menu'
      )
    const repositionMenus = (container) => {
      const maybeActivePrimary = layeredState.getPrimary().bind((primary) =>
        getActiveItem(container)
          .bind((currentItem) => {
            const itemValue = getItemValue(currentItem)
            const allMenus = values(layeredState.getMenus())
            const preparedMenus = cat(
              map$2(allMenus, LayeredState.extractPreparedMenu)
            )
            return layeredState.getTriggeringPath(itemValue, (v) =>
              getItemByValue(container, preparedMenus, v)
            )
          })
          .map((triggeringPath) => ({
            primary,
  ...
  const makeMenu = (
    ...
    const getSubmenuLayouts = (triggeringPaths) =>
      isFirstTierSubmenu(triggeringPaths) ? layouts : {}
  ...
  const factory$m = (detail, spec) => {
    ...
    const showWithin = (sandbox, thing, placementSpec, boxElement) => {
      showWithinBounds(sandbox, thing, placementSpec, () =>
        boxElement.map((elem) => box$1(elem))
      )
    }
    const showWithinBounds = (sandbox, thing, placementSpec, getBounds) => {
      const sink = detail.lazySink(sandbox).getOrDie()
      Sandboxing.openWhileCloaked(sandbox, thing, () =>
        Positioning.positionWithinBounds(
          sink,
          sandbox,
          placementSpec,
      ...
      behaviours: augment(detail.inlineBehaviours, [
        Sandboxing.config({
          isPartOf: (sandbox, data, queryElem) => {
            return (
              isPartOf$1(data, queryElem) || isPartOfRelated(sandbox, queryElem)
            )
          },
          ...
          channels: {
            ...receivingChannel$1({
              ...
              ...detail.fireDismissalEventInstead
                .map((fe) => ({ fireEventInstead: { event: fe.event } }))
                .getOr({})
            }),
            ...receivingChannel({
  ...
  const factory$l = (detail) => {
    ...
    const lookupAttr = (attr) =>
      get$g(detail.dom, 'attributes').bind((attrs) => get$g(attrs, attr))
  ...
  const record = (spec) => {
    const uid =
      isSketchSpec(spec) && hasNonNullableKey(spec, 'uid')
        ? spec.uid
        : generate$5('memento')
    const get = (anyInSystem) =>
      anyInSystem.getSystem().getByUid(uid).getOrDie()
    const getOpt = (anyInSystem) =>
      anyInSystem.getSystem().getByUid(uid).toOptional()
    const asSpec = () => ({
      ...spec,
      uid
    })
  ...
  const defaultIcon = (icons) => () =>
    get$g(icons, defaultIconName).getOr('!not found!')
  ...
  const getOr = (name, iconProvider, fallbackIcon) => {
    const icons = iconProvider()
    return lookupIcon(name, icons)
      .or(fallbackIcon)
      .getOrThunk(defaultIcon(icons))
  }
  const needsRtlTransform = (iconName) =>
    global$8.isRtl() ? has$2(rtlTransform, iconName) : false
  const addFocusableBehaviour = () =>
    config('add-focusable', [
      runOnAttached((comp) => {
        child(comp.element, 'svg').each((svg) =>
          set$9(svg, 'focusable', 'false')
        )
      })
    ])
  const renderIcon$2 = (spec, iconName, icons, fallbackIcon) => {
    ...
    const iconHtml = get$g(icons, getIconName(iconName, icons))
      .or(fallbackIcon)
      .getOrThunk(defaultIcon(icons))
    return {
      dom: {
  ...
  ) => renderIcon$2(spec, iconName, iconProvider(), fallbackIcon)
  const renderFirst = (iconNames, spec, iconProvider) => {
    const icons = iconProvider()
    const iconName = find$5(iconNames, (name) =>
      has$2(icons, getIconName(name, icons))
    )
    return renderIcon$2(
      spec,
      iconName.getOr(defaultIconName),
      icons,
      Optional.none()
    )
  }
  ...
  const factory$k = (detail) => {
    ...
    const iconChoices = flatten([
      ...
      detail.level
        .bind((level) => Optional.from(notificationIconMap[level]))
        .toArray()
    ])
    const memButton = record(
      Button.sketch({
        dom: {
          tag: 'button',
          classes: [
    ...
    const notificationIconSpec = renderFirst(
      iconChoices,
      {
        tag: 'div',
  ...
  var NotificationManagerImpl = (editor, extras, uiMothership) => {
    ...
    const open = (settings, closeCallback) => {
      ...
      const notificationWrapper = build$1(
        InlineView.sketch({
          dom: {
            tag: 'div',
  ...
  const register$e = (editor) => {
    ...
    const stringOrNumberProcessor = (value) =>
  ...
  const getFilePickerValidatorHandler = option$2(
  ...
  const getLineHeightFormats = (editor) =>
  ...
  const isToolbarEnabled = (editor) => {
    ...
    return (
      ...
      (isToolbarObjectArray || isToolbarString || toolbar === true)
    )
  }
  const getMultipleToolbarsOption = (editor) => {
    const toolbars = range$2(9, (num) =>
      editor.options.get('toolbar' + (num + 1))
    )
    const toolbarArray = filter$2(toolbars, isString)
    return someIf(toolbarArray.length > 0, toolbarArray)
  }
  const isMultipleToolbars = (editor) =>
    getMultipleToolbarsOption(editor).fold(() => {
      const toolbar = getToolbar(editor)
      return isArrayOf(toolbar, isString) && toolbar.length > 0
    }, always)
  const isToolbarLocationBottom = (editor) =>
    getToolbarLocation(editor) === ToolbarLocation$1.bottom
  const fixedContainerTarget = (editor) => {
    ...
    const selector =
      (_a = fixedContainerSelector(editor)) !== null && _a !== void 0 ? _a : ''
  ...
  const useFixedContainer = (editor) =>
  ...
  const getUiContainer = (editor) => {
    ...
    return fixedContainer.getOrThunk(() =>
      getContentContainer(
        getRootNode(SugarElement.fromDom(editor.getElement()))
      )
    )
  }
  const isDistractionFree = (editor) =>
    editor.inline &&
  ...
  const setup$e = (api, editor) => {
    ...
    editor.on('NodeChange', (e) => {
      if (
        ...
        detect(SugarElement.fromDom(e.element)).isNone()
      ) {
        api.cancelIfNecessary()
      }
    })
  ...
  const containerValignMiddleClass =
    'tox-collection__item-container--valign-middle'
  const containerValignBottomClass =
    'tox-collection__item-container--valign-bottom'
  const classForPreset = (presets) =>
    get$g(presetClasses, presets).getOr(navClass)
  ...
  const redirectMenuItemInteractionEvent = generate$6(
    'redirect-menu-item-interaction'
  )

  const menuSearcherClass = 'tox-menu__searcher'
  const findWithinSandbox = (sandboxComp) => {
    return descendant(sandboxComp.element, `.${menuSearcherClass}`).bind(
  ...
  const setActiveDescendant = (inputComp, active) => {
    getOpt(active.element, 'id').each((id) =>
      set$9(inputComp.element, 'aria-activedescendant', id)
    )
  }
    ...
    return {
      ...
      components: [
        Input.sketch({
          ...
          inputAttributes: {
            ...spec.placeholder
              .map((placeholder) => ({ placeholder: spec.i18n(placeholder) }))
              .getOr({}),
            type: 'search',
  ...
  const forCollectionWithSearchResults = (
    columns,
    initItems,
    _hasIcons = true
  ) => {
  ...
  const createHorizontalPartialMenuWithAlloyItems = (
    value,
    _hasIcons,
    items,
    _columns,
  ...
  const createPartialMenuWithAlloyItems = (
    value,
    hasIcons,
    items,
    columns,
    ...
    if (menuLayout.menuType === 'color') {
      ...
      menuLayout.menuType === 'normal' ||
      menuLayout.menuType === 'searchable'
    ) {
  ...
  const generatedName = (namePrefix) =>
    field$1(
      'name',
      'name',
      defaultedThunk(() => generate$6(`${namePrefix}-name`)),
      string
    )
  const generatedValue = (valuePrefix) =>
    field$1(
      'value',
      'value',
      defaultedThunk(() => generate$6(`${valuePrefix}-value`)),
      anyValue()
    )

  const separatorMenuItemSchema = objOf([type, optionalText])
  const createSeparatorMenuItem = (spec) =>
    asRaw('separatormenuitem', separatorMenuItemSchema, spec)
  ...
  const createSeparatorItem = (spec) =>
    asRaw('Autocompleter.Separator', separatorMenuItemSchema, spec)
  const createAutocompleterItem = (spec) =>
    asRaw('Autocompleter.Item', autocompleterItemSchema, spec)
  ...
  const toolbarButtonSchema = objOf(
    [type, onAction].concat(baseToolbarButtonFields)
  )
  const createToolbarButton = (spec) =>
    asRaw('toolbarbutton', toolbarButtonSchema, spec)

  const baseToolbarToggleButtonFields = [active].concat(baseToolbarButtonFields)
  const toggleButtonSchema = objOf(
    baseToolbarToggleButtonFields.concat([type, onAction])
  )
  const createToggleButton = (spec) =>
    asRaw('ToggleButton', toggleButtonSchema, spec)
  ...
  const createContextForm = (spec) =>
    asRaw('ContextForm', contextFormSchema, spec)

  const contextToolbarSchema = objOf(
    [defaultedType('contexttoolbar'), requiredString('items')].concat(
      contextBarFields
    )
  )
  const createContextToolbar = (spec) =>
    asRaw('ContextToolbar', contextToolbarSchema, spec)
  ...
  const createCardMenuItem = (spec) =>
    asRaw('cardmenuitem', cardMenuItemSchema, spec)

  const choiceMenuItemSchema = objOf(
    [type, active, optionalIcon].concat(commonMenuItemFields)
  )
  const createChoiceMenuItem = (spec) =>
    asRaw('choicemenuitem', choiceMenuItemSchema, spec)
  ...
  const createFancyMenuItem = (spec) =>
    asRaw('fancymenuitem', fancyMenuItemSchema, spec)

  const menuItemSchema = objOf(
    [type, onSetup, defaultedOnAction, optionalIcon].concat(
  ...
  const nestedMenuItemSchema = objOf(
    [type, getSubmenuItems, onSetup, optionalIcon].concat(commonMenuItemFields)
  )
  const createNestedMenuItem = (spec) =>
    asRaw('nestedmenuitem', nestedMenuItemSchema, spec)

  const toggleMenuItemSchema = objOf(
    [type, optionalIcon, active, onSetup, onAction].concat(commonMenuItemFields)
  )
  const createToggleMenuItem = (spec) =>
    asRaw('togglemenuitem', toggleMenuItemSchema, spec)
  ...
  const unnamedEvents = (handlers) =>
    namedEvents(generate$6('unnamed-events'), handlers)
  ...
  const splitButton = (disabled) =>
    Disabling.config({
      disabled,
      disableClass: 'tox-tbtn--disabled'
    })
  const toolbarButton = (disabled) =>
    Disabling.config({
      disabled,
      disableClass: 'tox-tbtn--disabled',
      useNative: false
  ...
  const onControlAttached = (info, editorOffCell) =>
    runOnAttached((comp) => {
  ...
  const onControlDetached = (getApi, editorOffCell) =>
    runOnDetached((comp) => runWithApi(getApi, comp)(editorOffCell.get()))

  const onMenuItemExecute = (info, itemResponse) =>
    runOnExecute$1((comp, simulatedEvent) => {
      ...
      itemBehaviours: derive$1(
        [
          ...
          DisablingConfigs.item(
            () => !spec.enabled || providersBackstage.isDisabled()
          ),
          receivingConfig(),
          Replacing.config({})
  ...
  const renderIcon$1 = (name, icons, classes = [iconClass]) =>
    render$3(
      name,
      {
        tag: 'div',
  ...
  const renderCheckmark = (icons) =>
    renderIcon$1('checkmark', icons, [checkmarkClass])
  const renderSubmenuCaret = (icons) =>
    renderIcon$1('chevron-right', icons, [caretClass])
  const renderDownwardsCaret = (icons) =>
    renderIcon$1('chevron-down', icons, [caretClass])
  ...
  const renderColorStructure = (item, providerBackstage, fallbackIcon) => {
    ...
    const iconSvg = item.iconContent.map((name) =>
      getOr(name, providerBackstage.icons, fallbackIcon)
    )
    const getDom = () => {
      const common = colorClass
      const icon = iconSvg.getOr('')
      const attributes = itemText
        .map((text) => ({ title: providerBackstage.translate(text) }))
        .getOr({})
      const baseDom = {
        tag: 'div',
        attributes,
  ...
  const renderItemDomStructure = (ariaLabel) => {
    const domTitle = ariaLabel
      .map((label) => ({ attributes: { title: global$8.translate(label) } }))
      .getOr({})
    return {
      tag: 'div',
  ...
  const renderNormalItemStructure = (
    ...
    const renderIcon = (iconName) =>
      render$3(iconName, iconSpec, providersBackstage.icons, fallbackIcon)
    const renderEmptyIcon = () => Optional.some({ dom: iconSpec })
    const leftIcon = renderIcons
      ? info.iconContent.map(renderIcon).orThunk(renderEmptyIcon)
      : Optional.none()
    const checkmark = info.checkMark
    const textRender = Optional.from(info.meta).fold(
      () => renderText,
      (meta) =>
        has$2(meta, 'style') ? curry(renderStyledText, meta.style) : renderText
    )
    const content = info.htmlContent.fold(
      () => info.textContent.map(textRender),
      (html) => Optional.some(renderHtml(html, [textClass]))
    )
    const menuItem = {
  ...
  const tooltipBehaviour = (meta, sharedBackstage) =>
    get$g(meta, 'tooltipWorker')
      .map((tooltipWorker) => [
        Tooltipping.config({
          lazySink: sharedBackstage.getSink,
          tooltipDom: {
          ...
          onShow: (component, _tooltip) => {
            tooltipWorker((elm) => {
              Tooltipping.setComponents(component, [
                external$1({ element: SugarElement.fromDom(elm) })
              ])
            })
          }
        })
  ...
  const replaceText = (text, matchText) => {
    ...
    if (matchText.length > 0) {
      ...
      return encoded.replace(
        escapedMatchRegex,
        (match) => `<span class="tox-autocompleter-highlight">${match}</span>`
      )
    } else {
      return encoded
    }
    ...
    const structure = renderItemStructure(
      {
        ...
        htmlContent: useText
          ? spec.text.map((text) => replaceText(text, matchText))
          : Optional.none(),
        ariaLabel: spec.text,
  ...
  const render$2 = (items, extras) =>
    map$2(items, (item) => {
      switch (item.type) {
        ...
        case 'cardtext':
          const shouldHighlight = item.name.exists((name) =>
            contains$2(extras.cardText.highlightOn, name)
          )
          const matchText = shouldHighlight
            ? Optional.from(extras.cardText.matchText).getOr('')
            : ''
    ...
    const structure = renderItemStructure(
      {
        ...
        checkMark: useText
          ? Optional.some(renderCheckmark(providersBackstage.icons))
          : Optional.none(),
        caret: Optional.none(),
        value: spec.value
      },
  ...
  const fromString$1 = (hex) =>
    isHexString(hex)
      ? Optional.some({ value: normalizeHex(hex) })
      : Optional.none()
  ...
  const fireResolveName = (editor, node) =>
  ...
  const getCurrentColor = (editor, format) => {
    ...
    editor.dom.getParents(editor.selection.getStart(), (elm) => {
      const value = hasStyleApi(elm)
        ? elm.style[format === 'forecolor' ? 'color' : 'backgroundColor']
        : null
      if (value) {
  ...
  const getColors$1 = (colors, hasCustom) =>
    colors.concat(getCurrentColors().concat(getAdditionalColors(hasCustom)))
    ...
    editor.ui.registry.addSplitButton(name, {
      ...
      select: (value) => {
        ...
        return optCurrentRgb
          .bind((currentRgb) =>
            fromString(currentRgb).map((rgba) => {
    ...
    const onSubmit = (api) => {
      ...
      if (isValid) {
        ...
        editor.windowManager.alert(
          editor.translate(['Invalid hex color code: {0}', hex])
        )
      }
    }
    ...
    cat(
      map$2(items, (item) => {
        if (item.type === 'choiceitem') {
          return createChoiceMenuItem(item).fold(handleError, (d) =>
            Optional.some(
              renderChoiceItem(
                d,
                columns === 1,
  ...
  const getColorItems = (spec, backstage) => {
    const useCustomColors =
      spec.initData.allowCustomColors && backstage.colorinput.hasCustomColors()
    return spec.initData.colors.fold(
      () => getColors$1(backstage.colorinput.getColors(), useCustomColors),
  ...
  const makeCell = (row, col, labelId) => {
    const emitCellOver = (c) =>
      emitWith(c, cellOverEvent, {
        row,
        col
      })
    const emitExecute = (c) =>
      emitWith(c, cellExecuteEvent, {
        row,
        col
      })
    const onClick = (c, se) => {
  ...
  const makeComponents = (cells) =>
    bind$3(cells, (cellRow) => map$2(cellRow, premade))
  ...
  const renderFancyMenuItem = (spec, backstage) =>
    get$g(fancyMenuItems, spec.fancytype).map((render) =>
  ...
  const getCoupled = (component, coupleConfig, coupleState, name) =>
    coupleState.getOrCreate(component, coupleConfig, name)
  const getExistingCoupled = (component, coupleConfig, coupleState, name) =>
  ...
  const init$a = () => {
    ...
    const getOrCreate = (component, coupleConfig, name) => {
      return lookupCoupled(coupleConfig, name).getOrThunk(() => {
        const builder = get$g(coupleConfig.others, name).getOrDie(
    ...
    const getExisting = (component, coupleConfig, name) => {
      return lookupCoupled(coupleConfig, name).orThunk(() => {
        get$g(coupleConfig.others, name).getOrDie(
          'No information found for coupled component: ' + name
        )
        return Optional.none()
      })
  ...
  const getSink = (anyInSystem, sinkDetail) =>
    anyInSystem
      .getSystem()
  ...
  const factory$i = (detail, components, _spec, externals) => {
    ...
    const apis = {
      ...
      open: (comp) => {
        if (!Toggling.isOn(comp)) {
          togglePopup$1(comp, noop, HighlightOnOpen.HighlightMenuAndItem).get(
            noop
          )
        }
      },
  ...
  const handleRefetchTrigger = (originalSandboxComp) => {
    ...
    const optSearcherState =
      findWithinSandbox(originalSandboxComp).map(saveState)
    Dropdown.refetch(dropdown).get(() => {
      const newSandboxComp = Coupling.getCoupled(dropdown, 'sandbox')
      optSearcherState.each((searcherState) =>
        findWithinSandbox(newSandboxComp).each((inputComp) =>
          restoreState(inputComp, searcherState)
        )
      )
    })
  }
  ...
  const updateAriaOnHighlight = (tmenuComp, menuComp, itemComp) => {
    findWithinMenu(tmenuComp).each((inputComp) => {
      ...
      optActiveResults.each((resultsElem) => {
        getOpt(resultsElem, 'id').each((controlledId) =>
          set$9(inputComp.element, 'aria-controls', controlledId)
        )
      })
    })
  ...
  const focusSearchField = (tmenuComp) => {
    findWithinMenu(tmenuComp).each((searcherComp) =>
      Focusing.focus(searcherComp)
    )
  }
  ...
  const createMenuItemFromBridge = (
    ...
    const parseForHorizontalMenu = (menuitem) =>
      !isHorizontalMenu
        ? menuitem
        : {
            ...menuitem,
    ...
    switch (item.type) {
      case 'menuitem':
        return createMenuItem(item).fold(handleError, (d) =>
          Optional.some(
            normal(
      ...
      case 'nestedmenuitem':
        return createNestedMenuItem(item).fold(handleError, (d) =>
          Optional.some(
            nested(
              parseForHorizontalMenu(d),
              itemResponse,
      ...
      case 'togglemenuitem':
        return createToggleMenuItem(item).fold(handleError, (d) =>
          Optional.some(
            toggle$1(
              parseForHorizontalMenu(d),
              itemResponse,
      ...
      case 'separator':
        return createSeparatorMenuItem(item).fold(handleError, (d) =>
          Optional.some(separator$3(d))
        )
      case 'fancymenuitem':
        return createFancyMenuItem(item).fold(handleError, (d) =>
          fancy(d, backstage)
        )
      default: {
  ...
  const createAutocompleteItems = (
    ...
    return cat(
      map$2(items, (item) => {
        switch (item.type) {
          case 'separator':
            return createSeparatorItem(item).fold(handleError, (d) =>
              Optional.some(separator$3(d))
            )
          case 'cardmenuitem':
            return createCardMenuItem(item).fold(handleError, (d) =>
              Optional.some(
                card(
                  {
                    ...d,
          ...
          default:
            return createAutocompleterItem(item).fold(handleError, (d) =>
              Optional.some(
                autocomplete(
                  d,
                  matchText,
  ...
  const createPartialMenu = (
    ...
    const alloyItems = cat(
      map$2(items, (item) => {
        const itemHasIcon = (i) =>
          isHorizontalMenu ? !has$2(i, 'text') : hasIcons
        const createItem = (i) =>
          createMenuItemFromBridge(
            i,
            itemResponse,
            backstage,
            itemHasIcon(i),
  ...
  const createTieredDataFrom = (partialMenu) =>
  ...
  const register$b = (editor, sharedBackstage) => {
    ...
    const getMenu = () =>
      InlineView.getContent(autocompleter).bind((tmenu) => {
    ...
    const display = (lookupData, items) => {
      findIn(SugarElement.fromDom(editor.getBody())).each((element) => {
        const columns = findMap(lookupData, (ld) =>
          Optional.from(ld.columns)
        ).getOr(1)
  ...
  const closest = (scope, selector, isRoot) =>
    closest$1(scope, selector, isRoot).isSome()
  ...
  const monitor = (settings) => {
    ...
    const fireIfReady = (event, type) =>
      get$g(handlers, type).bind((handler) => handler(event))
    return { fireIfReady }
  }
  ...
  const isDangerous = (event) => {
    ...
    return (
      ...
      !contains$2(['input', 'textarea'], name$3(event.target)) &&
  ...
  const EventRegistry = () => {
    ...
    const findHandler = (handlers, elem) =>
      read$1(elem)
        .bind((id) => get$g(handlers, id))
        .map((descHandler) => eventHandler(elem, descHandler))
    const filterByType = (type) =>
      get$g(registry, type)
        .map((handlers) =>
          mapToArray(handlers, (f, id) => broadcastHandler(id, f))
        )
        .getOr([])
    const find = (isAboveRoot, type, target) =>
      get$g(registry, type).bind((handlers) =>
        closest$4(target, (elem) => findHandler(handlers, elem), isAboveRoot)
      )
  ...
  const takeover = (root) => {
    const isAboveRoot = (el) =>
      parent(root.element).fold(always, (parent) => eq(el, parent))
    const registry = Registry()
    const lookup = (eventName, target) =>
      registry.find(isAboveRoot, eventName, target)
    const domEvents = setup$d(root.element, {
      triggerEvent: (eventName, event) => {
        return monitorEvent(eventName, event.target, (logger) =>
          triggerUntilStopped(lookup, eventName, event, logger)
        )
      }
    })
    const systemApi = {
  ...
  const exhibit$2 = (base, tabConfig) =>
    nu$7({
      attributes: wrapAll([
        {
  ...
  const renderFormField = (pLabel, pField) =>
  ...
  const renderLabel$2 = (label, providersBackstage) =>
  ...
  const renderCollection = (spec, providersBackstage, initialData) => {
    const pLabel = spec.label.map((label) =>
      renderLabel$2(label, providersBackstage)
    )
    const runOnItem = (f) => (comp, se) => {
      closest$1(se.event.target, '[data-collection-item-value]').each(
        (target) => {
    ...
    const setContents = (comp, items) => {
      const htmlLines = map$2(items, (item) => {
        ...
        const disabledClass = providersBackstage.isDisabled()
          ? ' tox-collection__item--state-disabled'
          : ''
        return `<div class="tox-collection__item${disabledClass}" tabindex="-1" data-collection-item-value="${global$3.encodeAllRaw(
          item.value
        )}" title="${ariaLabel}" aria-label="${ariaLabel}">${iconContent}${textContent}</div>`
      })
      const chunks =
        spec.columns !== 'auto' && spec.columns > 1
          ? chunk$1(htmlLines, spec.columns)
          : [htmlLines]
      const html = map$2(
        chunks,
    ...
    const collectionEvents = [
      ...
      runOnExecute$1(
        runOnItem((comp, se, tgt, itemValue) => {
          emitWith(comp, formActionEvent, {
            name: spec.name,
            value: itemValue
          })
    ...
    const iterCollectionItems = (comp, applyAttributes) =>
      map$2(descendants(comp.element, '.tox-collection__item'), applyAttributes)
  ...
  const query = (component, invalidConfig, _invalidState) =>
  ...
  const events$8 = (invalidConfig, invalidState) =>
  ...
  const renderPanelButton = (spec, sharedBackstage) =>
    Dropdown.sketch({
      dom: spec.dom,
      ...
      lazySink: sharedBackstage.getSink,
      fetch: (comp) =>
        Future.nu((callback) => spec.fetch(callback)).map((items) =>
          Optional.from(
  ...
  const renderColorInput = (
    ...
    const pLabel = spec.label.map((label) =>
      renderLabel$2(label, sharedBackstage.providers)
    )
      ...
      memColorButton.getOpt(comp).each((colorBit) => {
        if (value === 'custom') {
          colorInputBackstage.colorPicker((valueOpt) => {
            valueOpt.fold(
              () => emit(colorBit, colorPickerCancelEvent),
              (value) => {
  ...
  const spectrumPart = required({
    ...
    overrides: (detail) => {
      ...
      const setValueFrom = (component, simulatedEvent) =>
        model
          .getValueFromEvent(simulatedEvent)
          .map((value) => model.setValueFrom(component, detail, value))
      return {
        behaviours: derive$1([
          Keying.config({
            mode: 'special',
  ...
  const getEventSource = (simulatedEvent) => {
    ...
    if (isTouchEvent$2(evt)) {
      ...
      return touchEvent.touches !== undefined && touchEvent.touches.length === 1
        ? Optional.some(touchEvent.touches[0]).map((t) =>
            SugarPosition(t.clientX, t.clientY)
          )
        : Optional.none()
    } else {
      const mouseEvent = evt
      return mouseEvent.clientX !== undefined
        ? Optional.some(mouseEvent).map((me) =>
            SugarPosition(me.clientX, me.clientY)
          )
        : Optional.none()
    }
  }
  ...
  const findOffsetOf = (value, min, max) =>
  ...
  const handleMovement$2 = (direction) => (spectrum, detail) =>
    moveBy$2(direction, spectrum, detail).map(always)
  ...
  const findOffsetOfValue$1 = (spectrum, detail, value, minEdge, maxEdge) => {
    ...
    const centerMinEdge = minEdge
      .bind((edge) => Optional.some(getXCenterOffSetOf(edge, spectrum)))
      .getOr(minOffset)
  ...
  const handleMovement$1 = (direction) => (spectrum, detail) =>
    moveBy$1(direction, spectrum, detail).map(always)
  ...
  const findOffsetOfValue = (spectrum, detail, value, minEdge, maxEdge) => {
    ...
    const centerMinEdge = minEdge
      .bind((edge) => Optional.some(getYCenterOffSetOf(edge, spectrum)))
      .getOr(minOffset)
    const centerMaxEdge = maxEdge
      .bind((edge) => Optional.some(getYCenterOffSetOf(edge, spectrum)))
      .getOr(maxOffset)
  ...
  const handleMovement =
    (direction, isVerticalMovement) => (spectrum, detail) =>
      moveBy(direction, isVerticalMovement, spectrum, detail).map(always)
  ...
  const setPositionFromValue = (slider, thumb, detail, edges) => {
    const value = currentValue(detail)
    const xPos = findPositionOfValue$1(
      slider,
      edges.getSpectrum(slider),
      value.x,
      edges.getLeftEdge(slider),
    ...
    const yPos = findPositionOfValue(
      slider,
      edges.getSpectrum(slider),
      value.y,
      edges.getTopEdge(slider),
  ...
  const sketch$2 = (detail, components, _spec, _externals) => {
    ...
    const getSpectrum = (component) =>
      getPartOrDie(component, detail, 'spectrum')
    ...
    const getBottomEdge = (component) =>
      getPart(component, detail, 'bottom-edge')
    const modelDetail = detail.model
    const model = modelDetail.manager
    ...
    return {
      ...
      behaviours: augment(detail.sliderBehaviours, [
        ...
        Receiving.config({
          channels: { [mouseReleased()]: { onReceive: choose } }
        })
      ]),
      events: derive$2([
  ...
  const sketch$1 = (fSpec) => {
    ...
    const fieldParts = map$2(partNames, (n) =>
      required({
        name: n,
      ...
      Representing.config({
        store: {
          ...
          getValue: (form) => {
            ...
            return map$1(resPs, (resPThunk, pName) =>
              resPThunk()
                .bind((v) => {
                  ...
                  return toResult(
                    opt,
                    new Error(
                      `Cannot find a current component to extract the value from for form part '${pName}': ` +
                        element(v.element)
                    )
                  )
                })
  ...
  const rgbFormFactory = (translate, getClass, onValidHexx, onInvalidHexx) => {
    const invalidation = (label, isValid) =>
      Invalidating.config({
      ...
      const getValueRgb = () =>
        get('red').bind((red) =>
          get('green').bind((green) =>
            get('blue').map((blue) => rgbaColour(red, green, blue, 1))
          )
        )
  ...
  const childAt = (index) =>
    Composing.config({
      find: (comp) =>
        child$2(comp.element, index).bind((element) =>
          comp.getSystem().getByDom(element).toOptional()
        )
    })
  ...
  const withComp = (optInitialValue, getter, setter) =>
    Representing.config({
      store: {
        mode: 'manual',
  ...
  const withElement = (initialValue, getter, setter) =>
    withComp(
      initialValue,
      (c) => getter(c.element),
      (c, v) => setter(c.element, v)
    )
  const domValue = (optInitialValue) =>
    withElement(optInitialValue, get$6, set$5)
  const domHtml = (optInitialValue) =>
    withElement(optInitialValue, get$9, set$6)
  const memory = (initialValue) =>
    Representing.config({
      store: {
        mode: 'memory',
        initialValue
  ...
  const renderColorPicker = (_spec, providerBackstage, initialData) => {
    ...
    const colourPickerFactory = makeFactory(
      translate$1(providerBackstage),
      getClass
    )
      ...
      behaviours: derive$1([
        RepresentingConfigs.withComp(
          ...
          (comp, newValue) => {
            ...
            const valOpt = Optional.from(pattern.exec(newValue)).bind(
              (matches) => get$h(matches, 1)
            )
            const picker = memPicker.get(comp)
            const optRgbForm = Composing.getCurrent(picker)
            optRgbForm.fold(
            ...
            memReplaced.getOpt(component).each((ta) => {
              ;(isOldCustomEditor(spec)
                ...
                : global$2
                    ...
                    .then((init) => init(ta.element.dom, spec.settings))
              ).then((ea) => {
  ...
  const filterByExtension = (files, providersBackstage) => {
    const allowedImageFileTypes = global$1.explode(
      providersBackstage.getOption('images_file_types')
    )
    const isFileInAllowedTypes = (file) =>
      exists(allowedImageFileTypes, (type) =>
        endsWith(file.name.toLowerCase(), `.${type.toLowerCase()}`)
      )
  ...
  const renderDropZone = (spec, providersBackstage, initialData) => {
    ...
    const onDrop = (comp, se) => {
      ...
      if (!Disabling.isDisabled(comp)) {
        ...
        handleFiles(
          comp,
          (_a = transferEvent.dataTransfer) === null || _a === void 0
            ? void 0
            : _a.files
        )
      }
    ...
    const handleFiles = (component, files) => {
      if (files) {
        Representing.setValue(
          component,
          filterByExtension(files, providersBackstage)
        )
  ...
  const renderIFrame = (spec, providersBackstage, initialData) => {
    ...
    const factory = (newSpec) =>
      craft({
        uid: newSpec.uid,
        dom: {
          tag: 'iframe',
          attributes,
          classes: isTransparent
            ? [baseClass]
            : [baseClass, `${baseClass}--opaque`]
        },
        behaviours: derive$1([
          Tabstopping.config({}),
          Focusing.config({}),
      ...
      const applyFramePositioning = () => {
        ...
        if (!isUndefined(cachedWidth) && !isUndefined(cachedHeight)) {
          ...
          const position = calculateImagePosition(
            get$c(frameComponent.element),
            get$d(frameComponent.element),
            cachedWidth,
            cachedHeight,
            translatedData.zoom
          )
  ...
  const onToolbarButtonExecute = (info) =>
    runOnExecute$1((comp, _simulatedEvent) => {
      runWithApi(
        info,
        comp
      )((itemApi) => {
  ...
  const renderIcon = (iconName, iconsProvider, behaviours) =>
    render$3(
      iconName,
      {
        tag: 'span',
  ...
  const renderIconFromPack = (iconName, iconsProvider) =>
    renderIcon(iconName, iconsProvider, [])
  const renderReplaceableIconFromPack = (iconName, iconsProvider) =>
  ...
  const renderCommonDropdown = (spec, prefix, sharedBackstage) => {
    ...
    const optMemDisplayText = spec.text.map((text) =>
      record(renderLabel(text, prefix, sharedBackstage.providers))
    )
    const optMemDisplayIcon = spec.icon.map((iconName) =>
      record(
        renderReplaceableIconFromPack(iconName, sharedBackstage.providers.icons)
      )
    )
      ...
      Dropdown.sketch({
        ...
        dropdownBehaviours: derive$1([
          ...
          DisablingConfigs.button(
            () => spec.disabled || sharedBackstage.providers.isDisabled()
          ),
          receivingConfig(),
          Unselecting.config({}),
          Replacing.config({}),
        ...
        parts: {
          menu: {
            ...
            onCollapseMenu: (
              tmenuComp,
              itemCompCausingCollapse,
              nowActiveMenuComp
            ) => {
              Highlighting.getHighlighted(nowActiveMenuComp).each(
                (itemComp) => {
  ...
  const unwrapReferences = (items, menuItems) => {
      ...
      (acc, item) => {
        if (isMenuItemReference(item)) {
          if (item === '') {
            ...
            return acc.length > 0 && !isSeparator$2(acc[acc.length - 1])
              ? acc.concat([separator$2])
              : acc
    ...
    if (
      realItems.length > 0 &&
      isSeparator$2(realItems[realItems.length - 1])
    ) {
      realItems.pop()
    }
    return realItems
  ...
  const generateValueIfRequired = (item) => {
    const itemValue = get$g(item, 'value').getOrThunk(() =>
      generate$6('generated-menu-item')
    )
  ...
  const build = (items, itemResponse, backstage, settings) => {
    ...
    const submenus = map$1(data.menus, (menuItems, menuName) =>
      createPartialMenu(
        menuName,
        menuItems,
        itemResponse,
  ...
  const fetchItems = (dropdownComp, name, items, selectedValue) =>
    map$2(items, (item) => {
      if (!isSingleListItem(item)) {
        return {
          ...
          text: item.text,
          getSubmenuItems: () =>
            fetchItems(dropdownComp, name, item.items, selectedValue)
        }
      } else {
        return {
          type: 'togglemenuitem',
  ...
  const getField = (comp, detail, partName) =>
    getPart(comp, detail, partName).bind(Composing.getCurrent)
  ...
  const parseSize = (sizeText) => {
    const numPattern =
      /^\s*(\d+(?:\.\d+)?)\s*(|cm|mm|in|px|pt|pc|em|ex|ch|rem|vw|vh|vmin|vmax|%)\s*$/
  ...
  const ratioSizeConversion = (scale, unit) => (size) =>
    convertUnit(size, unit).map((value) => ({
      value: value * scale,
      unit
    }))
  const makeRatioConverter = (currentFieldText, otherFieldText) => {
    ...
    return lift2(cValue, oValue, (cSize, oSize) =>
  ...
  const renderSizeInput = (spec, providersBackstage) => {
    ...
    const widthField = FormCoupledInputs.parts.field1(
      formGroup([FormField.parts.label(getLabel('Width')), getFieldPart(true)])
    )
    ...
    return Slider.sketch({
      ...
      model: {
        ...
        getInitialValue: constant$1(
          initialData.getOrThunk(
            () => (Math.abs(spec.max) - Math.abs(spec.min)) / 2
          )
        )
      },
  ...
  const renderInput = (spec, providersBackstage, initialData) =>
    renderTextField(
      {
        name: spec.name,
        multiline: false,
  ...
  const renderTextarea = (spec, providersBackstage, initialData) =>
    renderTextField(
      {
        name: spec.name,
        multiline: true,
    ...
    return derive$2(
      [
        ...
        streamConfig.cancelEvent
          .map((e) => [run$1(e, () => streamState.cancel())])
          .getOr([])
      )
    )
  }
  ...
  const make$3 = (detail, components, spec, externals) => {
    ...
    const mapFetch = (comp) => (tdata) =>
      tdata.map((data) => {
        const menus = values(data.menus)
        const items = bind$3(menus, (menu) =>
          filter$2(menu.items, (item) => item.type === 'item')
        )
          ...
          if (focusInInput) {
            if (get$6(component.element).length >= detail.minChars) {
              const previousValue = getActiveMenu(sandbox).bind((activeMenu) =>
                Highlighting.getHighlighted(activeMenu).map(
      ...
      Keying.config({
        ...
        onEnter: (comp) => {
          ...
          if (sandboxIsOpen && !detail.previewing.get()) {
            return getActiveMenu(sandbox)
              .bind((activeMenu) => Highlighting.getHighlighted(activeMenu))
              .map((item) => {
  ...
  const schema$g = constant$1(
    [
      ...
      defaultedObjOf('model', {}, [
        defaulted('getDisplayText', (itemData) =>
  ...
  const wrap = (delegate) => {
    ...
    const bindFuture = (f) => {
      return wrap(
        delegate.bind((resA) =>
          resA.fold(
            (err) => Future.pure(Result.error(err)),
            (a) => f(a)
          )
        )
      )
  ...
  const renderIconButtonSpec = (
    ...
    const icon = spec.icon.map((iconName) =>
      renderIconFromPack(iconName, providersBackstage.icons)
    )
  ...
  const renderButtonSpec = (
    ...
    const buttonType = spec.buttonType.getOr(
      !spec.primary && !spec.borderless ? 'secondary' : 'primary'
    )
  ...
  const isNormalFooterButtonSpec = (spec, buttonType) =>
    buttonType === 'custom' ||
    buttonType === 'cancel' ||
    buttonType === 'submit'
  const renderFooterButton = (spec, buttonType, backstage) => {
    if (isMenuFooterButtonSpec(spec, buttonType)) {
      ...
      const memButton = record(
        renderMenuButton(fixedSpec, 'tox-tbtn', backstage, Optional.none())
      )
  ...
  const filterLinkTargets = (type, targets) =>
    filter$2(targets, (target) => target.type === type)
  const filteredTargets = (type, targets) =>
    toMenuItems(filterLinkTargets(type, targets))
  const headerTargets = (linkInfo) =>
    filteredTargets('header', linkInfo.targets)
  const anchorTargets = (linkInfo) =>
    filteredTargets('anchor', linkInfo.targets)
  const anchorTargetTop = (linkInfo) =>
    Optional.from(linkInfo.anchorTop)
      .map((url) => staticMenuItem('<top>', url))
      .toArray()
  const anchorTargetBottom = (linkInfo) =>
    Optional.from(linkInfo.anchorBottom)
      .map((url) => staticMenuItem('<bottom>', url))
      .toArray()
  const historyTargets = (history) =>
    map$2(history, (url) => staticMenuItem(url, url))
  ...
  const filterByQuery = (term, menuItems) => {
    ...
    return filter$2(menuItems, (item) => {
      ...
      return (
        ...
        contains$1(value.toLowerCase(), lowerCaseTerm)
      )
    })
  }
  ...
  const renderUrlInput = (spec, backstage, urlBackstage, initialData) => {
    ...
    const makeIcon = (name, errId, icon = name, label = name) =>
      render$3(
        icon,
        {
          tag: 'div',
  ...
  const make$2 = (render) => {
    return (parts, spec, dialogData, backstage) =>
      get$g(spec, 'name').fold(
        () => render(spec, backstage, Optional.none()),
        (fieldName) =>
          parts.field(
            fieldName,
            render(spec, backstage, get$g(dialogData, fieldName))
          )
      )
  }
  ...
  const factories = {
    ...
    collection: make$2((spec, backstage, data) =>
      renderCollection(spec, backstage.shared.providers, data)
    ),
    alertbanner: make$2((spec, backstage) =>
      renderAlertBanner(spec, backstage.shared.providers)
    ),
    input: make$2((spec, backstage, data) =>
      renderInput(spec, backstage.shared.providers, data)
    ),
    textarea: make$2((spec, backstage, data) =>
      renderTextarea(spec, backstage.shared.providers, data)
    ),
    label: make$2((spec, backstage) => renderLabel$1(spec, backstage.shared)),
    iframe: makeIframe((spec, backstage, data) =>
      renderIFrame(spec, backstage.shared.providers, data)
    ),
    button: make$2((spec, backstage) =>
      renderDialogButton(spec, backstage.shared.providers)
    ),
    checkbox: make$2((spec, backstage, data) =>
      renderCheckbox(spec, backstage.shared.providers, data)
    ),
    colorinput: make$2((spec, backstage, data) =>
      renderColorInput(spec, backstage.shared, backstage.colorinput, data)
    ),
    colorpicker: make$2((spec, backstage, data) =>
      renderColorPicker(spec, backstage.shared.providers, data)
    ),
    dropzone: make$2((spec, backstage, data) =>
      renderDropZone(spec, backstage.shared.providers, data)
    ),
    grid: make$2((spec, backstage) => renderGrid(spec, backstage.shared)),
    listbox: make$2((spec, backstage, data) =>
      renderListBox(spec, backstage, data)
    ),
    selectbox: make$2((spec, backstage, data) =>
      renderSelectBox(spec, backstage.shared.providers, data)
    ),
    sizeinput: make$2((spec, backstage) =>
      renderSizeInput(spec, backstage.shared.providers)
    ),
    slider: make$2((spec, backstage, data) =>
      renderSlider(spec, backstage.shared.providers, data)
    ),
    urlinput: make$2((spec, backstage, data) =>
      renderUrlInput(spec, backstage, backstage.urlinput, data)
    ),
    customeditor: make$2(renderCustomEditor),
    htmlpanel: make$2(renderHtmlPanel),
    imagepreview: make$2((spec, _, data) => renderImagePreview(spec, data)),
    table: make$2((spec, backstage) =>
    ...
    const newBackstage = deepMerge(oldBackstage, {
      shared: {
        interpreter: (childSpec) =>
          interpretParts(parts, childSpec, dialogData, newBackstage)
      }
    })
    return interpretParts(parts, spec, dialogData, newBackstage)
  }
  const interpretParts = (parts, spec, dialogData, backstage) =>
    get$g(factories, spec.type).fold(
      () => {
        console.error(
  ...
  const north = (anchor, element, bubbles) =>
    nu$6(
      ...
      south$3(),
      'north',
      boundsRestriction(anchor, { top: 2 }),
      labelPrefix
    )
  const south = (anchor, element, bubbles) =>
    nu$6(
      middleX(anchor, element),
      southY(anchor, element),
      bubbles.insetSouth(),
      north$3(),
      'south',
      boundsRestriction(anchor, { bottom: 3 }),
      labelPrefix
    )
  const east = (anchor, element, bubbles) =>
    nu$6(
      eastEdgeX(anchor, element),
      ...
      west$3(),
      'east',
      boundsRestriction(anchor, { right: 0 }),
      labelPrefix
    )
  const west = (anchor, element, bubbles) =>
    nu$6(
      westEdgeX(anchor),
      centreY(anchor, element),
      bubbles.insetWest(),
      east$3(),
      'west',
      boundsRestriction(anchor, { left: 1 }),
      labelPrefix
    )
  ...
  const getInlineDialogAnchor = (
    contentAreaElement,
    lazyAnchorbar,
    lazyUseEditableAreaAnchor
  ) => {
    const bubbleSize = 12
    ...
    return () =>
      lazyUseEditableAreaAnchor() ? editableAreaAnchor() : standardAnchor()
  }
  const getBannerAnchor = (
    contentAreaElement,
    lazyAnchorbar,
    lazyUseEditableAreaAnchor
  ) => {
  ...
  const getAnchors = (editor, lazyAnchorbar, isToolbarTop) => {
    ...
    const contentAreaElement = () =>
      SugarElement.fromDom(editor.getContentAreaContainer())
    const lazyUseEditableAreaAnchor = () =>
      useFixedToolbarContainer || !isToolbarTop()
    return {
    ...
    foldl(
      ...
      (acc, fmt) => {
        if (isNestedFormats(fmt)) {
          ...
          const formatName = isString(fmt.name)
            ? fmt.name
            : fmt.title.toLowerCase()
  ...
  const getStyleFormats = (editor) =>
    getUserStyleFormats(editor)
      .map((userFormats) => {
        const registeredUserFormats = registerCustomFormats(editor, userFormats)
        return shouldMergeStyleFormats(editor)
  ...
  const register$a = (editor, formats, isSelectedFor, getPreviewFor) => {
    ...
    const enrichCustom = (item) => {
      const formatName = isString(item.name)
        ? item.name
        : generate$6(item.title)
      const formatNameWithPrefix = `custom-${formatName}`
      const newItem = {
        ...item,
        type: 'formatter',
  ...
  const init$8 = (editor) => {
    ...
    const getPreviewFor = (format) => () => {
      ...
      return fmt !== undefined
        ? Optional.some({
            tag:
              fmt.length > 0 ? fmt[0].inline || fmt[0].block || 'div' : 'div',
    ...
    editor.on('addStyleModifications', (e) => {
      const modifications = register$a(
        editor,
        e.items,
        isSelectedFor,
        getPreviewFor
  ...
  const select = (selector, root) => {
    return map$2(
      descendants(SugarElement.fromDom(root), selector),
      (element) => {
        return element.dom
      }
    )
  ...
  const headerTarget = (elm) => {
    ...
    return create(
      'header',
      (_a = getElementText(elm)) !== null && _a !== void 0 ? _a : '',
      '#' + headerId,
      getLevel(elm),
      attach
    )
  ...
  const isArrayOfUrl = (a) =>
    isArray(a) && a.length <= HISTORY_LENGTH && forall(a, isHttpUrl)
  const isRecordOfUrlArray = (r) =>
    isObject(r) && find$4(r, (value) => !isArrayOfUrl(value)).isNone()
  const getAllHistory = () => {
  ...
  const getPickerTypes = (editor) => {
    const optFileTypes = Optional.from(getFilePickerTypes(editor))
      .filter(isTruthy)
      .map(makeMap)
    return getPicker(editor).fold(never, (_picker) =>
      optFileTypes.fold(always, (types) =>
        keys(types).length > 0 ? types : false
      )
    )
  }
  ...
  const getUrlPicker = (editor, filetype) =>
    getPickerSetting(editor, filetype).map(
      (picker) => (entry) =>
        Future.nu((completer) => {
  ...
  const getTextSetting = (value) =>
    Optional.from(value).filter(isString).getOrUndefined()
  ...
  const getValidationHandler = (editor) =>
    Optional.from(getFilePickerValidatorHandler(editor))
  ...
  const setup$b = (editor, mothership, uiMothership) => {
    ...
    const fireDismissPopups = (evt) =>
      broadcastOn(dismissPopups(), { target: evt.target })
    const doc = getDocument()
    const onTouchstart = bind(doc, 'touchstart', fireDismissPopups)
    const onTouchmove = bind(doc, 'touchmove', (evt) =>
      broadcastEvent(documentTouchmove(), evt)
    )
    const onTouchend = bind(doc, 'touchend', (evt) =>
      broadcastEvent(documentTouchend(), evt)
    )
    ...
    const onContentClick = (raw) =>
      broadcastOn(dismissPopups(), { target: SugarElement.fromDom(raw.target) })
    ...
    const onWindowScroll = (evt) =>
      broadcastEvent(windowScroll(), fromRawEvent(evt))
  ...
  const factory$d = (detail, components, _spec, _external) => {
    ...
    const getListContainer = (component) =>
      detail.shell
  ...
  const getPrior = (elem, state) =>
    state
      .getInitialPos()
      .map((pos) =>
        bounds(pos.bounds.x, pos.bounds.y, get$c(elem), get$d(elem))
      )
  ...
  const revertToOriginal = (elem, box, state) =>
    state.getInitialPos().bind((position) => {
      ...
      switch (position.position) {
        ...
        case 'absolute':
          const offsetBox = getOffsetParent(elem)
            .map(box$1)
            .getOrThunk(() => box$1(body()))
          return Optional.some(
            morphAdt.absolute(
              NuPositionCss(
                'absolute',
                get$g(position.style, 'left').map(
                  (_left) => box.x - offsetBox.x
                ),
                get$g(position.style, 'top').map((_top) => box.y - offsetBox.y),
                get$g(position.style, 'right').map(
                  (_right) => offsetBox.right - box.right
                ),
                get$g(position.style, 'bottom').map(
                  (_bottom) => offsetBox.bottom - box.bottom
                )
              )
            )
          )
  ...
  const morphToOriginal = (elem, viewport, state) =>
    getPrior(elem, state)
  ...
  const getMorph = (component, viewport, state) => {
    ...
    return isDocked
      ? morphToOriginal(elem, viewport, state)
  ...
  const morphToStatic = (component, config, state) => {
    ...
    each$1(['left', 'right', 'top', 'bottom', 'position'], (prop) =>
      remove$6(component.element, prop)
    )
  ...
  const events$5 = (dockInfo, dockState) =>
    derive$2([
  ...
  const init$6 = (spec) => {
    ...
    const readState = () =>
      `docked:  ${docked.get()}, visible: ${visible.get()}, modes: ${modes
        .get()
        .join(',')}`
    return nu$8({
      isDocked: docked.get,
  ...
  const isDockedMode = (header, mode) =>
    contains$2(Docking.getModes(header), mode)
  const updateIframeContentFlow = (header) => {
    const getOccupiedHeight = (elm) =>
      getOuter$2(elm) +
  ...
  const restoreFocus = (headerElem, focusedElem) => {
    ...
    active$1(ownerDoc)
      .filter((activeElm) => !eq(focusedElem, activeElm))
      .filter(
        (activeElm) =>
          eq(activeElm, SugarElement.fromDom(ownerDoc.dom.body)) ||
          contains(headerElem, activeElm)
      )
      .each(() => focus$3(focusedElem))
  }
  const findFocusedElem = (rootElm, lazySink) =>
    search(rootElm).orThunk(() =>
      lazySink()
        .toOptional()
        .bind((sink) => search(sink.element))
    )
  ...
  const isDocked = (lazyHeader) =>
    lazyHeader().map(Docking.isDocked).getOr(false)
  const getIframeBehaviours = () => [
    Receiving.config({
      channels: {
        [toolbarHeightChange()]: { onReceive: updateIframeContentFlow }
      }
    })
  ]
  const getBehaviours = (editor, sharedBackstage) => {
    ...
    const onDockingSwitch = (comp) => {
      ...
      lazySink().each((sink) =>
        sink.getSystem().broadcastOn([repositionPopups()], {})
      )
    }
      ...
      Docking.config({
        contextual: {
          lazyContext: (comp) => {
            ...
            const topBound =
              box.y + (isDockedMode(comp, 'top') ? 0 : headerHeight)
          ...
          onHide: (comp) => {
            findFocusedElem(comp.element, lazySink).fold(
              focusedElm.clear,
              focusedElm.set
            )
          ...
          onHidden: () => {
            runOnSinkElement((elem) =>
              remove$1(elem, [visibility.transitionClass])
            )
          },
          ...visibility
        },
        lazyViewport: (comp) => {
          ...
          const height =
            win$1.height - (isDockedMode(comp, 'bottom') ? offset : 0)
  ...
  const groupToolbarButtonSchema = objOf(
    [
      ...
      requiredOf(
        'items',
        oneOf([arrOfObj([name$1, requiredArrayOf('items', string)]), string])
      )
    ].concat(baseToolbarButtonFields)
  )
  const createGroupToolbarButton = (spec) =>
    asRaw('GroupToolbarButton', groupToolbarButtonSchema, spec)
  ...
  const baseMenuButtonFields = [
    ...
    defaultedOf(
      'search',
      false,
      oneOf([boolean, objOf([optionString('placeholder')])], (x) => {
        if (isBoolean(x)) {
          return x
            ? Optional.some({ placeholder: Optional.none() })
            : Optional.none()
        } else {
          return Optional.some(x)
        }
  ...
  const createSplitButton = (spec) =>
    asRaw('SplitButton', splitButtonSchema, spec)
    ...
    const setMenus = (comp, menus) => {
      const newMenus = map$2(menus, (m) => {
        ...
        const internal = createMenuButton(buttonSpec)
          .mapError((errInfo) => formatError(errInfo))
          .getOrDie()
      ...
      behaviours: derive$1([
        ...
        config('menubar-events', [
          ...
          run$1(mouseover(), (comp, se) => {
            descendant(comp.element, '.' + 'tox-mbtn--active').each(
              (activeButton) => {
                closest$1(se.event.target, '.' + 'tox-mbtn').each(
                  (hoveredButton) => {
          ...
          run$1(focusShifted(), (comp, se) => {
            se.event.prevFocus
              .bind((prev) => comp.getSystem().getByDom(prev).toOptional())
              .each((prev) => {
                se.event.newFocus
                  .bind((nu) => comp.getSystem().getByDom(nu).toOptional())
                  .each((nu) => {
  ...
  const promotionLink =
    'https://www.tiny.cloud/tinymce-self-hosted-premium-features/?utm_source=TinyMCE&utm_medium=SPAP&utm_campaign=SPAP&utm_id=editorreferral'
  ...
  const getAnimationRoot = (component, slideConfig) =>
  ...
  const hasGrown = (component, slideConfig, slideState) =>
    slideState.isExpanded()
  const hasShrunk = (component, slideConfig, slideState) =>
  ...
  const isTransitioning = (component, slideConfig, slideState) =>
    isGrowing(component, slideConfig) || isShrinking(component, slideConfig)
  ...
  const events$4 = (slideConfig, slideState) =>
    derive$2([
      runOnSource(transitionend(), (component, simulatedEvent) => {
  ...
  const make$1 = (detail, components) => {
    ...
    const onSlot = (f, def) => (container, key) =>
      getPart(container, detail, key)
        .map((slot) => f(slot, key))
        .getOr(def)
    const onSlots = (f) => (container, keys) => {
      each$1(keys, (key) => f(container, key))
    }
    const doShowing = (comp, _key) =>
      get$f(comp.element, 'aria-hidden') !== 'true'
  ...
  const setup$8 = (editor) => {
    ...
    each$1(keys(sidebars), (name) => {
      ...
      const isActive = () =>
        is$1(Optional.from(editor.queryCommandValue('ToggleSidebar')), name)
  ...
  const makeSidebar = (panelConfigs) =>
  ...
  const setSidebar = (sidebar, panelConfigs, showSidebar) => {
    ...
    optSlider.each((slider) => {
      ...
      const configKey =
        showSidebar === null || showSidebar === void 0
          ? void 0
          : showSidebar.toLowerCase()
    ...
    return optSlider.bind((slider) => {
      ...
      if (sidebarOpen) {
        ...
        return optSlotContainer.bind((slotContainer) =>
          find$5(SlotContainer.getSlotNames(slotContainer), (name) =>
  ...
  const focusBusyComponent = (throbber) =>
    Composing.getCurrent(throbber).each((comp) => focus$3(comp.element))
  const toggleEditorTabIndex = (editor, state) => {
    ...
    Optional.from(editor.iframeElement)
      ...
      .each((iframe) => {
        if (state) {
          getOpt(iframe, tabIndexAttr).each((tabIndex) =>
            set$9(iframe, dataTabIndexAttr, tabIndex)
          )
  ...
  const toggleThrobber = (editor, comp, state, providerBackstage) => {
    ...
    if (state) {
      ...
      const throbberFocus = Composing.getCurrent(comp).exists((busyComp) =>
        hasFocus(busyComp.element)
      )
  ...
  const isPasteBinTarget = (event) => {
    if (isFocusEvent(event)) {
      const node = event.composed
        ? head(event.composedPath())
        : Optional.from(event.target)
      return node
        .map(SugarElement.fromDom)
        .filter(isElement$1)
  ...
  const setup$7 = (editor, lazyThrobber, sharedBackstage) => {
    ...
    editor.on('ProgressState', (e) => {
      timer.on(clearTimeout)
      if (isNumber(e.time)) {
        const timerId = global$9.setEditorTimeout(
          editor,
          () => toggle(e.state),
          e.time
        )
  ...
  const fitAll = (within, extra, withinWidth) =>
    output(toUnit(within), [], withinWidth)
  ...
  const findFocusedComp = (comps) =>
    findMap(comps, (comp) =>
      search(comp.element).bind((focusedElm) =>
        comp.getSystem().getByDom(focusedElm).toOptional()
      )
    )
  const refresh$2 = (toolbar, detail, setOverflow) => {
    ...
    const overflows = partition(
      availableWidth,
      detail.builtGroups.get(),
      (comp) => get$c(comp.element),
      overflowGroup
    )
  ...
  const factory$8 = (detail, components, spec, externals) => {
    const memFloatingToolbarButton = record(
    ...
    return {
      ...
      apis: {
        ...
        isOpen: (toolbar) =>
          memFloatingToolbarButton
            .getOpt(toolbar)
            .map(FloatingToolbarButton.isOpen)
            .getOr(false),
        reposition: (toolbar) => {
          memFloatingToolbarButton
            .getOpt(toolbar)
        ...
        getOverflow: (toolbar) =>
          memFloatingToolbarButton
            .getOpt(toolbar)
            .bind(FloatingToolbarButton.getToolbar)
      },
      domModification: { attributes: { role: 'group' } }
    }
  }
  ...
  const isOpen = (toolbar, detail) =>
    getPart(toolbar, detail, 'overflow').map(Sliding.hasGrown).getOr(false)
  const toggleToolbar = (toolbar, detail) => {
    getPart(toolbar, detail, 'overflow-button')
      .bind(() => getPart(toolbar, detail, 'overflow'))
      .each((overf) => {
  ...
  const renderToolbarGroupCommon = (toolbarGroup) => {
    ...
    return {
      ...
      markers: {
        itemSelector:
          '*:not(.tox-split-button) > .tox-tbtn:not([disabled]), ' +
  ...
  const renderToolbarGroup = (toolbarGroup) =>
    ToolbarGroup.sketch(renderToolbarGroupCommon(toolbarGroup))
  ...
  const renderSlidingMoreToolbar = (toolbarSpec) => {
    ...
    return SplitSlidingToolbar.sketch({
      ...
      onOpened: (comp) => {
        comp
          .getSystem()
          .broadcastOn([toolbarHeightChange()], { type: 'opened' })
      },
      onClosed: (comp) => {
        comp
          .getSystem()
          .broadcastOn([toolbarHeightChange()], { type: 'closed' })
      }
    })
  }
    ...
    return Toolbar.sketch({
      ...
      dom: {
        ...
        classes: ['tox-toolbar'].concat(
          toolbarSpec.type === ToolbarMode$1.scrolling
    ...
    const apis = {
      ...
      setSidebar: (comp, panelConfigs, showSidebar) => {
        parts$a
          ...
          .each((sidebar) => setSidebar(sidebar, panelConfigs, showSidebar))
      },
      toggleSidebar: (comp, name) => {
        parts$a
          .getPart(comp, detail, 'sidebar')
          .each((sidebar) => toggleSidebar(sidebar, name))
      },
      whichSidebar: (comp) => {
        return parts$a
          .getPart(comp, detail, 'sidebar')
      ...
      setToolbars: (comp, toolbars) => {
        parts$a.getPart(comp, detail, 'multiple-toolbar').each((mToolbar) => {
          const renderedToolbars = map$2(toolbars, (g) =>
            map$2(g, renderToolbarGroup)
          )
      ...
      isToolbarDrawerToggled: (comp) => {
        return parts$a
          ...
          .bind((toolbar) =>
            Optional.from(toolbar.getApis().isOpen).map((isOpen) =>
              isOpen(toolbar)
            )
          )
          .getOr(false)
      ...
      focusToolbar: (comp) => {
        const optToolbar = parts$a
          .getPart(comp, detail, 'toolbar')
          .orThunk(() => parts$a.getPart(comp, detail, 'multiple-toolbar'))
  ...
  const identifyMenus = (editor, registry) => {
    ...
    const validMenus = filter$2(menubar, (menuName) => {
      ...
      if (userDefinedMenus) {
        return (
          isDefaultMenu ||
          get$g(registry.menus, menuName).exists((menu) => has$2(menu, 'items'))
        )
      } else {
    ...
    return filter$2(menus, (menu) => {
      const isNotSeparator = (item) =>
        isString(item) || item.type !== 'separator'
      return (
        menu.getItems().length > 0 && exists(menu.getItems(), isNotSeparator)
      )
    })
  }
  ...
  const fireSkinLoadError = (editor, err) => () =>
    fireSkinLoadError$1(editor, { message: err })
  ...
  const loadShadowDomUiSkins = (editor, skinUrl) => {
    const isInShadowRoot$1 = isInShadowRoot(
  ...
  const loadSkin = (isInline, editor) => {
    ...
    if (!isSkinDisabled(editor) && isString(skinUrl)) {
      Promise.all([
        ...
        fireSkinLoadError(editor, 'Skin could not be loaded')
      )
    } else {
      fireSkinLoaded(editor)()
    }
  ...
  const onActionExecCommand = (editor, command) => () =>
  ...
  const generateSelectItems = (_editor, backstage, spec) => {
    const generateItem = (rawItem, response, invalid, value) => {
      ...
      if (rawItem.type === 'separator') {
        ...
        const items = bind$3(rawItem.getStyleItems(), (si) =>
        ...
        if (response === 0 && items.length <= 0) {
          ...
          return Optional.some({
            ...
            getSubmenuItems: () =>
              bind$3(rawItem.getStyleItems(), (si) =>
                validate(si, response, value)
              )
          })
        }
  ...
  const createMenuItems = (editor, backstage, spec) => {
    ...
    const getStyleItems =
      dataset.type === 'basic'
        ? () =>
            map$2(dataset.data, (d) =>
              processBasic(d, spec.isSelectedFor, spec.getPreviewFor)
            )
        : dataset.getData
    return {
  ...
  const process = (rawFormats) =>
    map$2(rawFormats, (item) => {
      let title = item,
        format = item
  ...
  const getSpec$4 = (editor) => {
    const getMatchingValue = () =>
      find$5(alignMenuItems, (item) => editor.formatter.match(item.format))
    const isSelectedFor = (format) => () => editor.formatter.match(format)
    const getPreviewFor = (_format) => Optional.none
    const updateSelectMenuIcon = (comp) => {
      const match = getMatchingValue()
      const alignment = match.fold(constant$1('left'), (item) =>
    ...
    const onAction = (rawItem) => () =>
      find$5(alignMenuItems, (item) => item.format === rawItem.format).each(
        (item) => editor.execCommand(item.command)
      )
    return {
      tooltip: 'Align',
      text: Optional.none(),
  ...
  const createAlignButton = (editor, backstage) =>
    createSelectButton(editor, backstage, getSpec$4(editor))
  ...
  const findNearest = (editor, getStyles) => {
    ...
    return Optional.from(editor.formatter.closest(formats))
      .bind((fmt) => find$5(styles, (data) => data.format === fmt))
      .orThunk(() =>
        someIf(editor.formatter.match('p'), {
          title: 'Paragraph',
          format: 'p'
        })
      )
  ...
  const createBlocksButton = (editor, backstage) =>
    createSelectButton(editor, backstage, getSpec$3(editor))
  ...
  const getSpec$2 = (editor) => {
    ...
    const isSelectedFor = (item) => (valueOpt) =>
  ...
  const createFontFamilyButton = (editor, backstage) =>
    createSelectButton(editor, backstage, getSpec$2(editor))
  ...
  const createFontSizeButton = (editor, backstage) =>
    createSelectButton(editor, backstage, getSpec$1(editor))
  ...
  const getTooltipAttributes = (tooltip, providersBackstage) =>
    tooltip
      .map((tooltip) => ({
        'aria-label': providersBackstage.translate(tooltip),
        title: providersBackstage.translate(tooltip)
              ...
              .map((r) =>
                Reflecting.config({
                  ...
                  renderComponents: (data, _state) =>
                    componentRenderPipeline([
                      data.icon.map((iconName) =>
                        renderIconFromPack(iconName, providersBackstage.icons)
                      ),
                      data.text.map((text) =>
                        renderLabel(text, 'tox-tbtn', providersBackstage)
                      )
                    ])
                })
              )
  ...
  const renderFloatingToolbarButton = (
    spec,
    backstage,
    identifyButtons,
    attributes
  ...
  const renderCommonToolbarButton = (
    spec,
    specialisation,
    providersBackstage
  ) => {
  ...
  const renderToolbarButton = (spec, providersBackstage) =>
    renderToolbarButtonWith(spec, providersBackstage, [])
  const renderToolbarButtonWith = (spec, providersBackstage, bonusEvents) =>
  ...
  const renderToolbarToggleButton = (spec, providersBackstage) =>
    renderToolbarToggleButtonWith(spec, providersBackstage, [])
  const renderToolbarToggleButtonWith = (
    spec,
    providersBackstage,
    bonusEvents
  ) =>
  ...
  const fetchChoices = (getApi, spec, providersBackstage) => (comp) =>
    Future.nu((callback) => spec.fetch(callback)).map((items) =>
      Optional.from(
        createTieredDataFrom(
          deepMerge(
            ...
            ),
            {
              ...
              menuBehaviours: SimpleBehaviours.unnamedEvents(
                spec.columns !== 'auto'
                  ...
                  : [
                      runOnAttached((comp, _se) => {
                        detectSize(comp, 4, classForPreset(spec.presets)).each(
                          ({ numRows, numColumns }) => {
                            Keying.setGridSize(comp, numRows, numColumns)
                          }
                        )
      ...
      setActive: (state) => {
        ...
        descendant(comp.element, 'span').each((button) => {
          comp
            .getSystem()
            .getByDom(button)
            .each((buttonComp) => Toggling.set(buttonComp, state))
        })
      },
      isActive: () =>
        descendant(comp.element, 'span').exists((button) =>
          comp.getSystem().getByDom(button).exists(Toggling.isOn)
        )
    })
    const editorOffCell = Cell(noop)
  ...
  const renderFromBridge =
    (bridgeBuilder, render) => (spec, backstage, editor) => {
  ...
  const types = {
    button: renderFromBridge(createToolbarButton, (s, backstage) =>
      renderToolbarButton(s, backstage.shared.providers)
    ),
    togglebutton: renderFromBridge(createToggleButton, (s, backstage) =>
      renderToolbarToggleButton(s, backstage.shared.providers)
    ),
    menubutton: renderFromBridge(createMenuButton, (s, backstage) =>
      renderMenuButton(s, 'tox-tbtn', backstage, Optional.none())
    ),
    splitbutton: renderFromBridge(createSplitButton, (s, backstage) =>
      renderSplitButton(s, backstage.shared)
    ),
    grouptoolbarbutton: renderFromBridge(
      createGroupToolbarButton,
      (s, backstage, editor) => {
        const buttons = editor.ui.registry.getAll().buttons
        const identify = (toolbar) =>
          identifyButtons(
            editor,
            {
              buttons,
        ...
        const attributes = {
          [Attribute]: backstage.shared.header.isPositionedAtTop()
  ...
  const extractFrom = (spec, backstage, editor) =>
    get$g(types, spec.type).fold(
      () => {
  ...
  const removeUnusedDefaults = (buttons) => {
    const filteredItemGroups = map$2(defaultToolbar, (group) => {
      const items = filter$2(
        group.items,
        (subItem) => has$2(buttons, subItem) || has$2(bespokeButtons, subItem)
      )
      return {
        name: group.name,
        items
  ...
  const isToolbarGroupSettingArray = (toolbar) =>
    isArrayOf(toolbar, (t) => has$2(t, 'name') && has$2(t, 'items'))
  ...
  const lookupButton = (
    ...
    get$g(buttons, toolbarItem.toLowerCase())
      .orThunk(() =>
        prefixes.bind((ps) =>
          findMap(ps, (prefix) =>
            get$g(buttons, prefix + toolbarItem.toLowerCase())
          )
        )
      )
      .fold(
        () =>
          get$g(bespokeButtons, toolbarItem.toLowerCase()).map((r) =>
            r(editor, backstage)
          ),
        (spec) => {
  ...
  const setupEvents$1 = (editor, uiComponents) => {
    ...
    const lastWindowDimensions = Cell(
      SugarPosition(contentWindow.innerWidth, contentWindow.innerHeight)
    )
    const lastDocumentDimensions = Cell(
      SugarPosition(initialDocEle.offsetWidth, initialDocEle.offsetHeight)
    )
    const resizeWindow = () => {
    ...
    const resizeDocument = () => {
      ...
      if (
        ...
        lastDocumentDimensions.set(
          SugarPosition(docEle.offsetWidth, docEle.offsetHeight)
        )
        fireResizeContent(editor)
      }
    }
  ...
  const render$1 = (editor, uiComponents, rawUiConfig, backstage, args) => {
    ...
    const socket = OuterContainer.getSocket(outerContainer).getOrDie(
    ...
    editor.addQueryValueHandler('ToggleSidebar', () => {
      var _a
      return (_a = OuterContainer.whichSidebar(outerContainer)) !== null &&
        _a !== void 0
        ? _a
        : ''
    })
    ...
    if (
      toolbarMode === ToolbarMode$1.sliding ||
      toolbarMode === ToolbarMode$1.floating
    ) {
  ...
  const getHeight = (editor) => {
    ...
    return parseToInt(baseHeight).map((height) =>
      calcCappedSize(height, minHeight, maxHeight)
    )
  }
  ...
  const getWidth = (editor) => {
    ...
    return parseToInt(baseWidth).map((width) =>
      calcCappedSize(width, minWidth, maxWidth)
    )
  }
  ...
  const InlineHeader = (
    ...
    const isSplitToolbar =
      toolbarMode === ToolbarMode.sliding ||
      toolbarMode === ToolbarMode.floating
    const visible = Cell(false)
    const isVisible = () => visible.get() && !editor.removed
    const calcToolbarOffset = (toolbar) =>
      isSplitToolbar
        ? toolbar.fold(constant$1(0), (tbar) =>
            tbar.components().length > 1
              ? get$d(tbar.components()[1].element)
              : 0
          )
        : 0
        ...
        case ToolbarLocation.auto:
          ...
          if (roomAtTop) {
            ...
            if (roomAtBottom) {
              ...
              const winBounds = win()
              const isRoomAtBottomViewport =
                winBounds.bottom < targetBounds.bottom - toolbarHeight
    ...
    const setupMode = (mode) => {
      floatContainer.on((container) => {
        ...
        const verticalDir = isPositionedAtTop()
          ? AttributeValue.TopToBottom
          : AttributeValue.BottomToTop
  ...
  const setupEvents = (editor, targetElm, ui, toolbarPersist) => {
    const prevPosAndBounds = Cell(
      getTargetPosAndBounds(targetElm, ui.isPositionedAtTop())
    )
    const resizeContent = (e) => {
      const { pos, bounds } = getTargetPosAndBounds(
        targetElm,
        ui.isPositionedAtTop()
      )
    ...
    elementLoad.set(
      capture(SugarElement.fromDom(editor.getBody()), 'load', (e) =>
        resizeContent(e.raw)
      )
    )
    editor.on('remove', () => {
  ...
  const runOnExecute = (memInput, original) =>
    run$1(internalToolbarButtonExecute, (comp, se) => {
  ...
  const generate = (memInput, buttons, providersBackstage) => {
    const mementos = map$2(buttons, (button) =>
      record(generateOne(memInput, button, providersBackstage))
    )
    const asSpecs = () => map$2(mementos, (mem) => mem.asSpec())
    const findPrimary = (compInSystem) =>
      findMap(buttons, (button, i) => {
        if (button.primary) {
          return Optional.from(mementos[i])
  ...
  const renderContextForm = (toolbarType, ctx, providers) =>
    renderToolbar({
      type: toolbarType,
  ...
  const isVerticalOverlap = (a, b, threshold = 0.01) =>
  ...
  const getAnchorElementBounds = (editor, lastElement) =>
    lastElement
  ...
  const getVerticalBounds = (
    ...
    const containerBounds =
      toolbarType === 'line' ? box$1(container) : contentAreaBox
    if (isToolbarAbove) {
      return {
        y: Math.max(headerBox.bottom + margin, viewportBounds.y),
  ...
  const getContextToolbarBounds = (
    ...
    const toolbarOrMenubarEnabled =
      ...
      isToolbarEnabled(editor) ||
      isMultipleToolbars(editor)
    const { x, width } = getHorizontalBounds(
      contentAreaBox,
      viewportBounds,
      margin
    )
  ...
  const isEntireElementSelected = (editor, elem) => {
    ...
    return (
      ...
      rng.startOffset === rng.endOffset - 1 &&
      eq(leaf$1.element, elem)
    )
  }
  ...
  const determineInsetLayout = (editor, contextbar, elem, data, bounds) => {
    ...
    const isSameAnchorElement = data
      .lastElement()
      .exists((prev) => eq(elem, prev))
    if (isEntireElementSelected(editor, elem)) {
      return isSameAnchorElement ? preserve : north
    } else if (isSameAnchorElement) {
      return preservePosition(contextbar, data.getMode(), () => {
        const isOverlapping = isVerticalOverlap(
          selectionBounds,
          box$1(contextbar)
        )
  ...
  const matchTargetWith = (elem, candidates) => {
    const ctxs = filter$2(candidates, (toolbarApi) =>
  ...
  const filterByPositionForStartNode = (toolbars) => {
    if (toolbars.length <= 1) {
      ...
      const doesPositionExist = (value) =>
        exists(toolbars, (t) => t.position === value)
      const filterToolbarsByPosition = (value) =>
        filter$2(toolbars, (t) => t.position === value)
      ...
      if (hasSelectionToolbars || hasNodeToolbars) {
        if (hasNodeToolbars && hasSelectionToolbars) {
          ...
          const selectionToolbars = map$2(
            filterToolbarsByPosition('selection'),
            (t) => ({
              ...t,
              position: 'node'
            })
          ...
          return hasSelectionToolbars
            ? filterToolbarsByPosition('selection')
            : filterToolbarsByPosition('node')
        }
      } else {
  ...
  const filterByPositionForAncestorNode = (toolbars) => {
    if (toolbars.length <= 1) {
      ...
      const findPosition = (value) =>
        find$5(toolbars, (t) => t.position === value)
  ...
  const matchStartNode = (elem, nodeCandidates, editorCandidates) => {
    ...
    if (nodeMatches.contextForms.length > 0) {
      ...
      if (editorMatches.contextForms.length > 0) {
        ...
        nodeMatches.contextToolbars.length > 0 ||
        editorMatches.contextToolbars.length > 0
      ) {
        const toolbars = filterByPositionForStartNode(
          nodeMatches.contextToolbars.concat(editorMatches.contextToolbars)
        )
        return Optional.some({
          elem,
          toolbars
      ...
      return ancestor$2(
        ...
        (ancestorElem) => {
          if (isElement$1(ancestorElem)) {
            const { contextToolbars, contextForms } = matchTargetWith(
              ancestorElem,
              scopes.inNodeScope
            )
            const toolbars =
              contextForms.length > 0
                ? contextForms
                : filterByPositionForAncestorNode(contextToolbars)
  ...
  const lookup$1 = (scopes, editor) => {
    ...
    const isOutsideRoot = (startNode) =>
      !isRoot(startNode) && !contains(rootElem, startNode)
    const startNode = SugarElement.fromDom(editor.selection.getNode())
    if (isOutsideRoot(startNode)) {
      return Optional.none()
    }
    return matchStartNode(
      startNode,
      scopes.inNodeScope,
      scopes.inEditorScope
    ).orThunk(() => matchAncestor(isRoot, startNode, scopes))
  }
  ...
  const register$9 = (editor, registryContextToolbars, sink, extras) => {
    ...
    const isSameLaunchElement = (elem) =>
      is$1(lift2(elem, lastElement.get(), eq), true)
    const shouldContextToolbarHide = () => {
      if (!canLaunchToolbar()) {
        ...
        return (
          contextToolbarBounds.height <= 0 ||
          !isVerticalOverlap(anchorBounds, contextToolbarBounds)
        )
      }
    }
    const close = () => {
    ...
    const getScopes = cached(() =>
      categorise(registryContextToolbars, (toolbarApi) => {
    ...
    const buildContextFormGroups = (ctx, providers) =>
      ContextForm.buildInitGroups(ctx, providers)
    const buildToolbar = (toolbars) => {
      ...
      const toolbarType =
        getToolbarMode(editor) === ToolbarMode$1.scrolling
          ? ToolbarMode$1.scrolling
          : ToolbarMode$1.default
      const initGroups = flatten(
        map$2(toolbars, (ctx) =>
          ctx.type === 'contexttoolbar'
            ? buildContextToolbarGroups(allButtons, ctx)
            : buildContextFormGroups(ctx, sharedBackstage.providers)
        )
      )
      return renderToolbar({
        type: toolbarType,
    ...
    const getAnchor = (position, element) => {
      const anchorage =
        position === 'node'
          ? sharedBackstage.anchors.node(element)
          : sharedBackstage.anchors.cursor()
  ...
  const isUnit = (unit, accepted) =>
    exists(accepted, (acc) => exists(units[acc], (check) => unit === check))
  ...
  const normalise = (input, accepted) =>
  ...
  const lineHeightSpec = {
    ...
    hash: (input) =>
      normalise(input, ['fixed', 'relative', 'empty']).getOr(input),
    display: identity,
    watcher: (editor, value, callback) =>
      editor.formatter.formatChanged('lineheight', callback, false, { value })
        .unbind,
    getCurrent: (editor) =>
      Optional.from(editor.queryCommandValue('LineHeight')),
    setCurrent: (editor, value) =>
  ...
  const languageSpec = (editor) => {
    ...
    return settingsOpt.map((settings) => ({
      ...
      hash: (input) =>
        isUndefined(input.customCode)
          ? input.code
          : `${input.code}/${input.customCode}`,
      ...
      watcher: (editor, value, callback) => {
        ...
        return editor.formatter.formatChanged('lang', callback, false, {
          value: value.code,
          customValue:
            (_a = value.customCode) !== null && _a !== void 0 ? _a : null
        }).unbind
      },
      getCurrent: (editor) => {
        const node = SugarElement.fromDom(editor.selection.getNode())
        return closest$4(node, (n) =>
          Optional.some(n)
            .filter(isElement$1)
            .bind((ele) => {
      ...
      onToolbarSetup: (api) => {
        ...
        unbinder.set(
          editor.formatter.formatChanged('lang', api.setActive, true)
        )
  ...
  const onSetupOutdentState = (editor) =>
    onSetupEvent(editor, 'NodeChange', (api) => {
  ...
  const onSetupUndoRedoState = (editor, type) =>
    onSetupEvent(
      editor,
  ...
  const onSetupVisualAidState = (editor) =>
    onSetupEvent(editor, 'VisualAid', (api) => {
  ...
  const patchPipeConfig = (config) =>
    isString(config) ? config.split(/[ ,]/) : config
  ...
  const isTouchWithinSelection = (editor, e) => {
    ...
    if (selection.isCollapsed() || e.touches.length < 1) {
      ...
      const rngRectOpt = getFirstRect(
        editor.getWin(),
        SimSelection.domRange(rng)
      )
        ...
        () =>
          Optional.some(
            getContextToolbarBounds(
              editor,
              backstage.shared,
              anchorType === 'node' ? 'node' : 'selection'
            )
          )
      )
  ...
  const initAndShow = (
    ...
    const shouldHighlightImmediately = () =>
      !(isAndroid || isiOS || (isMacOS && isTouch))
    const open = () => {
      const items = buildMenu()
      show(
        editor,
  ...
  const isSeparator = (item) =>
    isString(item) ? item === '|' : item.type === 'separator'
  ...
  const generateContextMenu = (contextMenus, menuConfig, selectedElement) => {
    const sections = foldl(
      menuConfig,
      (acc, name) => {
        return get$g(contextMenus, name.toLowerCase())
          .map((menu) => {
  ...
  const isNativeOverrideKeyEvent = (editor, e) =>
    e.ctrlKey && !shouldNeverUseNative(editor)
  const isTouchEvent = (e) => e.type === 'longpress' || has$2(e, 'touches')
  const isTriggeredByKeyboard = (editor, e) =>
    !isTouchEvent(e) &&
    (e.button !== 2 || (e.target === editor.getBody() && e.pointerType === ''))
  const getSelectedElement = (editor, e) =>
    isTriggeredByKeyboard(editor, e)
  ...
  const subtract = (change) => (point) =>
    point.translate(-change.left, -change.top)
  const add = (change) => (point) => point.translate(change.left, change.top)
  const transform = (changes) => (x, y) =>
    foldl(changes, (rest, f) => f(rest), SugarPosition(x, y))
  const asFixed = (coord, scroll, origin) =>
    coord.fold(
      transform([add(origin), subtract(scroll)]),
      transform([subtract(scroll)]),
      transform([])
    )
  const asAbsolute = (coord, scroll, origin) =>
    coord.fold(
      transform([add(origin)]),
      transform([]),
      transform([add(scroll)])
    )
  const asOffset = (coord, scroll, origin) =>
    coord.fold(
      transform([]),
  ...
  const translate = (coord, deltaX, deltaY) =>
    coord.fold(
  ...
  const get = (component, snapsInfo) => {
    ...
    return isNaN(x) || isNaN(y)
      ? Optional.none()
      : Optional.some(SugarPosition(x, y))
  }
  ...
  const getCoords = (component, snapInfo, coord, delta) =>
    get(component, snapInfo).fold(
      () => coord,
  ...
  const findMatchingSnap = (snaps, newCoord, scroll, origin) =>
    findMap(snaps, (snap) => {
  ...
  const resetZIndex = (blocker) => {
    parent(blocker.element)
      .filter(isElement$1)
      .each((root) => {
        getOpt(root, initialAttribute).fold(
  ...
  const createComponent = (component, blockerClass, blockerEvents) =>
  ...
  const getDelta$1 = (old, nu) =>
    SugarPosition(nu.left - old.left, nu.top - old.top)
  ...
  const events$2 = (dragConfig, dragState, updateStartState) => [
    run$1(mousedown(), (component, simulatedEvent) => {
      ...
      simulatedEvent.stop()
      const stop$1 = () =>
        stop(component, Optional.some(blocker), dragConfig, dragState)
  ...
  const calcSnap = (selectorOpt, td, x, y, width, height) =>
    selectorOpt.fold(
      () =>
        Dragging.snap({
      ...
      (selectorHandle) => {
        ...
        return Dragging.snap({
          ...
          output: absolute(
            Optional.some(x - rect.width / 2),
            Optional.some(y - rect.height / 2)
          ),
          extra: { td }
        })
      }
  ...
  const getSnapsConfig = (getSnapPoints, cell, onChange) => {
    const isSameCell = (cellOpt, td) =>
      cellOpt.exists((currentTd) => eq(currentTd, td))
    return {
      getSnapPoints,
  ...
  const setup$4 = (editor, sink) => {
    ...
    const showOrHideHandle = (selector, cell, isAbove, isBelow) => {
      ...
      const viewportHeight = defaultView(SugarElement.fromDom(editor.getBody()))
        .dom.innerHeight
    ...
    const snapBottomRight = (cell) =>
      snapTo(bottomRight, cell, getBottomRightSnap, 'bottom')
  ...
  const isHidden = (elm) =>
    elm.nodeName === 'BR' ||
    !!elm.getAttribute('data-mce-bogus') ||
    elm.getAttribute('data-mce-type') === 'bookmark'
  const renderElementPath = (editor, settings, providersBackstage) => {
    var _a
    const delimiter =
      (_a = settings.delimiter) !== null && _a !== void 0 ? _a : '\u203A'
    const renderElement = (name, element, index) =>
      Button.sketch({
        dom: {
          tag: 'div',
        ...
        config('elementPathEvents', [
          runOnAttached((comp, _e) => {
            ...
            editor.on('NodeChange', (e) => {
              ...
              const newChildren =
                newPath.length > 0 ? renderPathData(newPath) : []
  ...
  const getDimensions = (
    ...
    const dimensions = {
      height: calcCappedSize(
        ...
        getMinHeightOption(editor),
        getMaxHeightOption(editor)
      )
    }
    if (resizeType === ResizeTypes.Both) {
      dimensions.width = calcCappedSize(
        originalWidth + deltas.left,
        getMinWidthOption(editor),
        getMaxWidthOption(editor)
      )
    }
    return dimensions
  }
  ...
  const renderWordCount = (editor, providersBackstage) => {
    const replaceCountText = (comp, count, mode) =>
      Replacing.set(comp, [
        text$1(providersBackstage.translate(['{0} ' + mode, count[mode]]))
      ])
    return Button.sketch({
      dom: {
        tag: 'button',
  ...
  const getLazyMothership = (singleton) =>
  ...
  const setup$3 = (editor) => {
    ...
    const lazyHeader = () =>
      lazyOuterContainer.get().bind(OuterContainer.getHeader)
    const lazySinkResult = () =>
      Result.fromOption(lazySink.get(), 'UI has not been rendered')
    const lazyAnchorBar = () =>
      lazyOuterContainer
        .get()
    ...
    const makeHeaderPart = () => {
      ...
      const partToolbar = OuterContainer.parts.toolbar({
        ...
        lazyToolbar,
        lazyHeader: () =>
          lazyHeader().getOrDie('Could not find header element'),
    ...
    const renderSink = () => {
      const uiContainer = getUiContainer(editor)
      const isGridUiContainer =
        eq(body(), uiContainer) && get$e(uiContainer, 'display') === 'grid'
      const sinkSpec = {
        dom: {
          tag: 'div',
          classes: ['tox', 'tox-silver-sink', 'tox-tinymce-aux'].concat(
            deviceClasses
          ),
          attributes: { ...(global$8.isRtl() ? { dir: 'rtl' } : {}) }
        },
        behaviours: derive$1([
          Positioning.config({ useFixed: () => header.isDocked(lazyHeader) })
        ])
      }
      ...
      const sink = build$1(
        deepMerge(sinkSpec, isGridUiContainer ? reactiveWidthSpec : {})
      )
    ...
    const renderContainer = () => {
      ...
      const statusbar =
        useStatusBar(editor) && !isInline
          ? Optional.some(renderStatusbar(editor, backstage.shared.providers))
          : Optional.none()
    ...
    const setupShortcutsAndCommands = (outerContainer) => {
      ...
      editor.addQueryStateHandler('ToggleToolbarDrawer', () =>
        OuterContainer.isToolbarDrawerToggled(outerContainer)
      )
    }
    const renderUI = () => {
      ...
      const { buttons, menuItems, contextToolbars, sidebars } =
  ...
  const factory$4 = (detail, components, spec, externals) => {
    ...
    const eventOrder = {
      ...detail.eventOrder,
      [attachedToDom()]: [modalEventsId].concat(
  ...
  const dialogToggleMenuItemSchema = objOf(
    [type, name$1].concat(commonMenuItemFields)
  )
  ...
  const customEditorSchema = valueOf((v) =>
    asRaw('customeditor.old', objOfOnly(customEditorFieldsOld), v).orThunk(() =>
      asRaw('customeditor.new', objOfOnly(customEditorFields), v)
    )
  )
  ...
  const imagePreviewSchema = objOf(
    formComponentFields.concat([optionString('height')])
  )
  ...
  const listBoxNestedItemFields = [
    ...
    requiredArrayOf(
      'items',
      thunkOf('items', () => listBoxItemSchema)
    )
  ]
  ...
  const selectBoxFields = formComponentWithLabelFields.concat([
    requiredArrayOfObj('items', [text, value$1]),
  ...
  const createItemsField = (name) =>
    field$1(
      ...
      arrOf(
        valueOf((v) =>
          asRaw(`Checking item of ${name}`, itemSchema, v).fold(
            (sErr) => Result.error(formatError(sErr)),
            (passValue) => Result.value(passValue)
          )
        )
      )
    )
  const itemSchema = valueThunk(() =>
    choose$2('type', {
  ...
  const getNamedItems = (structure) =>
    filter$2(getAllObjects(structure), isNamedItem)

  const createDataValidator = (structure) => {
    const namedItems = getNamedItems(structure)
    const fields = bind$3(namedItems, (item) =>
      getDataProcessor(item).fold(
        () => [],
        (schema) => [requiredOf(item.name, schema)]
      )
    )
    return objOf(fields)
  }
  ...
  const extract = (structure) => {
    ...
    const initialData =
      (_a = structure.initialData) !== null && _a !== void 0 ? _a : {}
    return {
      internalDialog,
      dataValidator,
      initialData
  ...
  const renderBodyPanel = (spec, dialogData, backstage) => {
    const memForm = record(
      Form.sketch((parts) => ({
        ...
        components: map$2(spec.items, (item) =>
          interpretInForm(parts, item, dialogData, backstage)
        )
      }))
    )
    return {
  ...
  const measureHeights = (allTabs, tabview, tabviewComp) =>
    map$2(allTabs, (_tab, i) => {
  ...
  const getMaxTabviewHeight = (dialog, tabview, tablist) => {
    ...
    const isTabListBeside =
      tabview.dom.offsetLeft >= tablist.dom.offsetLeft + get$c(tablist)
    const currentTabHeight = isTabListBeside
      ? Math.max(get$d(tablist), tabviewHeight)
      : tabviewHeight
    ...
    const extraEvents = [
      ...
      run$1(formResizeEvent, (comp, _se) => {
        ...
        getTabview(dialog).each((tabview) => {
          ...
          set$8(tabview, 'visibility', 'hidden')
          const oldHeight = getRaw(tabview, 'height').map((h) =>
            parseInt(h, 10)
          )
        ...
        view: () => {
          return [
            Form.sketch((parts) => ({
              ...
              components: map$2(tab.items, (item) =>
                interpretInForm(parts, item, dialogData, backstage)
              ),
              formBehaviours: derive$1([
                Keying.config({
  ...
  const renderBody = (spec, dialogId, contentId, backstage, ariaAttrs) => {
    ...
    const updateState = (_comp, incoming) =>
      Optional.some({ isTabPanel: () => incoming.body.type === 'tabpanel' })
  ...
  const pClose = (onClose, providersBackstage) =>
  ...
  const pBodyMessage = (message, providersBackstage) =>
  ...
  const renderClose = (providersBackstage) =>
    Button.sketch({
      dom: {
        tag: 'button',
        classes: ['tox-button', 'tox-button--icon', 'tox-button--naked'],
  ...
  const renderInlineHeader = (spec, dialogId, titleId, providersBackstage) =>
    Container.sketch({
  ...
  const getHeader = (title, dialogId, backstage) =>
    renderModalHeader(
      {
  ...
  const getEventExtras = (lazyDialog, providers, extra) => ({
    ...
    onBlock: (blockEvent) => {
      ModalDialog.setBusy(lazyDialog(), (_comp, bs) =>
        getBusySpec(blockEvent.message, bs, providers)
      )
    },
    onUnblock: () => {
  ...
  const initUrlDialog = (getInstanceApi, extras) => {
    const fireApiEvent = (eventName, f) =>
      run$1(eventName, (c, se) => {
        withSpec(c, (spec, _c) => {
  ...
  const initDialog = (getInstanceApi, extras, getSink) => {
    ...
    return [
      ...
      fireApiEvent(formActionEvent, (api, spec, event, component) => {
        ...
        const isDisabled = (focused) =>
          has$1(focused, 'disabled') ||
          getOpt(focused, 'aria-disabled').exists((val) => val === 'true')
        ...
        active$1(rootNode).fold(focusIn, (focused) => {
          if (isDisabled(focused)) {
            ...
            current.exists((cur) => contains(focused, cur) && isDisabled(cur))
          ) {
            focusIn()
          } else {
            getSink()
  ...
  const makeButton = (button, backstage) =>
    renderFooterButton(button, button.type, backstage)
  const lookup = (compInSystem, footerButtons, buttonName) =>
    find$5(footerButtons, (button) => button.name === buttonName).bind(
      (memButton) => memButton.memento.getOpt(compInSystem)
    )
  const renderComponents = (_data, state) => {
    const footerButtons = state.map((s) => s.footerButtons).getOr([])
    const buttonGroups = partition$3(
      footerButtons,
      (button) => button.align === 'start'
    )
    const makeGroup = (edge, buttons) =>
      Container.sketch({
        dom: {
          tag: 'div',
  ...
  const renderInlineFooter = (initSpec, dialogId, backstage) =>
    renderFooter(initSpec, dialogId, backstage)
  const renderModalFooter = (initSpec, dialogId, backstage) =>
  ...
  const validateData$1 = (access, data) => {
    ...
    return Reflecting.getState(root)
      .get()
      .map((dialogState) =>
        getOrDie(asRaw('data', dialogState.dataValidator, data))
      )
      .getOr(data)
  }
  const getDialogApi = (access, doRedial, menuItemStates) => {
    ...
    const setEnabled = (name, state) => {
      getCompByName(access, name).each(
        state ? Disabling.enable : Disabling.disable
      )
    }
    const focus = (name) => {
    ...
    const redial = (d) => {
      withRoot((root) => {
        ...
        root
          .getSystem()
          .broadcastOn([`${titleChannel}-${id}`], dialogInit.internalDialog)
        root
          .getSystem()
          .broadcastOn([`${bodyChannel}-${id}`], dialogInit.internalDialog)
        root
          .getSystem()
          .broadcastOn([`${footerChannel}-${id}`], dialogInit.internalDialog)
  ...
  const renderInlineDialog = (dialogInit, extra, backstage, ariaAttrs) => {
    ...
    const memFooter = record(
      renderInlineFooter({ buttons: storagedMenuButtons }, dialogId, backstage)
    )
    const dialogEvents = SilverDialogEvents.initDialog(
      () => instanceApi,
      {
        onBlock: (event) => {
          Blocking.block(dialog, (_comp, bs) =>
            getBusySpec(event.message, bs, backstage.shared.providers)
          )
        },
        onUnblock: () => {
          Blocking.unblock(dialog)
  ...
  const isSupportedMessage = (data) =>
    isObject(data) && SUPPORTED_MESSAGE_ACTIONS.indexOf(data.mceAction) !== -1
  const isCustomMessage = (data) =>
    !isSupportedMessage(data) && isObject(data) && has$2(data, 'mceAction')
  ...
  const renderUrlDialog = (internalDialog, extra, editor, backstage) => {
    ...
    const footer = internalDialog.buttons.bind((buttons) => {
      if (buttons.length === 0) {
        ...
        return Optional.some(
          renderModalFooter({ buttons }, dialogId, backstage)
        )
      }
    })
  ...
  const setup$2 = (backstage) => {
    ...
    const open = (message, callback) => {
      ...
      const alertDialog = build$1(
        renderDialog$1({
          lazySink: () => sharedBackstage.getSink(),
          header: hiddenHeader(titleSpec, closeSpec),
          body: pBodyMessage(message, sharedBackstage.providers),
          footer: Optional.some(
  ...
  const setup$1 = (backstage) => {
    ...
    const open = (message, callback) => {
      ...
      const titleSpec = pUntitled()
      const closeSpec = pClose(
        () => closeDialog(false),
        sharedBackstage.providers
      )
      const confirmDialog = build$1(
        renderDialog$1({
          lazySink: () => sharedBackstage.getSink(),
          header: hiddenHeader(titleSpec, closeSpec),
          body: pBodyMessage(message, sharedBackstage.providers),
          footer: Optional.some(
            pFooter(pFooterGroup([], [footerNo, memFooterYes.asSpec()]))
          ),
  ...
  const validateData = (data, validator) =>
    getOrDie(asRaw('data', validator, data))
  const isAlertOrConfirmDialog = (target) =>
    closest(target, '.tox-alert-dialog') ||
    closest(target, '.tox-confirm-dialog')
      ...
      return [
        Docking.config({
          contextual: {
            lazyContext: () =>
              Optional.some(
                box$1(SugarElement.fromDom(editor.getContentAreaContainer()))
              ),
  ...
  const setup = (extras) => {
    ...
    const openUrl = (config, closeWindow) =>
    ...
    global$a.add('silver', (editor) => {
      ...
      return {
        ...
        getNotificationManagerImpl: () =>
          NotificationManagerImpl(editor, { backstage }, getUiMothership())
      }
    })
  }
