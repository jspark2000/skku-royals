...
;(function () {
  ...
  const hasNonNullableKey = (obj, key) =>
    has$1(obj, key) && obj[key] !== undefined && obj[key] !== null
  const isEmpty = (r) => {
    for (const x in r) {
  ...
  const all$1 = (selector, scope) => {
    ...
    return bypassSelector(base)
      ? []
      : map$1(base.querySelectorAll(selector), SugarElement.fromDom)
  }
  ...
  const children$2 = (element) =>
    map$1(element.dom.childNodes, SugarElement.fromDom)
  ...
  const isValidSection = (parentName) =>
    contains$2(validSectionList, parentName)
  ...
  const ancestors$4 = (scope, predicate, isRoot) =>
    filter$2(parents(scope, isRoot), predicate)
  const children$1 = (scope, predicate) =>
    filter$2(children$2(scope), predicate)
  ...
  const ancestors$3 = (scope, selector, isRoot) =>
    ancestors$4(scope, (e) => is$2(e, selector), isRoot)
  ...
  const ancestor$1 = (scope, selector, isRoot) =>
    ancestor$2(scope, (e) => is$2(e, selector), isRoot)
  ...
  const bindFrom = (a, f) =>
    a !== undefined && a !== null ? f(a) : Optional.none()
  ...
  const getCssValue = (element, property) =>
    parseInt(get$a(element, property), 10)
  ...
  const columns$1 = (ancestor) => {
    if (is$2(ancestor, 'colgroup')) {
      ...
      return bind$2(columnGroups(ancestor), (columnGroup) =>
        children(columnGroup, 'col')
      )
    }
  }
  ...
  const columnGroups = (ancestor) =>
    table(ancestor).fold(constant([]), (table) => children(table, 'colgroup'))
  ...
  const fromPastedRows = (elems, section) =>
  ...
  const getProp = (element, name, fallback) =>
    toNumber(get$a(element, name), fallback)
  ...
  const setCells = (gridRow, cells) =>
    rowcells(gridRow.element, cells, gridRow.section, gridRow.isNew)
  ...
  const generate$1 = (list) => {
    ...
    let maxColumns = 0
    let rowCount = 0
    const { pass: colgroupRows, fail: rows } = partition(
      list,
    ...
    const { columns, colgroups } = last$2(colgroupRows)
      .map((rowData) => {
  ...
  const columns = (warehouse, isValidCell = always) => {
    ...
    return map$1(cols, (col) => {
      ...
      const isValid = (detail) =>
        detail.colspan === 1 && isValidCell(detail.element)
  ...
  const decide = (getBlock, isValid, getFallback) => {
    ...
    const detailOption = validInBlock.orThunk(() =>
      Optional.from(inBlock[0]).orThunk(getFallback)
    )
  ...
  const rows = (warehouse) => {
    ...
    return map$1(rowsArr, (row) => {
      const getBlock = () =>
        bind$2(cols, (c) =>
          Warehouse.getAt(warehouse, row, c)
  ...
  const getDirection = (element) =>
    get$a(element, 'direction') === 'rtl' ? 'rtl' : 'ltr'
  ...
  const getPercentSize = (elm, outerGetter, innerGetter) => {
    const relativeParent = parentElement(elm).getOrThunk(() =>
      getBody$1(owner(elm))
    )
  ...
  const getPixelWidth$1 = (cell) =>
    isCol$2(cell) ? get$9(cell) : getRuntime$1(cell)
  ...
  const getPercentTableWidth = (table) =>
    getPercentSize(table, get$9, getInner) + '%'
  const isPercentSizing$1 = (table) =>
    getRawWidth$1(table).exists((size) => rPercentageBasedSizeRegex.test(size))
  const isPixelSizing$1 = (table) =>
    getRawWidth$1(table).exists((size) => rPixelBasedSizeRegex.test(size))
  ...
  const justCols = (warehouse) =>
    map$1(Warehouse.justColumns(warehouse), (column) =>
  ...
  const getDimension = (cellOpt, index, backups, filter, getter, fallback) =>
    cellOpt.filter(filter).fold(
      () => fallback(deduce(backups, index)),
      (cell) => getter(cell)
    )
  const getWidthFrom = (warehouse, table, getWidth, fallback) => {
    const columnCells = columns(warehouse)
    const columns$1 = Warehouse.hasColumns(warehouse)
      ? justCols(warehouse)
      : columnCells
    const backups = [Optional.some(width.edge(table))].concat(
      map$1(width.positions(columnCells, table), (pos) => pos.map((p) => p.x))
    )
        ...
        (column) => {
          if (isValidColumn(column)) {
            ...
            return getDimension(
              ...
              backups,
              colFilter,
              (cell) => fallback(Optional.some(get$9(cell))),
              fallback
            )
          }
        },
  ...
  const getHeightFrom = (warehouse, table, direction, getHeight, fallback) => {
    ...
    const backups = [Optional.some(direction.edge(table))].concat(
      map$1(direction.positions(rows$1, table), (pos) => pos.map((p) => p.y))
    )
  ...
  const noneSize = (table) => {
    ...
    const getWidths = (warehouse, tableSize) =>
      getPixelWidths(warehouse, table, tableSize)
    return {
      width: getWidth,
      pixelWidth: getWidth,
      getWidths,
  ...
  const percentageSize = (table) => {
    const getFloatWidth = widthLookup(table, (elem) =>
      parseFloat(getPercentTableWidth(elem))
    )
    const getWidth = widthLookup(table, get$9)
    ...
    const getWidths = (warehouse, tableSize) =>
      getPercentageWidths(warehouse, table, tableSize)
    return {
      width: getFloatWidth,
      pixelWidth: getWidth,
      getWidths,
  ...
  const clean = (replica, stats, house, widthDelta) => {
    ...
    const emptyRows = filter$2(
      firstLayer(replica, 'tr'),
      (row) => row.dom.childElementCount === 0
    )
  ...
  const getTableWidthDelta = (table, warehouse, tableSize, stats) => {
    ...
    const selectedColsWidth = foldl(
      colWidths.slice(stats.minCol, stats.maxCol + 1),
      (acc, width) => acc + width,
      0
    )
  ...
  const extract$1 = (table, selectedSelector) => {
    ...
    const unselectedCells = filterFirstLayer(replica, 'th,td', (cell) =>
      is$2(cell, selector)
    )
  ...
  const isCursorPosition = (elem) => {
    ...
    return (
      hasCursorPosition || contains$2(elementsWithCursorPosition, name(elem))
    )
  }
  ...
  const cloneAppropriateAttributes = (original, clone) => {
    each$1(transferableAttributes, (validAttributes, attributeName) =>
      getOpt(original, attributeName)
  ...
  const removeDataStyle = (table) => {
    ...
    const removeStyleAttribute = (element) =>
      remove$7(element, 'data-mce-style')
  ...
  const intercepts$1 = (warehouse, start, finish) => {
    return getAnyBox(warehouse, start, finish).map((bounds) => {
      const inside = Warehouse.filterItems(
        warehouse,
        curry(inSelection, bounds)
      )
  ...
  const nestedIntercepts = (table, first, firstTable, last, lastTable) => {
    const warehouse = getWarehouse(table)
    const optStartCell = eq$1(table, firstTable)
      ? Optional.some(first)
      : parentCell(warehouse, first)
    const optLastCell = eq$1(table, lastTable)
      ? Optional.some(last)
      : parentCell(warehouse, last)
    return optStartCell.bind((startCell) =>
      optLastCell.bind((lastCell) =>
        intercepts$1(warehouse, startCell, lastCell)
      )
    )
  }
  ...
  var DomUniverse = () => {
    ...
    const isNonEditable = (element) =>
      isElement(element) && get$b(element, 'contenteditable') === 'false'
      ...
      return lookupTable(start).bind((startTable) => {
        return lookupTable(finish).bind((finishTable) => {
          if (eq$1(startTable, finishTable)) {
            ...
            const ancestorCells = ancestors$3(
              finish,
              'td,th',
            ...
            return ancestors(start, finish).shared.bind((lca) => {
              return closest$1(lca, 'table', isRoot).bind((lcaTable) => {
                const finishAncestorCells = ancestors$3(
                  finish,
                  'td,th',
                  getIsRoot(lcaTable)
                )
                const finishCell =
                  finishAncestorCells.length > 0
                    ...
                    : finish
                const startAncestorCells = ancestors$3(
                  start,
                  'td,th',
                  getIsRoot(lcaTable)
                )
  ...
  const getCellsFromFakeSelection = (editor) =>
    filter$2(getCellsFromSelection(editor), (cell) =>
      is$2(cell, ephemera.selectedSelector)
    )
  ...
  const serializeElements = (editor, elements) =>
    map$1(elements, (elm) =>
      editor.selection.serializer.serialize(elm.dom, {})
    ).join('')
  const getTextContent = (elements) =>
    map$1(elements, (element) => element.dom.innerText).join('')
  ...
  const freefallRtl$2 = (universe, element) => {
    const candidate = scan$1(
      universe,
      element,
      universe.query().prevSibling
    ).getOr(element)
    ...
    return children.length > 0
      ? freefallRtl$2(universe, children[children.length - 1])
  ...
  const surround = (sizes, startIndex, endIndex, results, f) =>
    f(sizes.slice(0, startIndex))
      .concat(results)
      .concat(f(sizes.slice(endIndex)))
  const clampDeltaHelper =
    (predicate) => (sizes, index, delta, minCellSize) => {
      if (!predicate(delta)) {
        return delta
      } else {
  ...
  const resizeTable = () => {
    ...
    const calcRightEdgeDeltas = (
      ...
      if (isRelative) {
        ...
        const clampedDelta = clampNegativeDelta(
          sizes,
          index,
          delta,
          minCellSize
  ...
  const preserveTable = () => {
    ...
    const calcMiddleDeltas = (sizes, _prev, index, next, delta, minCellSize) =>
  ...
  const isHeaderCells = (cells) =>
    forall(cells, (cell) => isHeaderCell(cell.element))
  ...
  const findTableRowHeaderType = (warehouse) =>
    findMap(warehouse.all, (row) => {
      const rowType = getRowType(row)
      return rowType.type === 'header'
        ? Optional.from(rowType.subType)
        : Optional.none()
    })

  const transformCell = (cell, comparator, substitution) =>
    elementnew(substitution(cell.element, comparator), true, cell.isLocked)
  const transformRow = (row, section) =>
    row.section !== section
      ? rowcells(row.element, row.cells, section, row.isNew)
      : row
  const section = () => ({
    transformRow,
    transformCell: (cell, comparator, substitution) => {
      ...
      const fixedCell =
        name(newCell) !== 'td' ? mutate$1(newCell, 'td') : newCell
  ...
  const insert$1 = (table, selector, element) => {
    last$2(children(table, selector)).fold(
      () => prepend(table, element),
  ...
  const render$1 = (table, grid) => {
    const newRows = []
    const newCells = []
    const syncRows = (gridSection) =>
      map$1(gridSection, (row) => {
        if (row.isNew) {
    ...
    const syncColGroup = (gridSection) =>
      bind$2(gridSection, (colGroup) =>
        map$1(colGroup.cells, (col) => {
  ...
  const subgrid = (grid, row, column, comparator) => {
    ...
    const rowspan = isColRow
      ? 1
      : findDiff(getColumn(grid.slice(row), column), comparator)
    return {
      colspan,
      rowspan
    }
  ...
  const toGrid = (warehouse, generators, isNew) => {
    ...
    for (let rowIndex = 0; rowIndex < warehouse.grid.rows; rowIndex++) {
      ...
      for (
        ...
        const element = Warehouse.getAt(warehouse, rowIndex, columnIndex)
          .map((item) => elementnew(item.element, isNew, item.isLocked))
  ...
  const fromWarehouse = (warehouse, generators) =>
    toGrid(warehouse, generators, false)
  const toDetailList = (grid) => toDetails(grid, eq$1)
  const findInWarehouse = (warehouse, element) =>
    findMap(warehouse.all, (r) =>
      find$1(r.cells, (e) => eq$1(element, e.element))
    )
  ...
  const run =
    (operation, extract, adjustment, postAction, genWrappers) =>
    (table, target, generators, behaviours) => {
      const warehouse = Warehouse.fromTable(table)
      const tableSection = Optional.from(
        behaviours === null || behaviours === void 0
          ? void 0
          : behaviours.section
      ).getOrThunk(TableSection.fallback)
      ...
      return output.bind((out) => {
        ...
        const tableSizing = Optional.from(
          behaviours === null || behaviours === void 0
            ? void 0
            : behaviours.sizing
        ).getOrThunk(() => TableSize.getTableSize(table))
        const resizing = Optional.from(
          behaviours === null || behaviours === void 0
            ? void 0
            : behaviours.resize
        ).getOrThunk(preserveTable)
  ...
  const onPaste = (warehouse, target) =>
    cell(target.element).bind((cell) =>
  ...
  const onPasteByEditor = (warehouse, target) =>
    extractCells(warehouse, target, always).map((cells) => ({
      cells,
      generators: target.generators,
      clipboard: target.clipboard
    }))
  ...
  const onUnlockedCells = (warehouse, target) =>
      ...
      for (let j = 0; j < cellLength(rows[0]); j++) {
        ...
        if (isToReplace && !first) {
          mutateCell(
            row,
            j,
            elementnew(substitution(), true, currentCell.isLocked)
          )
        } else if (isToReplace) {
          first = false
        }
  ...
  const splitCols = (grid, index, comparator, substitution) => {
    if (index > 0 && index < grid[0].cells.length) {
      each$2(grid, (row) => {
        ...
        while (
          row.cells.length > index + offset &&
          comparator(prevCell.element, row.cells[index + offset].element)
        ) {
          mutateCell(
            row,
            index + offset,
            elementnew(substitute, true, row.cells[index + offset].isLocked)
          )
          offset++
        }
      })
  ...
  const generateElements = (amount, row, generators, isLocked) => {
    ...
    return range$1(amount, (idx) =>
      elementnew(generator(), true, isLocked(idx))
    )
  }
    ...
    return grid.concat(
      range$1(amount, () => {
        ...
        const elements = generateElements(
          ...
          row,
          generators,
          (idx) => has$1(lockedColumns, idx.toString())
        )
        return setCells(row, elements)
      })
    )
  }
  const colFill = (grid, amount, generators, startIndex) =>
    map$1(grid, (row) => {
      const newChildren = generateElements(amount, row, generators, never)
      return addCells(row, startIndex, newChildren)
    })
  const lockedColFill = (grid, generators, lockedColumns) =>
    map$1(grid, (row) => {
      return foldl(
        lockedColumns,
        (acc, colNum) => {
  ...
  const tailor = (gridA, delta, generators) => {
    ...
    const isLastColLocked = exists(
      lockedColumns,
      (locked) => locked === gridWidth - 1
    )
    const modifiedCols = fillCols(
      gridA,
      Math.abs(delta.colDelta),
      generators,
      isLastColLocked ? gridWidth - 1 : gridWidth
    )
    const newLockedColumns = getLockedColumnsFromGrid(modifiedCols)
    return fillRows(
      modifiedCols,
      Math.abs(delta.rowDelta),
      generators,
      mapToObject(newLockedColumns, always)
    )
  }
  ...
  const getValidStartAddress = (currentStartAddress, grid, lockedColumns) => {
    ...
    const adjustedRowAddress =
      extractGridDetails(grid).cols.length + currentStartAddress.row
    const possibleColAddresses = range$1(
      gridColLength - currentStartAddress.column,
      (num) => num + currentStartAddress.column
    )
    const validColAddress = find$1(possibleColAddresses, (num) =>
      forall(lockedColumns, (col) => col !== num)
  ...
  const getLockedColumnsWithinBounds = (startAddress, rows, lockedColumns) =>
    filter$2(
      lockedColumns,
      (colNum) =>
        colNum >= startAddress.column &&
        colNum <= cellLength(rows[0]) + startAddress.column
    )
  const merge$1 = (startAddress, gridA, gridB, generator, comparator) => {
    const lockedColumns = getLockedColumnsFromGrid(gridA)
    const validStartAddress = getValidStartAddress(
      startAddress,
      gridA,
      lockedColumns
    )
    const gridBRows = extractGridDetails(gridB).rows
    const lockedColumnsWithinBounds = getLockedColumnsWithinBounds(
      validStartAddress,
      gridBRows,
      lockedColumns
    )
    ...
    return result.map((diff) => {
      ...
      const newLockedColumnsWithinBounds = getLockedColumnsWithinBounds(
        validStartAddress,
        gridBRows,
        newLockedColumns
      )
  ...
  const cloneRow = (row, cloneCell, comparator, substitution) =>
    clone(row, (elem) => substitution(elem, comparator), cloneCell)
    ...
    const newRow = cloneRow(
      ...
      (ex, c) => {
        ...
        const ret = withinSpan
          ...
          : elementnew(substitution(ex.element, comparator), true, ex.isLocked)
        return ret
      },
      comparator,
      substitution
  ...
  const insertColumnAt = (grid, index, example, comparator, substitution) =>
    map$1(grid, (row) => {
      const withinSpan =
        index > 0 &&
        index < cellLength(row) &&
        comparator(getCellElement(row, index - 1), getCellElement(row, index))
      const sub = getElementFor(
        row,
        index,
        row.section,
        withinSpan,
  ...
  const deleteColumnsAt = (grid, columns) =>
    bind$2(grid, (row) => {
      const existingCells = row.cells
      const cells = foldr(
        columns,
        (acc, column) =>
          column >= 0 && column < acc.length
            ? acc.slice(0, column).concat(acc.slice(column + 1))
            : acc,
        existingCells
      )
  ...
  const notInStartColumn = (row, index, comparator) =>
    index > 0 &&
    comparator(getCellElement(row, index - 1), getCellElement(row, index))
  const isDuplicatedCell = (grid, rowIndex, colIndex, comparator) =>
    notInStartRow(grid, rowIndex, colIndex, comparator) ||
    notInStartColumn(grid[rowIndex], colIndex, comparator)
  const rowReplacerPredicate = (targetRow, columnHeaders) => {
    const entireTableIsHeader =
      forall(columnHeaders, identity) && isHeaderCells(targetRow.cells)
    return entireTableIsHeader
      ? always
      : (cell, _rowIndex, colIndex) => {
  ...
  const columnReplacePredicate = (targetColumn, rowHeaders) => {
    const entireTableIsHeader =
      forall(rowHeaders, identity) && isHeaderCells(targetColumn)
    return entireTableIsHeader
      ? always
      : (cell, rowIndex, _colIndex) => {
          const type = name(cell.element)
  ...
  const determineScope = (applyScope, cell, newScope, isInHeader) => {
    const hasSpan = (scope) =>
      scope === 'row' ? hasRowspan(cell) : hasColspan(cell)
  ...
  const rowScopeGenerator =
    (applyScope, columnHeaders) => (cell, rowIndex, columnIndex) =>
      Optional.some(
        determineScope(
          applyScope,
          cell.element,
  ...
  const columnScopeGenerator = (applyScope, rowHeaders) => (cell, rowIndex) =>
    Optional.some(
      determineScope(applyScope, cell.element, 'row', rowHeaders[rowIndex])
    )
  const replace = (cell, comparator, substitute) =>
    elementnew(substitute(cell.element, comparator), true, cell.isLocked)
  const replaceIn = (
    grid,
    targets,
    comparator,
    ...
    return map$1(grid, (row, rowIndex) => {
      return mapCells(row, (cell, colIndex) => {
        if (isTarget(cell)) {
          const newCell = shouldReplace(cell, rowIndex, colIndex)
            ? replacer(cell, comparator, substitute)
            : cell
  ...
  const replaceColumns = (
    ...
    const rows = extractGridDetails(grid).rows
    const targets = bind$2(indexes, (index) =>
      getColumnCells(rows, index, comparator)
    )
  ...
  const replaceRows = (
    ...
    const targets = bind$2(indexes, (index) =>
      getRowCells(rows, index, comparator)
    )
    const columnHeaders = map$1(targetRow.cells, (_cell, index) =>
      isHeaderCells(getColumnCells(rows, index, comparator))
    )
    const newRows = [...rows]
  ...
  const replaceCells = (grid, details, comparator, substitution) => {
    const rows = extractGridDetails(grid).rows
    const targetCells = map$1(details, (detail) =>
      getCell(rows[detail.row], detail.column)
    )
    return replaceIn(
      grid,
      targetCells,
  ...
  const determine = (input, column, step, tableSize, resize) => {
    ...
    const onLeft = (index, next) =>
      resize.calcLeftEdgeDeltas(
        result,
        index,
        next,
        step,
    ...
    const onMiddle = (prev, index, next) =>
      resize.calcMiddleDeltas(
        result,
        prev,
        index,
        next,
    ...
    const onRight = (prev, index) =>
      resize.calcRightEdgeDeltas(
        result,
        prev,
        index,
        step,
  ...
  const adjustWidth = (table, delta, index, resizing, tableSize) => {
    ...
    const clampedStep = resizing.clampTableDelta(
      widths,
      index,
      step,
      tableSize.minCellWidth(),
  ...
  const adjustHeight = (table, delta, index, direction) => {
    ...
    const newHeights = map$1(heights, (dy, i) =>
      index === i ? Math.max(delta + dy, minHeight()) : dy
    )
  ...
  const adjustAndRedistributeWidths$1 = (
    _table,
    list,
    details,
    tableSize,
    ...
    const { newSizes, delta } = resizeBehaviour.calcRedestributedWidths(
      sizes,
      tablePixelWidth,
      details.pixelDelta,
      tableSize.isRelative
  ...
  const uniqueColumns = (details) => {
    const uniqueCheck = (rest, detail) => {
      const columnExists = exists(
        rest,
        (currentDetail) => currentDetail.column === detail.column
      )
      return columnExists ? rest : rest.concat([detail])
    }
    return foldl(details, uniqueCheck, []).sort(
      (detailA, detailB) => detailA.column - detailB.column
    )
  }
  ...
  const modification = (generators, toData = elementToData) => {
    ...
    const nuRow = (data) =>
      isColgroup(data.element)
        ? generators.colgroup(data)
        : generators.row(data)
    const add = (element) => {
      if (isRow$1(element)) {
  ...
  const getScopeAttribute = (cell) =>
    getOpt(cell, 'scope').map((attribute) => attribute.substr(0, 3))
        ...
        .bind((rightEdge) => {
          ...
          return parent(rightEdge).map((parent) => {
            return rightSiblingIsBlock === true ||
              isListItem(parent) ||
              isBr(rightEdge) ||
              (isBlock(parent) && !eq$1(cell, parent))
              ? []
  ...
  const findEditableCursorPosition = (rows) =>
    findMap(rows, (row) =>
      findMap(row.cells, (cell) => {
  ...
  const elementFromGrid = (grid, row, column) => {
    ...
    return Optional.from(
      (_b =
        (_a = rows[row]) === null || _a === void 0
          ? void 0
          : _a.cells[column]) === null || _b === void 0
        ? void 0
        : _b.element
    )
      .filter(isEditable)
  ...
  const uniqueRows = (details) => {
    const rowCompilation = (rest, detail) => {
      const rowExists = exists(
        rest,
        (currentDetail) => currentDetail.row === detail.row
      )
      return rowExists ? rest : rest.concat([detail])
    }
    return foldl(details, rowCompilation, []).sort(
      (detailA, detailB) => detailA.row - detailB.row
    )
  }
  ...
  const opInsertColumnsBefore = (
    ...
    const newGrid = foldr(
      columns,
      (acc, col) => {
        const newG = insertColumnAt(
          acc.grid,
          targetIndex,
          col.column + acc.delta,
          comparator,
          genWrappers.getOrInit
        )
        return {
  ...
  const makeRowsSection =
    (section, applyScope) =>
    (initialGrid, details, comparator, genWrappers, tableSection) => {
      const rows = uniqueRows(details)
      const rowIndexes = map$1(rows, (detail) => detail.row)
      const newGrid = replaceRows(
        initialGrid,
        rowIndexes,
        section,
        applyScope,
  ...
  const opEraseColumns = (grid, extractDetail, _comparator, _genWrappers) => {
    ...
    const newGrid = deleteColumnsAt(
      grid,
      map$1(columns, (column) => column.column)
    )
    const maxColIndex = newGrid.length > 0 ? newGrid[0].cells.length - 1 : 0
    return bundle(
      newGrid,
      columns[0].row,
      Math.min(columns[0].column, maxColIndex)
    )
  }
  const opEraseRows = (grid, details, _comparator, _genWrappers) => {
    ...
    return bundle(
      newGrid,
      Math.min(details[0].row, maxRowIndex),
      details[0].column
    )
  }
  const opMergeCells = (grid, mergable, comparator, genWrappers) => {
    ...
    merge(cells)
    const newGrid = merge$2(
      grid,
      mergable.bounds,
      comparator,
      genWrappers.merge(cells)
    )
    return outcome(newGrid, Optional.from(cells[0]))
  }
  const opUnmergeCells = (grid, unmergable, comparator, genWrappers) => {
    const unmerge$1 = (b, cell) =>
      unmerge(b, cell, comparator, genWrappers.unmerge(cell))
  ...
  const opPasteColsBefore = (grid, pasteDetails, comparator, _genWrappers) => {
    ...
    const gridB = gridifyRows(
      pasteDetails.clipboard,
      pasteDetails.generators,
      context
    )
    const mergedGrid = insertCols(
      index,
      grid,
      gridB,
      pasteDetails.generators,
  ...
  const opGetColumnsType = (table, target) => {
    ...
    return details
      .bind((selectedCells) => {
        ...
        const selectedColumnCells = flatten(
          map$1(house.all, (row) =>
            filter$2(
              row.cells,
              (cell) => cell.column >= minColRange && cell.column < maxColRange
            )
          )
        )
  ...
  const resize = (table, list, details, behaviours) =>
    adjustWidthTo(table, list, details, behaviours.sizing)
  const adjustAndRedistributeWidths = (table, list, details, behaviours) =>
    adjustAndRedistributeWidths$1(
      table,
      list,
      details,
      behaviours.sizing,
  ...
  const firstColumnIsLocked = (_warehouse, details) =>
    exists(details, (detail) => detail.column === 0 && detail.isLocked)
  const lastColumnIsLocked = (warehouse, details) =>
    exists(
      details,
      (detail) =>
        detail.column + detail.colspan >= warehouse.grid.columns &&
        detail.isLocked
    )
  ...
  const insertColumnsExtractor = (before) => (warehouse, target) =>
    onCells(warehouse, target)
      .filter((details) => {
  ...
  const eraseColumnsExtractor = (warehouse, target) =>
    onUnlockedCells(warehouse, target).map((details) => ({
      details,
      pixelDelta: -getColumnsWidth(warehouse, details)
    }))
  const pasteColumnsExtractor = (before) => (warehouse, target) =>
    onPasteByEditor(warehouse, target).filter((details) => {
      const checkLocked = before ? firstColumnIsLocked : lastColumnIsLocked
      return !checkLocked(warehouse, details.cells)
    })
  const headerCellGenerator = Generators.transform('th')
  const bodyCellGenerator = Generators.transform('td')
  const insertRowsBefore = run(
    opInsertRowsBefore,
    onCells,
    noop,
    noop,
  ...
  const insertColumnsBefore = run(
    opInsertColumnsBefore,
    insertColumnsExtractor(true),
    adjustAndRedistributeWidths,
    noop,
    Generators.modification
  )
  const insertColumnsAfter = run(
    opInsertColumnsAfter,
    insertColumnsExtractor(false),
    adjustAndRedistributeWidths,
    noop,
    Generators.modification
  )
  const eraseColumns = run(
    opEraseColumns,
    eraseColumnsExtractor,
    adjustAndRedistributeWidths,
    prune,
    Generators.modification
  )
  ...
  const pasteColsBefore = run(
    opPasteColsBefore,
    pasteColumnsExtractor(true),
    noop,
    noop,
    Generators.modification
  )
  const pasteColsAfter = run(
    opPasteColsAfter,
    pasteColumnsExtractor(false),
    noop,
    noop,
    Generators.modification
  )
  const pasteRowsBefore = run(
    opPasteRowsBefore,
    onPasteByEditor,
    noop,
    noop,
  ...
  const getTableDefaultAttributes = (editor) => {
    ...
    return options.isSet('table_default_attributes')
      ? defaultAttributes
      : determineDefaultTableAttributes(editor, defaultAttributes)
  }
  const getTableDefaultStyles = (editor) => {
    ...
    return options.isSet('table_default_styles')
      ? defaultStyles
      : determineDefaultTableStyles(editor, defaultStyles)
  }
  ...
  const TableActions = (editor, resizeHandler, cellSelectionHandler) => {
    ...
    const lastRowGuard = (table) =>
      !isTableBody(editor) || getGridSize(table).rows > 1
    const lastColumnGuard = (table) =>
      !isTableBody(editor) || getGridSize(table).columns > 1
    ...
    const setSelectionFromAction = (table, result) =>
      result.cursor.fold(
        () => {
          const cells = cells$1(table)
          return head(cells)
    ...
    const execute =
      (operation, guard, mutate, effect) =>
      (table, target, noEvents = false) => {
        removeDataStyle(table)
        const doc = SugarElement.fromDom(editor.getDoc())
        const generators = cellOperations(mutate, doc, cloneFormats)
        const behaviours = {
          sizing: get$5(editor, table),
          resize: isResizeTableColumnResizing(editor)
            ? resizeTable()
            : preserveTable(),
          section: getTableSectionType(table)
        }
        return guard(table)
          ? operation(table, target, generators, behaviours).bind((result) => {
    ...
    const insertRowsBefore$1 = execute(
      insertRowsBefore,
      always,
      noop,
      structureModified
    )
    const insertRowsAfter$1 = execute(
      insertRowsAfter,
      always,
      noop,
      structureModified
    )
    const insertColumnsBefore$1 = execute(
      insertColumnsBefore,
      always,
      colMutationOp,
      structureModified
    )
    const insertColumnsAfter$1 = execute(
      insertColumnsAfter,
      always,
      colMutationOp,
      structureModified
    ...
    const unmergeCells$1 = execute(
      unmergeCells,
      always,
      noop,
      structureModified
    )
    const pasteColsBefore$1 = execute(
      pasteColsBefore,
      always,
      noop,
      structureModified
    )
    const pasteColsAfter$1 = execute(
      pasteColsAfter,
      always,
      noop,
      structureModified
    )
    const pasteRowsBefore$1 = execute(
      pasteRowsBefore,
      always,
      noop,
      structureModified
    )
    const pasteRowsAfter$1 = execute(
      pasteRowsAfter,
      always,
      noop,
      structureModified
    )
    const pasteCells$1 = execute(
      pasteCells,
      always,
      noop,
      styleAndStructureModified
    )
    const makeCellsHeader$1 = execute(
      makeCellsHeader,
      always,
      noop,
      structureModified
    )
    const unmakeCellsHeader$1 = execute(
      unmakeCellsHeader,
      always,
      noop,
      structureModified
    )
    const makeColumnsHeader$1 = execute(
      makeColumnsHeader,
      always,
      noop,
      structureModified
    )
    const unmakeColumnsHeader$1 = execute(
      unmakeColumnsHeader,
      always,
      noop,
      structureModified
    )
    const makeRowsHeader$1 = execute(
      makeRowsHeader,
      always,
      noop,
      structureModified
    )
    const makeRowsBody$1 = execute(
      makeRowsBody,
      always,
      noop,
      structureModified
    )
    const makeRowsFooter$1 = execute(
      makeRowsFooter,
      always,
      noop,
      structureModified
  ...
  const generateColGroup = (house, minColRange, maxColRange) => {
    if (Warehouse.hasColumns(house)) {
      const colsToCopy = filter$2(
        Warehouse.justColumns(house),
        isColInRange(minColRange, maxColRange)
      )
  ...
  const generateRows = (house, minColRange, maxColRange) =>
    map$1(house.all, (row) => {
      const cellsToCopy = filter$2(
        row.cells,
        isColInRange(minColRange, maxColRange)
      )
  ...
  const normalize = (values) => {
    ...
    return r
      .slice(0, r.length - 1)
      .concat([add$3(r[r.length - 1], Math.round(scan.remainder))])
  }
  ...
  const createRows = (rows, columns, rowHeaders, columnHeaders) =>
    range$1(rows, (r) => createRow(columns, rowHeaders, columnHeaders, r))
  const render = (
    rows,
    columns,
    rowHeaders,
    ...
    if (rowHeadersGoInThead && rowHeaders > 0) {
      ...
      const theadRowHeaders =
        headerType === 'sectionCells' ? actualRowHeaders : 0
      const theadRows = createRows(
        rowHeaders,
        columns,
        theadRowHeaders,
        columnHeaders
  ...
  const insert = (editor, columns, rows, colHeaders, rowHeaders) => {
    ...
    return descendant(getBody(editor), 'table[data-mce-id="__mce"]')
      .map((table) => {
        if (isTablePixelsForced(editor)) {
          ...
          isTablePercentagesForced(editor) ||
          isPercentage(defaultStyles.width)
        ) {
  ...
  const setColumns = (columnsOpt) => {
    columnsOpt.fold(clearColumns, (columns) =>
      setData({ [tableTypeColumn]: columns })
    )
  }
  const getColumns = () => getData(tableTypeColumn)
  const clearColumns = () => clearData(tableTypeColumn)

  const getSelectionStartCellOrCaption = (editor) =>
    getSelectionCellOrCaption(getSelectionStart(editor), getIsRoot(editor))
  const getSelectionStartCell = (editor) =>
    getSelectionCell(getSelectionStart(editor), getIsRoot(editor))
  const registerCommands = (editor, actions) => {
    ...
    const setSizingMode = (sizing) =>
      getSelectionStartCellOrCaption(editor).each((cellOrCaption) => {
        const isForcedSizing =
          isTableResponsiveForced(editor) ||
          isTablePixelsForced(editor) ||
          isTablePercentagesForced(editor)
        if (!isForcedSizing) {
    ...
    const performActionOnSelection = (action) =>
    ...
    const toggleTableCellClass = (_ui, clazz) => {
      performActionOnSelection((table) => {
        const selectedCells = getCellsFromSelection(editor)
        const allHaveClass = forall(selectedCells, (cell) =>
          editor.formatter.match('tablecellclass', { value: clazz }, cell.dom)
        )
        const formatterAction = allHaveClass
          ? editor.formatter.remove
          : editor.formatter.apply
        each$2(selectedCells, (cell) =>
    ...
    const actOnSelection = (execute, noEvents = false) =>
    ...
    const copyRowSelection = () =>
      performActionOnSelection((table, startCell) => {
        const targets = forMenu(getCellsFromSelection(editor), table, startCell)
        const generators = cellOperations(
          noop,
    ...
    const actOnType = (getAction) => (_ui, args) =>
      get$c(args, 'type').each((type) => {
    ...
    each$1(
      {
        ...
        mceTableCopyCol: () =>
          copyColSelection().each((selection) => setColumns(selection)),
        mceTableCopyRow: () =>
          copyRowSelection().each((selection) => setRows(selection)),
        mceTablePasteColBefore: () =>
          pasteOnSelection(actions.pasteColsBefore, getColumns),
        mceTablePasteColAfter: () =>
          pasteOnSelection(actions.pasteColsAfter, getColumns),
        mceTablePasteRowBefore: () =>
          pasteOnSelection(actions.pasteRowsBefore, getRows),
        mceTablePasteRowAfter: () =>
          pasteOnSelection(actions.pasteRowsAfter, getRows),
        mceTableDelete: eraseTable,
        mceTableCellToggleClass: toggleTableCellClass,
        mceTableToggleClass: toggleTableClass,
        mceTableToggleCaption: toggleCaption,
        mceTableSizingMode: (_ui, sizing) => setSizingMode(sizing),
        mceTableCellType: actOnType((type) =>
          type === 'th' ? actions.makeCellsHeader : actions.unmakeCellsHeader
        ),
        mceTableColType: actOnType((type) =>
          type === 'th'
    ...
    editor.addCommand('mceTableApplyCellStyle', (_ui, args) => {
      ...
      const validArgs = filter$1(
        args,
        (value, style) =>
          editor.formatter.has(getFormatName(style)) && isString(value)
      )
      ...
      getTableFromCell(cells[0]).each((table) =>
        fireTableModified(editor, table.dom, styleModified)
      )
    })
  }
  ...
  const registerQueryCommands = (editor, actions) => {
    const isRoot = getIsRoot(editor)
    const lookupOnSelection = (action) =>
      getSelectionCell(getSelectionStart(editor))
        .bind((cell) =>
          table(cell, isRoot).map((table) => {
  ...
  const cata$1 = (subject, onBefore, onOn, onAfter) =>
  ...
  const selectNodeContentsUsing = (rng, element) =>
  ...
  const isLeaf = (universe) => (element) =>
    universe.property().children(element).length === 0
  ...
  const ancestor = (scope, predicate, isRoot) =>
    ancestor$2(scope, predicate, isRoot).isSome()
  ...
  const indexInParent = (element) =>
    parent(element).bind((parent) => {
      const children = children$2(parent)
      return indexOf(children, element).map((index) =>
        inParent(parent, children, element, index)
      )
    })
  const indexOf = (elements, element) =>
    findIndex(elements, curry(eq$1, element))

  const isBr = isTag('br')
  const gatherer = (cand, gather, isRoot) => {
    return gather(cand, isRoot).bind((target) => {
      return isText(target) && get$6(target).trim().length === 0
  ...
  const adjustDown = (bridge, element, guessBox, original, caret) => {
    ...
    if (Math.abs(guessBox.bottom - original.bottom) < 1) {
      ...
      return inOutsideBlock(bridge, element, caret)
        ? adt$1.retry(translate(lowerCaret, JUMP_SIZE, 0))
        : adt$1.none()
    }
  }
  const adjustUp = (bridge, element, guessBox, original, caret) => {
    ...
    if (Math.abs(guessBox.top - original.top) < 1) {
      ...
      return inOutsideBlock(bridge, element, caret)
        ? adt$1.retry(translate(higherCaret, JUMP_SIZE, 0))
        : adt$1.none()
    }
  }
  const upMovement = {
      ...
      return simulate(bridge, isRoot, direction, initial, anchor).map(
        (info) => {
          ...
          return Response.create(
            Optional.some(
              makeSitus(range.start, range.soffset, range.finish, range.foffset)
            ),
            true
          )
        }
        ...
        findCell(event.target, isRoot).each((finish) => {
          identify(start, finish, isRoot).each((cellSel) => {
            ...
            if (boxes.length === 1) {
              ...
              const isCellClosestContentEditable = is(
  ...
  const setRangeFromRelative = (win, relative) =>
    diagnose(win, relative).match({
      ltr: (start, soffset, finish, foffset) => {
  ...
  const byAttr = (ephemera, onSelection, onClear) => {
    ...
    const clearBeforeUpdate = (container) => {
      const sels = descendants(
        container,
        `${ephemera.selectedSelector},${ephemera.firstSelectedSelector},${ephemera.lastSelectedSelector}`
      )
  ...
  const Selections = (lazyRoot, getStart, selectedSelector) => {
    const get = () =>
      retrieve(lazyRoot(), selectedSelector).fold(
        () => getStart().fold(none, single),
        multiple
      )
    return { get }
  }
  ...
  const getUpOrLeftCells = (grid, selectedCells) => {
    const upGrid = grid.slice(
      0,
      selectedCells[selectedCells.length - 1].row + 1
    )
    const upDetails = toDetailList(upGrid)
    return bind$2(upDetails, (detail) => {
      const slicedCells = detail.cells.slice(
        0,
  ...
  const bind$1 = (element, event, filter, handler) =>
  ...
  const bind = (element, event, handler) =>
    bind$1(element, event, filter, handler)
  const fromRawEvent = fromRawEvent$1

  const hasInternalTarget = (e) =>
    !has(SugarElement.fromDom(e.target), 'ephox-snooker-resizer-bar')
  const TableCellSelectionHandler = (editor, resizeHandler) => {
    ...
    const onSelection = (cells, start, finish) => {
      ...
      tableOpt.each((table) => {
        ...
        const selectedCells = getCellsFromSelection(editor)
        const otherCells = getOtherCells(
          table,
    ...
    const clearSelectedCells = (container) =>
  ...
  const resizableRows = (warehouse, isResizable) =>
    bind$2(warehouse.all, (row, i) => (isResizable(row.element) ? [i] : []))
  const resizableColumns = (warehouse, isResizable) => {
    const resizableCols = []
    range$1(warehouse.grid.columns, (index) => {
      const colElmOpt = Warehouse.getColumnAt(warehouse, index).map(
        (col) => col.element
      )
    ...
    return filter$2(resizableCols, (colIndex) => {
      const columnCells = Warehouse.filterItems(
        warehouse,
        (cell) => cell.column === colIndex
      )
  ...
  const refreshGrid = (warhouse, wire, table, rows, cols) => {
    ...
    const resizableRowBars =
      rowPositions.length > 0 ? resizableRows(warhouse, isResizable) : []
    const resizableRowPositions = filter$2(rowPositions, (_pos, i) =>
      exists(resizableRowBars, (barIndex) => i === barIndex)
    )
    refreshRow(wire, resizableRowPositions, position, getOuter$2(table))
    const colPositions = cols.length > 0 ? width.positions(cols, table) : []
    const resizableColBars =
      colPositions.length > 0 ? resizableColumns(warhouse, isResizable) : []
    const resizableColPositions = filter$2(colPositions, (_pos, i) =>
      exists(resizableColBars, (barIndex) => i === barIndex)
    )
  ...
  const BarManager = (wire) => {
    ...
    const findClosestEditableTable = (target) =>
      closest$1(target, 'table', isRoot).filter(isEditable$1)
  ...
  const TableResizeHandler = (editor) => {
    ...
    const lazyResizingBehaviour = () =>
      isPreserveTableColumnResizing(editor) ? preserveTable() : resizeTable()
    ...
    editor.on('ObjectResizeStart', (e) => {
      ...
      if (isTable(targetElm)) {
        ...
        each$2(editor.dom.select('.mce-clonedresizable'), (clone) => {
          editor.dom.addClass(
            clone,
            'mce-' + getTableColumnResizingBehaviour(editor) + '-columns'
          )
        })
        if (!isPixelSizing(table) && isTablePixelsForced(editor)) {
          convertToPixelSize(table)
        } else if (
          !isPercentSizing(table) &&
          isTablePercentagesForced(editor)
        ) {
          convertToPercentSize(table)
        }
        ...
        startW = e.width
        startRawW = isTableResponsiveForced(editor)
          ? ''
          : getRawWidth(editor, targetElm).getOr('')
      }
    })
    ...
    const refresh = (table) => {
      tableResize.on((resize) =>
        resize.refreshBars(SugarElement.fromDom(table))
      )
    }
    const hide = () => {
  ...
  const setupTable = (editor) => {
    ...
    const cellSelectionHandler = TableCellSelectionHandler(
      editor,
      resizeHandler
    )
